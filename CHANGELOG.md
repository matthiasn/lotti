...
- **An edit from another device is no longer dropped from the conflicts
  ...
  open conflict resolved even when the other device's edit was not part of it,
  and a late copy of an older version could replace the version the conflict
  showed. A conflict now stays until a version that includes it is saved.
...
- **Retrying a failed sync item no longer sends an outdated change.** When an
  ...
  with it. Saving several changes to the same item in quick succession also no
  longer risks one of them being skipped. Queuing a change for sync is faster,
  because it no longer has to look up earlier sends first.
- **Notification text stays consistent across devices.** Marking an alert
  seen, acted on or deleted no longer changes which same-time text update wins
  during sync.
- **Other devices no longer wait for a change that never happened.** When a
  change was interrupted before it was saved, and the local sync queue could
  not record that it was cancelled, other devices kept waiting for it. The
  cancellation is now retried on the next startup or repair request.
- **Sync no longer drops some link and agent changes.** Changes saved without
  version information now travel on their own, so sending cannot discard one
  before the receiving device has resolved it.
...
- **One plain phrase for one thing.** Whether the app watches a person and
  ...
  the button reads "Remind me about" followed by their name.
...
- **The AI summary card keeps the summary and its proposals; the agent
  plumbing moved behind *Open agent internals*.** *Update now*, the
  next-update schedule, the automatic-updates switch and the model and
  provider the agent runs on are now in the internals panel, which the agent's
  name in the card header opens. On task and project detail the card speaks up
  about freshness only when the summary is out of date, and then it offers
  *Update now* beside the warning.
- **The photo viewer shows the time, not only the day.** A photo is often the
  only record of when something happened — when an event wound up, when you
  got there — and the date on its own could not answer that.
...
- **A briefing scheduled for a few minutes' time now arrives then.** Agent
  work waiting on a deadline was only noticed by an hourly sweep, so a
  briefing due two minutes after your last check-in could sit until the next
  hour came round.
...
- **The running-timer and recording indicators on the phone now float as one
  glass capsule above the navigation.** They used to be two square-bottomed
  tabs drawn to sit on the old full-width bar; over the new floating
  navigation they hung in mid-air above the chips, in colours the rest of the
  bar no longer uses. A timer shows as a red dot with its elapsed time, a
  recording as the live level orb with its elapsed time, and both at once
  share the capsule. Tapping the timer opens its entry and tapping the
  recording reopens it, exactly as before, and the capsule reads the same
  over the classic tab bar. Nothing shows while nothing runs.
- **Editing a saved check-in no longer offers Dictate.** Once a check-in is
  text, it is edited as text, and the button no longer raises the question of
  what a recording would do to the words already there. A new check-in still
  offers Dictate, and a recording is added below anything already typed.
- **"Ask about this task" is now a compact Chat button beside Read more** on
  the task's AI summary card, instead of a wide button on its own row below.

### Fixed

- **Calls and meetings logged as check-ins now show up on the Day view.** A
  check-in is time spent with someone, but the Day timeline never asked for
  check-ins, so a tracked 1 h 15 call was missing however long it ran. A
  check-in with a length now sits on the recorded lane under the person's
  name, in their category's colour. One saved with *No duration* stays off
  it, like any instant note.
- **The check-in text field drew two frames while typing.** A second outline
  appeared inside the field's own border as soon as the cursor was in it.
  The field now shows one.
...
- **The project world can now be walked in daylight.** The district was built
  for one hour of the day: a neon city at night, where a task's health was
  told in lit windows, roof lanterns and pools of lamplight. A **Night | Day**
  switch in the world's controls now rebuilds it under a clear mid-morning sky
...
- **Ask your task, project or category agent about earlier notes and
  meetings.** Separate conversations find relevant material within the same
  category and show summaries with expandable saved quotes. Dictate an
  editable question, revisit or archive chats, and choose whether to keep
  their conclusions when deleting them. Private sources and the answers
  derived from them stay hidden whenever private entries are hidden.

### Changed

- **Explore category moved into the category header.** In the Projects list
  the way into a category's world was a button on a row of its own under every
  category, repeated down the pane, and it disappeared whenever the category
  was folded. It now sits in the header's trailing corner beside the fold
  control, where the list's other row actions are, and stays reachable when
...
- **A database integrity check in Settings → Advanced → Maintenance.** It
  verifies that the journal, sync, agent, editor and search files are sound
  and names any that are not.
- **Swipe to decide the project agent's proposed changes.** Proposed changes
  on a project page now swipe like the ones on a task page and like the
  suggested next steps beside them — right to confirm, left to reject, with
  the action named on the band the swipe reveals.
...
- **Dismissing an AI-suggested next step did not make it go away.** The
  ...
  triaged looked exactly as long as one you had not. A dismissed suggestion
  now leaves the list immediately. Nothing is lost: it moves to the band's
  history, which opens under the remaining suggestions, and that is where it
  can be restored.
...
- **The Project Tasks header was laid out wrong.** The sort control and "Add
  task" floated in the middle of the row instead of sitting against the right
  edge, and the total estimate was wedged between the heading and its count.
  The header now reads as the title with its count on the left and the
  estimate, sort control and "Add task" together on the right, all on one
  line.
- **Month headings cut the task card's outline open.** Each collapsible group
  heading painted over the card's left and right edges, leaving the card
  border visibly interrupted at every group. The outline is now continuous.
- **AI analyses on image and audio entries were set too large.** An analysis
  nested inside an entry card was rendered bigger than the entry's own text
  and padded like a top-level card. It now matches the text editor's size and
  sits in a tighter card.

### Security
- **AI provider API keys are now protected by platform secure storage.**
  Existing plaintext keys are migrated on first use, while encrypted sync
  continues to keep provider configurations working across devices.
...
- **Purging deleted entries and rebuilding search no longer slow down on a
  ...
  progress visible. They now walk the journal in fixed-size chunks and report
  progress as they go.
...
- **An interrupted database upgrade could leave the journal half-migrated.**
  ...
  applied stayed while the version number did not advance, and the next launch
  would try to apply them again. The whole upgrade now either completes or
  rolls back to the version that was running.
...
- **Delayed log writes stay with their original profile.** Switching profiles
  while disk writes are blocked no longer sends older buffered records into the
...
- **Database backups could miss your latest changes.** The copy taken before
  a database upgrade, and the one made when purging deleted entries, copied the
  database file alone — while SQLite may still be holding the most recent
...
- **Sync handles out-of-order labels and pending writes more reliably.**
  Missing label definitions no longer fail background sync, and a pending local
  write is not incorrectly reported to other devices as an unused sequence
  number.

## [1.0.29]

### Added
- **A project's tasks are grouped and sortable.** The task list on a project
  page now groups by the month each task was created, newest month first, and
  folds finished tasks into a collapsed "Done" group at the end so a long
  project leads with open work. Every group shows its task count and total
  estimate, collapses on tap, and its header stays pinned at the top while its
  tasks scroll past. A new "Sort and group" control — a popover on desktop, a
  sheet on a phone — offers grouping by status, priority or due window, or no
  grouping, and ordering by actionability, creation, due date, estimate,
  priority, last update or title, plus a switch to keep done tasks in their
  groups. The choice, and which groups are folded, is remembered per project.
...
- **The project agent's next steps now say what each button does, and what
  ...
  **Add task**, which creates a project task and links the step to it, and a
  ...
  instead of disappearing, an added step's tag lights the new task up in the
  task list and scrolls to it, and both decisions can be undone — an addition
  for eight seconds, a dismissal as long as the list is current. A failed
  addition keeps the step with a Retry and says why. **Add all as tasks** and
  **Dismiss all** replace "Confirm all". The agent's proposed changes, such as
  a status change, now sit in their own band under the steps. Phones show
  three steps at a time with a "Show more" link, a list you finished earlier
  collapses to a one-line summary with its history, and an empty list says
  when the agent last looked.
...
- **Deciding a project suggestion no longer re-renders the whole project
  ...
  added or dismissed.
- **The Project Tasks header no longer breaks on narrow screens or with large
  text.** The title truncates instead of overflowing, and on phones, narrow
  desktop panes and large text the header drops its total estimate (each group
  shows its own) and turns Add task into an icon.
...
- **The day-view column beside the tasks list is now opt-in, and it can step
  through days.** On desktop the column that shows a day's planned and recorded
  time next to your tasks used to open by default; it now starts tucked away as
  a slim rail, and comes up with the calendar button when you want it. Once
  open, it no longer shows today only: the same date controls as the planner —
  previous and next arrows, and a date you can tap to pick any day or hold to
  jump back to today — sit in its top row beside the lane toggle, in a compact
...
- **The day-view column's toggle vanished from the Tasks tab whenever a task
  ...
  open task instead of disappearing. When the window has no room for the task
  list, the column and the open task side by side, the task list tucks away
  behind its usual "show list" button until the column is hidden or the
  window is wider.
...
## [1.0.23]
### Added
- **Your day is now always in view next to your tasks.** On desktop, a
  day-view column docks on the right edge of the Tasks tab, showing today's
  planned time next to the time actually recorded — the same view the Daily
  OS day surface uses. A calendar button at the top hides it to a slim rail
  or brings it back, its edge drags wider or narrower, and when it gets
  narrow the planned and recorded lanes become swipeable pages. The column's
  visibility and width survive app restarts.
- **The tasks list gained a compact mode.** A toggle below the search and
  filter row switches between the familiar full task cards and a terse,
  line-per-task list showing just the titles — easier to scan when the day
  view column is open. The chosen mode is remembered across restarts.
- **Lock the desktop app down to a single category for demos.** Tapping the
  Lotti logo at the top of the desktop sidebar opens a hidden menu listing your
  categories. Pick one and the app shows only that category: the sidebar keeps
  Tasks, Habits, Insights, Goals and Logbook, each filtered to the chosen
  category, while Settings, saved filters, the activity panel and every other
  tab disappear, and keyboard shortcuts and the command palette cannot reach
  them either. It is a demo aid rather than a privacy wall: project and label
  names in the task filter sheet and entries linked from another category can
  still show. The same logo menu offers "Exit lockdown" to bring everything
  back, and restarting the app always exits it too.
...
- **The day squares under each habit now say which day they are, and open
  that day.** Every square carries its weekday (Tu, Th, Sa, Su — in your
  language) until something is recorded for it, then a tick for a kept day or
  a red cross for a missed one — so a missed day no longer looks like a day
  you never looked at. The square itself stays grey, so a rough week is a few
  red marks rather than a red row: enough to sting, not enough to shout.
  Tapping a square opens the completion sheet for that day rather than for
  today, which is what makes catching up on Tuesday from Thursday a one-tap
  job. The squares are a size larger everywhere, and on a desktop window a
  row shows the last fourteen days instead of seven.
...
- **Habit day squares on a goal page stop at today.** A habit measured over a
  ...
  yet, a date span dated into the future, and — because the squares share the
  card's width between however many days there are — squares visibly smaller
  than the Goal days strip above them. Every day track now ends on today, on
  the same grid, and the signal charts follow the same rule.
...
- **A recording that cannot be transcribed says so.** Asking for a transcript
  ...
  nothing is spent, and Retry is there when you want that one transcribed. And a
  recording that nothing ever picked up — because it was made before this
  worked, or because the attempt never started — stops claiming to be in
  progress after ten minutes: it reads "Not transcribed", with a Retry beside
  it, and the page updates itself the moment that happens.
...
- **Goal chat now remembers and reliably answers.** Recent exchanges reach the
  ...
  out the goal's current context, while failed check-in summaries back off
  instead of being retried and billed on every update.
...
- **The goal detail page is a dashboard, with the agent as a drawer.** The
  ...
  "This week" above the agent's read (now carrying an "as of" timestamp),
  each at the column's full width,
...
- **A failed agent update says why.** When the goal read cannot refresh —
  ...
  "Out of date" under a button that looks dead. The line clears with the
...
- **Knowledge-graph zoom now stays anchored under the cursor.** Two-finger
  trackpad zoom (and pinch) kept sliding the graph while zooming because the
  fixed point was computed in window coordinates instead of the canvas's own —
...
- **The task knowledge graph is now a readable local workspace.** Instead of
  ...
  control what remains visible. Task cover art and photo nodes now appear at
  twice the standard node diameter, turning them into useful visual landmarks
  instead of icon-sized textures. The compact task inspector surfaces cover art
  and related photos, keeps long titles intact, and leaves the detailed AI brief
  collapsed until requested. Keyboard, reduced-motion, high-contrast, and
...
- **The task page's Add menu speaks the same language as the page.** The menu
  ...
  and "Record a voice note". One action now has one name wherever it appears,
  every row is a verb, and every row says in one quiet line what the tap does
  — "Add a task" explains that it creates a task linked to this one, "Capture
  a screenshot" says what is captured and where it goes, "Start a timer"
...
- **Setting a category is now an offer, not a report.** An uncategorized task
  ...
  chip now sits with "Set due date" and "Add estimate", and the breadcrumb
  appears once the task actually belongs somewhere. The chips themselves split
  into two tidy rows — what the task *is* (status, priority, set values),
  then what you can still add — instead of wrapping wherever the window edge
  happened to fall, and the priority chip gained the same little arrow the
  status chip has, so it reads as changeable. The dashed chips are also drawn
  more visibly in both themes — they were nearly invisible against a white
...
- **Assistants for past days stop waking up.** Daily OS gives each day its own
  ...
  to the planner, and then retires. Those days stay readable; their assistant
  simply does not think again on its own.
...
- **See and manage every device in your sync account.** The sync status page
  ...
  a hint when a device has been silent long enough to be presumed dead.
...
- **Sync finishes catching up after a large or still-growing backlog.** A
  ...
  It also remembers the oldest unresolved encrypted message across restarts,
  so a newer applied message cannot move recovery past a gap that still needs
  its decryption key.
...
- **AI-generated work now carries a consistent attribution record.** Coding
  ...
  usage, timing, interaction evidence, and provider-reported cost. Attribution
  is preserved across encrypted sync. Existing work without attribution stays
  unattributed rather than receiving a guessed creator or cost.
...
### Added
- **Italian is now available across Lotti and its Manual.** The app follows
  Italian system locales, and the published Manual now has an Italian route
  with matching localized screenshots. Settings → Advanced → Language can also
  select any published Manual language explicitly.
...
- **Pause and resume audio recordings mid-capture.** The recording sheet now
  has a pause control next to stop and cancel, so you can stop talking to
  gather your thoughts and pick the recording back up without splitting it into
  separate entries. While paused, the elapsed timer and level meter freeze;
  resuming continues the same recording. (Available for standard recordings;
  live-transcription recordings are unaffected.)
...
- **Settings controls now speak the same visual language as the rest of the
  ...
  keeping its colour on a small avatar dot. The Sync conflicts and outbox
  lists now sit on the same dark background as the rest of Settings instead
  of a near-black sheet (with their cards reading as elevated), and the Sync
  Outbox row shows its pending count as a clean badge rather than a postbox
  glyph with the number squeezed on top.
...
- The Habits tab was reworked to match the rest of the app. It now uses the same
  ...
  shows, at a glance, how many habits you've done today ("2 / 5"), how many are
  left ("3 to go"), and your current streaks. Your overall consistency now reads
  as a scrollable heatmap — a calendar of week columns, month-labelled along the
  top, where each day is shaded by how many of your active habits you completed
  that day, the way a contributions graph works. It spans the full width of the
  window and scrolls back through months and years with today pinned to the right
  edge; quiet (not-yet-done) days stay a plain neutral rather than looking like
  failures, and every cell announces its date and count to screen readers. The
  habit list itself is now lean and action-focused: each row shows its own streak
  ("🔥 23"), a clear "＋" button to log it in one tap (turning into a green check
  once done), and you can still swipe right to mark it done or left to mark it
  ...
  the day lights the summary card with an all-done glow. The whole celebration
  respects the system "reduce motion" setting and announces completion to screen
  readers.
