...
- Mistral Image Generation: Added Mistral as an alternative image generation provider for EU data sovereignty. Uses the Mistral Agents API with FLUX1.1 pro Ultra for image generation, with all data processed on European infrastructure. Auto-detected when Mistral is configured — no additional settings needed.
