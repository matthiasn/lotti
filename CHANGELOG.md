...
### Changed
- **Goal signal charts now show the trend behind the number.** Daily steps and
  weight plot both the recorded values and their trailing seven-day average,
  with separate legends for actuals, averages and targets plus an
  above/below-average direction read in the context of the goal. Habit strips
  and every signal chart now share the same plot bounds, and the steps card
  distinguishes daily steps from the average it evaluates.
