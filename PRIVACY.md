...
### Local Storage
- All data is stored locally on your devices in SQLite databases
...
- Your logbook is an ordinary SQLite file on your own disk. There is no export wizard, because there is nothing to unlock: you can read the database directly with any SQLite client. Take a consistent copy with `VACUUM INTO` while the app is running, and copy the attachments directory alongside it, since audio and images live on the filesystem rather than in the database. On phones the database sits inside the app sandbox, so the practical route is to pair a desktop and copy from that replica.
- There is no server-side backup and no account recovery, because there is no account. Every device you pair holds a complete replica, so pairing a second device early is the redundancy story — and an ordinary device backup covers the rest.
...
- **How pairing actually works.** The QR code you scan carries a pairing bundle — homeserver, account id, room id and a live account password — from a device that already syncs to one that does not. Treat that code as the credential it is: letting your own new device scan it is fine; keeping a screenshot of it or sending it through a chat app is not. Both devices then display a six-character check code derived from the account, room and homeserver. That code is a recognition aid that catches a wrong-code mistake; it is not a security control.
...
- **Before an AI feature runs**, the privacy notice names the provider it is configured against and whether processing is local or remote. A friendly display name is not the boundary — the provider detail view shows the real base URL, the models attached to it, and every profile depending on them.
- **Usage & Impact** records every request that left the machine, which model served it, and what it cost in money, energy and CO₂e. If something reached a provider, it is in that log.
...
- ❌ No sharing with third parties, except the AI providers you explicitly configure
- ❌ No backend servers collecting your data

Diagnostic logs are local too. *Settings → Advanced → Logging* controls what is recorded, and the files stay on your device until you choose to share them.
...
- **Deletion**: delete any or all data in the app; deleting the database files removes everything
