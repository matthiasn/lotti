...
2. Update the matching file for every published locale under
...
7. Review the production build locally with `make manual_serve`, including the
   language selector and every localized route.

Validation requires every published translation tree to contain the same page
paths as the English source tree. It also rejects translated page bodies that
are identical to English, so new pages cannot quietly ship as untranslated
copies. The public English URLs remain unchanged; other languages are
published below their own versioned locale prefix.
...
links to screenshot media — legacy repository images or the R2 bucket
