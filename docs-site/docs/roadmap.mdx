...
Relationship data will get the strictest treatment Lotti has: it will be
stored locally on your devices, never on a Lotti server — there is none. If you
turn on sync, entries will travel end-to-end encrypted between your devices, so
the relay server would only ever handle ciphertext. Briefings run on local AI
