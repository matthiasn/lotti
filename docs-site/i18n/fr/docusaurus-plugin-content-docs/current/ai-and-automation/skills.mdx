...
<TutorialVideo
  ...
  title="Transformer une tâche de code en prompt prêt pour l'IA"
  caption="Vidéo commentée : la compétence Générer un prompt de code assemble la description et la liste de contrôle d'une tâche en un prompt prêt à l'emploi."
/>
...
Lotti propose cinq types de compétences ; le menu affiche celles que l’entrée prend en charge :

| Compétence | Apparaît sur |
| --- | --- |
| **Transcription** | entrées audio |
| **Analyse d’image** | images |
| **Génération d’image** | entrées audio et texte liées à une tâche |
| **Génération de prompt** | tout ce qui porte du texte — notes, audio, images, tâches |
| **Génération de prompt d’image** | tout ce qui porte du texte, comme ci-dessus |

Si une action attendue est absente, le type d’entrée ne prend pas en charge cette compétence — ou aucun profil ne lui attribue de modèle.
...
peut mal interpréter une urgence de sardines pourtant très sérieuse.
