...
la capture de la première tâche et les surfaces de capture vocale suivantes. Tu
pourras le modifier dans **Paramètres → Préférences → Style d’enregistrement**.
...
normale. Tu peux alors examiner les propositions de liste de contrôle — elles n’existent que si la catégorie de la tâche a un modèle d’agent par défaut, que l’intégration attribue aux catégories qu’elle crée dès lors que tu as connecté un fournisseur d’IA ; une catégorie créée plus tard dans les Paramètres n’en a pas tant que tu ne lui en assignes pas un, et la liste de contrôle est alors abandonnée plutôt que proposée —, ajouter
