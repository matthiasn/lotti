...
1. Ouvre **Paramètres → Définitions → Étiquettes**.
...
## Laisser un agent de tâche suggérer des étiquettes

Lorsqu’un agent de tâche configuré dispose d’assez de contexte, il peut suggérer des étiquettes pertinentes parmi d’autres modifications de la tâche. Lotti ne rend disponibles que les étiquettes actives valides pour la catégorie de la tâche, et rejette les suggestions supprimées, hors champ, en double ou de faible confiance. L’agent ne dispose jamais d’un moyen libre de créer des étiquettes arbitraires à partir d’un prompt.

Examine une étiquette proposée par l’agent dans la zone des modifications proposées de la tâche, exactement comme un changement de liste de contrôle, de date ou d’état. Confirme-la lorsqu’elle améliore le filtrage futur ; ignore-la sinon. L’organisation visible de la tâche reste ainsi sous ton contrôle au lieu de traiter une supposition du modèle comme une vérité.
