...
Sur téléphone, la carte suit un ordre de lecture unique : identité, contrôles de mise à jour, configuration IA, rapport, puis modifications proposées et historique. Sur ordinateur, le rapport et les propositions forment la colonne de lecture tandis que les contrôles compacts restent à proximité.
...
La ligne **Configuration actuelle** de la carte identifie le profil, le modèle et le fournisseur utilisés pour son prochain réveil. Sélectionne-la pour choisir un profil d’inférence enregistré, remplacer uniquement le modèle de réflexion, revenir au réglage par défaut du profil, copier la configuration actuelle de la catégorie ou désactiver l’IA pour ce seul agent. Un choix reste attaché à cet agent pour les mises à jour futures ; copier la configuration d’une catégorie est un instantané, pas un lien qui changerait silencieusement plus tard.
