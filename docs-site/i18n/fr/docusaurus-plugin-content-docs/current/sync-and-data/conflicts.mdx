...
Ouvre **Paramètres → Paramètres de synchronisation → Conflits de synchronisation**. La liste sépare les éléments
...
- **Utiliser la version synchronisée** conserve l’intégralité de la version
  entrante.
...
Sélectionne **Combiner…**, choisis la version de départ sous **Partir de**, puis
bascule les champs individuels vers l’autre côté. Les champs qui ne peuvent pas
être fusionnés indépendamment suivent la version de départ sélectionnée.
...
que l’autre l’a modifiée, Lotti affiche **Supprimé sur un appareil** au lieu
d’une fusion de champs ordinaire : rien n’est retiré tant que tu n’as pas choisi
**Garder la version modifiée** ou **Confirmer la suppression**.
