...
<div className="manual-hero manual-hero--model">
  ...
  <h1>Sua vida não é uma lista de pendências.</h1>
...
<ManualScreenshot
  ...
  caption="Um bloco do Daily OS diz o que você escolheu para hoje. A tarefa vinculada e o registro de tempo continuam sendo coisas próprias e honestas."
/>
...
| Tarefa | Um resultado que precisa de progresso | Preparar o lançamento V1 |
| Lista de verificação | Passos concretos dentro de uma tarefa | Verificar as notas de lançamento |
...
Um agente pode ler o contexto que você fornece, formar uma opinião, resumir
uma tarefa bagunçada e propor uma próxima mudança útil. O relatório dele é uma
opinião com procedência, não um fato novo na sua história. Alterações
propostas de tarefa, lista de verificação, status ou data aguardam que você as
confirme ou descarte. As exceções são o título e o idioma: cada um é aplicado
imediatamente quando *aquele campo* ainda está vazio e o agente o define pela
primeira vez. O restante da tarefa pode estar cheio de trabalho — uma tarefa com
descrição, listas de verificação e tempo registrado, mas sem idioma definido,
tem o primeiro idioma aplicado sem perguntar. Depois que um campo tem valor,
vale a mesma regra de aprovação humana.

Isso não é mera cortesia embutida em um prompt. Lotti mantém o diário de bordo
pessoal e a memória de trabalho do agente separados, para que o agente não
possa tratar seu histórico como um bloco de rascunho. Você pode usar a IA,
ajustá-la ou simplesmente deixá-la de lado.
...
5. Revise hábitos e painéis em busca de padrões, em vez de dias individuais
   perfeitos.

## Mantenha a propriedade clara

- Uma tarefa vinculada possui seu título e categoria.
- Um bloco do Daily OS possui seu posicionamento e duração.
...
Se o plano mudou, deixe-o mudar. Se o dia saiu dos trilhos, registre o que de
fato aconteceu. Um sistema útil ajuda você a ver o formato da sua vida; ele
não pede que você encene uma vida mais produtiva para o banco de dados.

:::

Em seguida, [crie uma primeira tarefa](./first-task.mdx) ou vá diretamente
para o [Daily OS](../plan-and-capture/daily-os.mdx).
