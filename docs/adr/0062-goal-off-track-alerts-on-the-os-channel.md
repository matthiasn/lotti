# ADR 0062: Goal Off-Track Alerts on the OS Channel
...
about, and every kind arms rows through one producer contract (ADR 0061).
...
- The goal gained an OS channel for four hooks and one call: the producer
  ...
  calls the sink after its transaction. No repository change (ADR 0061's
  promise, kept).
...
- [ADR 0061: Notification Producers — One Episode Contract](./0061-notification-producers-one-episode-contract.md) — the contract this rides on
