...
route ADR 0067 named: make the tools' effects idempotent across devices, so
that the second application changes nothing. Decision 2 and 3 name the tools
this covers; the ones it does not are listed under Consequences.
...
2. **A create-style tool derives the id of what it creates** from the key and
   ...
   that knows the same deletions. The checklist is resolved only when an item
   still needs creating, so a repeated application whose items all exist
   creates no checklist, even after the user deleted the first one.
...
- ADR 0067's first residual is narrowed. An item decided on two devices is
  still dispatched on both, but for the tools of decisions 2 and 3 the
  journal ends up as if it was applied once: `NoDuplicateEffects`, `NoClobber` and `EffectsConverge` hold for two
  ...
  - **A checklist its creator never linked.** `createChecklist` writes the
    checklist and then the task update that lists it. A device that crashes
    between the two leaves a checklist no task lists, and its items out of
    sight, until someone adds an item to it: only a replay that still has an
    item to create writes the listing, and a replay with nothing to add does
    not repair it (TLC: `ChangeSetLifecycleRaceLink` with `CrashBeforeLink`
    violates `EffectsLinked` in 5 states). A replay with nothing to add stays
    a pure no-op on purpose: relinking would write the task concurrently with
    the creator's own listing, which is on its way, and raise a journal
    conflict (the model: a device that holds the checklist but not its link
    and writes nothing keeps `EffectsLinked`, 241 states).
  - **The reuse path's task write can raise a task conflict.** A replay that
    reuses a live derived checklist because an item still needs creating
    lists it on the task, concurrently with the creator's listing in the
    race window. That conflict is chosen deliberately: without the write, a
    creator that crashed before its listing would leave the new item in a
    checklist the task never shows.
