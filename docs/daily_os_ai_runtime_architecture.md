...
| --- | --- | --- |
| Immutable, append-only agent message log | `AgentMessageEntity` in `agent.sqlite` | **Built** — log persists; the `prevMessageId`/`messagePrev` causal-chain fields are *defined but not yet populated* (see gaps) |
...
| Memory compaction | `AgentMessageKind.summary`, `summaryStart/EndMessageId`, `summaryDepth`, `recentHeadMessageId`, `latestSummaryMessageId` | **Scaffolding only — not yet written by production** (agents README: "prepared, not active") |
...
- **Message causal chaining is not wired.** `prevMessageId` is declared on `AgentMessageEntity` and the `messagePrev` `AgentLink` variant exists, but no production code populates either — so "the log is a DAG" (§8) is a target built on existing fields, not current behavior.
...
1. When the tail past the active checkpoint exceeds a model-specific budget, **append a `summary`/checkpoint event** naming the **frontier** it covers (and the prior checkpoint it folds in) — an append, not a pointer write.
2. The **projection** selects the active checkpoint and head by reverse-indexing the log (ADR 0016); `recentHeadMessageId`/`latestSummaryMessageId` are a **local cache, never authoritative**, so compaction adds no mutable conflict surface. Preserve decisions, open commitments/negotiations, and non-negotiables; discard redundant tool chatter (per [Anthropic context engineering](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents): "overly aggressive compaction … loses subtle but critical context").
3. **Treat the summary as a derived projection, not a destructive overwrite — and don't expect a digest to make LLM outputs converge.** Two independent summarizations will *not* be byte-identical (a digest names an output; it can't make two equal). Summaries are **candidate checkpoints over causal frontiers** (§8): a summary covers a *frontier* (an antichain). Frontiers join cleanly (a semilattice), but the **join of two candidates may have no materialized text**, so the **active checkpoint is the maximal complete materialized checkpoint** — among checkpoints ancestral to every head, the one covering the most history; context is that summary + the **uncovered tail of original (non-summary) events** (non-active summaries excluded, so no derived text or double-counting). Same-frontier candidates share a `frontierDigest` (coverage only), so they tie-break by a separate content key — lowest `(contentDigest, id)`; **incomparable maxima** fall back to their **meet (common base)** + full verbatim uncovered region, which a **merge-summary** then collapses into one dominating checkpoint — fired *eagerly* if that verbatim region would exceed the on-device context budget (still append-only checkpoints over canonical frontiers — iteratively over bounded sub-frontiers if the region is already huge — original events retained; eager in *timing*, not a truncation), so the fallback never overflows (so a merge-summary always eventually becomes active). Never pick one candidate's text as the checkpoint when candidates are incomparable — that drops the other branch's history. `frontierDigest` (a hash of the antichain's canonical id-set) identifies coverage only; a separate **`contentDigest`** over a **canonical form** (sorted keys, RFC 3339 UTC timestamps, normalized numbers, UTF-8 canonical JSON/JCS, versioned tag `sha256-v1`) is the dedup/verification key, so the checkpoint's full key is `(frontierDigest, contentDigest)`. The lease (§8) only avoids *usually* summarizing twice; it is not required for convergence.
...
- **(a) Application-level response memoization — the layer Lotti owns.** `hash(rendered prompt + pinned model id + decoding params + resolved tool set + soul config) → stored response`, persisted alongside the immutable log and looked up *out of order* by request hash. Provider-agnostic (identical whether Ollama or an EU endpoint), it is the **determinism boundary** for any non-local tier and the mechanism the anchor paper's deterministic replay and cheap forking depend on. This is the realizable version of "key = the request, value = the response." **Two purposes, two keys.** For deterministic *replay/forking* the key is the **verbatim** rendered prompt — volatile fields like `currentLocalTime` included, so a miss when the clock advances is expected and harmless. For *active de-duplication* of concurrent duplicate calls, hashing the volatile prompt would miss on **every** wake, so dedup instead uses the **frontier-based behavior idempotency key** (`agentId + behaviorKind + frontierDigest + triggerId + toolName`, §8), where `triggerId` is a **stable, source-derived** id (source event id / scheduled-wake entity id / shared trigger token, **never** a per-run local UUID) — so the key collapses *the same wake run on two devices*, **not** two distinct wakes (e.g. a later scheduled, time-sensitive wake) over an unchanged frontier. It drops the wall-clock *value* but stays scoped to the wake epoch. And as a prompt-construction rule, volatile fields (`currentLocalTime`) belong in the **very last, volatile position** of the prompt, never in the stable prefix, so the *KV/prefix* cache (b) stays warm regardless.
...
1. Extend the partial order — the causal `messagePrev` DAG (the vector clock *classifies* concurrency but is not the ordering input) — to a single deterministic **total order** with a replica-independent tiebreak (a stable `hostId + id` key on otherwise-concurrent events) so every device linearizes concurrent events identically.
2. Make the LWW comparator a true total order with a deterministic secondary key — **break equal `updatedAt` by `hostId`** (then `id`) so two replicas never disagree when timestamps are identical. These are *two distinct* problems, and the tiebreak only fixes the first: **(a) equal-timestamp divergence** — solved by the `hostId`/`id` tiebreak; **(b) a skewed/fast physical clock winning a concurrent branch outright** — *not* solved by the tiebreak, because the fast clock's `updatedAt` is strictly greater, so the equal-timestamp tiebreak never fires. Bounding (b) requires a monotonic clock — a **hybrid logical clock** (or bounded physical-clock drift) on the concurrent-branch comparator. That is the one place an HLC earns its keep; it still does not replace the vector clock used for causal classification.

**Replicate facts; lease execution — but the lease is an optimization, not the correctness foundation.** Pure log appends converge for free under CRDT semantics, and correctness comes from the convergent DAG projection (below), *not* from mutual exclusion. The lease only avoids *usually* doing duplicate expensive or user-visible work and guards irreversible side effects. So **side-effecting actions take a leader lease keyed to the side-effect boundary** — `(agentId, behaviorKind)` for per-agent execution/compaction, `(userId, dayId, planner)` for the shared day planner — with a **monotonic fencing token** the resource rejects when stale ([Kleppmann](https://martin.kleppmann.com/2016/02/08/how-to-do-distributed-locking.html)). This is *not* new machinery: `WakeOrchestrator` already does **vector-clock self-suppression**, and the lease promotes that from intra-device to inter-device. When the lease fails (split-brain mobile+desktop) nothing is *wrong* — both devices act and the work converges; **idempotency + fencing, not the lease, is the safety net.**

**Forks are legal — the log is a convergent DAG.** Two devices waking the same runtime from a shared prefix both append, creating a fork: concurrent `messagePrev` children of a shared parent. This is normal, not corruption, and the repair is mostly *free* — the projection folds the whole event **set** in canonical order (a deterministic linear extension of the `messagePrev` DAG), so it is **multi-head tolerant**: any two devices with the same events compute identical state regardless of branching. `prevMessageId` is provenance, not a linearization lock; `recentHeadMessageId`/`latestSummaryMessageId` are *derived by reverse-indexing `messagePrev`*, not authoritative fields. What needs real work is bounded *cost* and *meaning*, not correctness:

- **Cost — heal the head lazily.** Unbounded heads make context and the prefix grow, so a **join-by-continuation** node (linking to all current heads) caps the fork — emitted **lazily** (only when ≥2 heads survive past one wake cycle). Its **id is content-addressed** — `hash("join-v1" + sorted parent-head ids)`, domain-separated from the summary coverage `frontierDigest` — so two devices emitting the join concurrently write the *same* log entry and set-union merges them into one node — concurrent joins can't themselves fork (no join storm). For that to hold, the join's **payload is deterministic** (sorted parent-head ids + a fixed kind, no clock/host in the content) and the projection reconciles per-device envelope metadata (`hostId`, VC, `createdAt`) canonically, so the shared id can't diverge under LWW. For prefix stability, not correctness.
- **Summaries** are candidate checkpoints over causal **frontiers** (antichains); the active checkpoint is the **maximal complete** materialized checkpoint ancestral to all heads (common-base fallback for incomparable maxima), with the uncovered tail of original events read verbatim until a lazy merge-summary collapses it — so two hosts summarizing the same trunk converge without needing byte-identical text (§6).
- **Side effects** carry an idempotency key `agentId + behaviorKind + frontierDigest + triggerId + toolName` — scoped to the wake epoch (`triggerId`), not just the frontier, so a later time-sensitive wake isn't wrongly suppressed; the projection dedups duplicates (ChangeSet dedup, ADR 0009). Irreversible external effects (a sent email) can't be undone, so those stay behind the lease + gate, kept minimal.
- **Meaning — reconcile visibly.** Derived state converges silently by canonical pick; *divergent user-facing commitments* (one device marked a task done, another proposed dropping it) surface as a `ChangeSet` ("your devices disagreed — here's the reconcile"), never silently overwritten.
...
flowchart TD
  ...
  Sync -->|union merge| PB
  Sync -->|union merge| BA
  PB -->|"skips redundant execution while A holds the lease"| Suppressed["(connected-case optimization)"]
```
...
Realized as ADRs **0016** (agent state as a projection), **0017** (deterministic compaction), **0018** (convergent execution: lease, fencing, fork reconciliation), and **0019** (attention-negotiation protocol). These extend the existing ADR series (0002 wake scheduling, 0008 inference profiles, 0009 redundant-proposal suppression, 0010 scheduled wakes).
...
- **Summary correctness under sync (B/G).** Recursive summarization can amplify hallucination at depth; each summary stores provenance *and* a replay hash so a bad one is regenerable. Cross-device convergence is now handled (§6, §8): summaries are candidate checkpoints over causal frontiers, the active checkpoint is the **maximal complete materialized checkpoint** (common-base fallback for incomparable maxima) + a verbatim tail of original events, and a lazy merge-summary collapses divergence — so byte-identical LLM output is *not* required. The open part is tuning the merge-summary trigger and bounding verbatim-tail growth, not convergence itself.
...
**Lotti mapping.** The dormant scaffolding maps almost 1:1 onto the proven MemGPT/Letta mechanism: `AgentMessageKind.summary` is the recursive-summary node; `summaryStart/EndMessageId` is provenance for the evicted range; `recentHeadMessageId` is the verbatim-tail boundary; `latestSummaryMessageId` lets a stateless wake reconstruct context as frozen-summary + recent-tail. Implement a deterministic in-app compaction policy: when the tail past `recentHeadMessageId` exceeds a model-specific budget, append a checkpoint event naming the covered frontier (the projection selects the active checkpoint; the persisted pointers are a local cache — §6), and preserve decisions, open commitments/negotiations, and non-negotiables while discarding redundant tool chatter. Treat summaries as candidate checkpoints whose cross-device reads converge by selecting the maximal complete materialized checkpoint (common-base fallback for incomparable maxima) plus a verbatim tail of original events, collapsed by a lazy merge-summary — not by content-addressing LLM text (§6, §8). Run summarization and the honest actual-vs-intended reflection as a separate background identity writing into the same log.
...
> **Compliance caveat — verify against primary sources before relying.** These provider claims are point-in-time and several lean on secondary write-ups (the Mistral retention summary above is a third-party article, not the primary DPA). Primary docs confirm the load-bearing *shape*: Mistral's [30-day abuse-monitoring retention](https://help.mistral.ai/en/articles/347628-how-long-do-you-store-my-data) and [Zero Data Retention option](https://help.mistral.ai/en/articles/347612-can-i-activate-zero-data-retention-zdr); Scaleway's [default ZDR and no-train](https://www.scaleway.com/en/docs/generative-apis/reference-content/data-privacy/); IONOS's [German-only processing, no-train per DPA](https://docs.ionos.com/cloud/ai/ai-model-hub/governance-and-compliance/eu-ai-act). But specific superlatives — Mistral "EU by default" and "no training on paid-API data by default", OVHcloud "only billing data" / "never trains on prompts", and any model-version *immutability* guarantee — are **not** explicitly confirmed by those primary pages and must be checked against each vendor's current DPA/terms (with an as-of date) before they inform a residency or cache-determinism decision. This restates §11's open question (D) at the point where the claims live.
...
**Lotti mapping.** The codebase confirms Lotti sits on the correct side of every line. `lib/features/sync/vector_clock.dart` implements a real version vector: a four-way `VclockStatus`, `compare` returning `concurrent` exactly when neither clock dominates, and element-wise-max `merge` — so Lotti *can* detect concurrency, which a scalar HLC cannot. The right reading of "vector clock + LWW" is not LWW-everywhere: the vector clock classifies each pair, causal order is respected by merge/replay, and the `updatedAt` LWW tiebreak is consulted only on the `concurrent` branch. Two conditions must hold for a fully deterministic projection: (1) extend the partial order — the causal `messagePrev` DAG, with the vector clock *classifying* concurrency rather than ordering — to a single deterministic *total* order with a replica-independent tiebreak (a stable `hostId+id` key on otherwise-concurrent events); and (2) make the LWW comparator a genuine total order with a deterministic secondary tiebreak — without breaking `updatedAt` ties by `hostId`, two devices can diverge on equal timestamps. The anchor's two open problems map directly: compaction should be an append-only, content-addressed checkpoint event naming the exact prefix it summarizes (so replay-from-summary reproduces the same projected state, provenance, and verbatim uncovered tail as replay-from-genesis — *not* byte-identical summary text, which two independent LLM summarizations cannot guarantee; see §6); and side-effecting actions (notifications, paid inference, external calendar writes, and especially the PLANNER committing a schedule) must serialize behind a lease keyed to the side-effect boundary (§8) + fencing token rather than merge via LWW, while pure log appends converge for free.
