...
- **VOI proxies (A/E/F).** What concrete proxies stand in for "expected value of information" and "cost of interruption"? JITAI receptivity normally leans on phone/wearable sensors; Lotti has no passive-sensing pipeline, so substrate-derived proxies (day-plan utility delta, deadline slack, energy-band fit, recent dismissal rate) are an unvalidated empirical bet.
