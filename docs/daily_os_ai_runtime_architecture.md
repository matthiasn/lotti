...
- **Cost — heal the head lazily.** Unbounded heads make context and the prefix grow, so a **join-by-continuation** node (linking to all current heads) caps the fork — emitted **lazily** (only when ≥2 heads survive past one wake cycle). Its **id is content-addressed** (`id = hash(frontierDigest)` over the parent-head set), so two devices emitting the join concurrently write the *same* log entry and set-union merges them into one node — concurrent joins can't themselves fork (no join storm). For prefix stability, not correctness.
...
- **Side effects** carry an idempotency key `agentId + behaviorKind + frontierDigest + toolName`; the projection dedups duplicates (ChangeSet dedup, ADR 0009). Irreversible external effects (a sent email) can't be undone, so those stay behind the lease + gate, kept minimal.
