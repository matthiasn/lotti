...
- **Cost — heal the head lazily.** Unbounded heads make context and the prefix grow, so a **join-by-continuation** node (linking to all current heads) caps the fork — emitted **lazily** (only when ≥2 heads survive past one wake cycle), by the lowest-`hostId` head author, idempotent by `frontierDigest` so concurrent joins dedup. For prefix stability, not correctness.
