...
3. **Treat the summary as a derived projection, not a destructive overwrite — and don't expect a digest to make LLM outputs converge.** Two independent summarizations will *not* be byte-identical (a digest names an output; it can't make two equal). Summaries are **candidate checkpoints over causal frontiers** (§8): a summary covers a *frontier* (an antichain). Frontiers join cleanly (a semilattice), but the **join of two candidates may have no materialized text**, so the **active checkpoint is the maximal complete materialized checkpoint** — among checkpoints ancestral to every head, the one covering the most history; context is that summary + the **uncovered tail of original (non-summary) events** (non-active summaries excluded, so no derived text or double-counting). Same-frontier candidates tie-break by `frontierDigest`; **incomparable maxima** fall back to their **meet (common base)** + full verbatim uncovered region, which a lazy **merge-summary** then collapses into one dominating checkpoint (so a merge-summary always eventually becomes active). Never pick one candidate's text as the checkpoint when candidates are incomparable — that drops the other branch's history. `frontierDigest` (a hash of the antichain's canonical id-set) keys dedup and the verification/replay hash over a **canonical form** (sorted keys, RFC 3339 UTC timestamps, normalized numbers, UTF-8 canonical JSON/JCS, versioned tag `sha256-v1`). The lease (§8) only avoids *usually* summarizing twice; it is not required for convergence.
...
- **Summaries** are candidate checkpoints over causal **frontiers** (antichains); the active checkpoint is the latest **materialized** checkpoint ancestral to all heads, with the divergent tail read verbatim until a lazy merge-summary collapses it — so two hosts summarizing the same trunk converge without needing byte-identical text (§6).
