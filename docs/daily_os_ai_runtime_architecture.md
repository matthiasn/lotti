...
- **Cost — heal the head lazily.** Unbounded heads make context and the prefix grow, so a **join-by-continuation** node (linking to all current heads) caps the fork — emitted **lazily** (only when ≥2 heads survive past one wake cycle). Its **id is the `frontierDigest`** of the parent-head set (already the content hash of that antichain's id-set — no extra hashing), so two devices emitting the join concurrently write the *same* log entry and set-union merges them into one node — concurrent joins can't themselves fork (no join storm). For that to hold, the join's **payload is deterministic** (sorted parent-head ids + a fixed kind, no clock/host in the content) and the projection reconciles per-device envelope metadata (`hostId`, VC, `createdAt`) canonically, so the shared id can't diverge under LWW. For prefix stability, not correctness.
