...
```dart
/// Links a template to an agent instance it is assigned to.
const factory AgentLink.templateAssignment({
  required String id,
  required String fromId, // template ID
  required String toId,   // agent instance ID
  required DateTime createdAt,
  required DateTime updatedAt,
  required VectorClock? vectorClock,
  DateTime? deletedAt,
...
4. **Version History section** (expandable):
