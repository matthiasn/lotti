...
@override
List<EntityMetadataRow> queryAllEntityMetadata() {
  // ObjectBox property queries avoid deserializing the embedding vector.
  // We query two indexed string columns — fast and memory-efficient.
  final query = _box.query().build();
  try {
    final entityIds = query
        .property(EmbeddingChunkEntity_.entityId)
        .find();
    final taskIds = query
        .property(EmbeddingChunkEntity_.taskId)
        .find();
    return [
...
**Why property queries?** ObjectBox property queries (`query.property(field).find()`)
return only the requested column values without deserializing the full entity. This
avoids loading the `Float32List embedding` field (4KB per row), reducing startup memory
from ~120MB to ~2MB for a 30k-row corpus (just two short strings per row).
...
above). This uses ObjectBox property queries to read only `entityId` and `taskId` columns
without deserializing the 1024-dim embedding vectors, keeping startup fast and memory-efficient.
...
Future<void> _cleanupInterruptedMoves(
  ...
  for (final entry in conflicts.entries) {
    ...
    // Fallback: keep the last-scanned shard (already in _primaryIndex)
    correctShard ??= _primaryIndex[entityId];
...
3. If best result distance >= T: **drop the entire shard** (no results from this shard)
...
| `lib/features/ai/database/real_objectbox_ops.dart` | Implement `findEntitiesByEntityId()` via `query.find()`; implement `queryAllEntityMetadata()` via property queries on entityId + taskId columns |
...
| Startup index loads full vectors into memory | `queryAllEntityMetadata()` uses ObjectBox property queries — reads only entityId + taskId columns (~2MB vs ~120MB for 30k rows) |
