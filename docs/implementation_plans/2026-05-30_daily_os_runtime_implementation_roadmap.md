...
- Status: Plan with Foundation A implemented · Date: 2026-05-30
...
> **Status note:** the ADR baseline and Foundation A are merged. Foundation B
> leader leasing/fencing remains planned and is intentionally not part of the
> lazy-join work.
...
- **Resolved Foundation A:** deterministic projection kernel; `messagePrev` DAG
  append path and head maintenance; wake-start state reconciliation; per-host
...
**PR 1 — Projection kernel (pure, property-tested) — implemented**
...
- **Defers:** none for Foundation A; authoring-host projection remains deferred
...
**PR 2 — Deterministic tiebreak hardening (existing sync) — implemented** — detailed plan: [`2026-05-30_deterministic_tiebreak_plan.md`](./2026-05-30_deterministic_tiebreak_plan.md).
...
**PR 2b — Convergent agent-state counters (per-host G-counters) — implemented** — detailed plan: [`2026-05-31_convergent_counters_plan.md`](./2026-05-31_convergent_counters_plan.md).
...
**PR 3 — `messagePrev` wiring + shadow projection — implemented** — detailed plan: [`2026-05-30_messageprev_shadow_projection_plan.md`](./2026-05-30_messageprev_shadow_projection_plan.md).
...
**PR 4 — State-as-projection migration (Move 1) — implemented** — detailed plan: [`2026-05-30_state_as_projection_plan.md`](./2026-05-30_state_as_projection_plan.md).
...
**PR 5 — Content-addressed input capture + compaction — implemented**
...
- **Touches:** input capture/frontier projection, compaction behavior, summary
  digest schema, wake context assembly (stable prefix = soul→tools→summary→recent
  tail; `currentLocalTime` last).
- **Defers:** merge-summary tuning for deeply diverged branches.
...
- **Goal:** lazy, capped, content-addressed join node (`id = hash("join-v1" + sorted parent-head ids)`); deterministic structural payload, with vector-clock/timestamp envelope canonicalization explicitly deferred until `hostIdOf` is populated.
- **Depends on:** PR 1, PR 3 (DAG). ✅ both merged.
- **Touches:** `projection/join_plan.dart` (pure decision + id), `AgentSyncService.appendJoin` (multi-parent append), `ForkHealer` + a `WakeOrchestrator.onWakeStart` pre-wake hook (one seam, all agent kinds), runtime config flag `enable_fork_healing`.
- **Done when:** ✅ two devices concurrently emitting a join over the same heads write one merged node (sim test in `fork_healer_test.dart`); context/prefix stay bounded under repeated forks (sim); partial-sync join nodes do not trigger second-order joins. **Scope note:** the gate is eager-at-wake-start + complete-view (no cross-wake marker, no `AgentStateEntity` field — see plan); sync-path **vector-clock/timestamp envelope canonicalization is deferred** (inert while `hostIdOf` is unpopulated and joins are immutable — see plan's open decision).
