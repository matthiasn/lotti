...
| Nastavení AI – profily | `Inference Profiles`, `Edit Profile`, `Profile Name`, `Description`, `Save`, `Thinking`, `Image Recognition`, `Transcription`, `Image Generation`, `Desktop Only` | Na několika obrazovkách profilů inference zůstávají nadpisy, pole a názvy pozic anglicky. Je potřeba rozlišit modelová ID od běžných ovládacích prvků a přeložit druhé z nich. |
