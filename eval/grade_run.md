...
- `eval/runs/<runId>/*.trace.json` — one `EvalTrace` per `(scenario, profile)`,
  written by the Level 2 runner.
...
1. List `eval/runs/<runId>/*.trace.json`. If `<runId>` is omitted, use the
   most recently modified subdirectory of `eval/runs/`.
...
3. For **each** trace file:
   ...
   2. Read the rubric matching `agentKind`
      (`taskAgent` → `rubric_task_agent.md`, `planningAgent` →
      `rubric_planning_agent.md`).
   3. Evaluate the trace strictly against the system prompt + rubric. Cite
      specifics (tool names, block times, token counts) in `rationale`.
   4. Write the verdict to a sibling file with the same stem and the
...
4. After all traces are graded, run the reporter to print the summary:
   ...
   ```
   fvm flutter test test/eval/scenarios/report_test.dart --dart-define=EVAL_RUN=<runId>
   ```

   (or summarise the verdicts inline if the reporter entrypoint is not present
   yet — Phase 2).
...
- Keep verdicts reproducible: the verdict file plus the trace file are the audit
  record for that `(scenario, profile)` at that `runId`.
