...
readonly TEMP_MANIFEST="flatpak/com.matthiasn.lotti.generated.yml"
readonly TEMP_METAINFO="flatpak/com.matthiasn.lotti.generated.metainfo.xml"
...
if ! sed -e "s|{{LOTTI_REPO_URL}}|${LOTTI_REPO_URL}|g" \
        ...
        flatpak/com.matthiasn.lotti.yml > "${TEMP_MANIFEST}"; then
...
if ! sed -e "s|{{LOTTI_VERSION}}|${LOTTI_VERSION}|g" \
        ...
        flatpak/com.matthiasn.lotti.metainfo.xml > "${TEMP_METAINFO}"; then
...
echo "  flatpak remote-add --user --if-not-exists lotti-repo repo --no-gpg-verify && flatpak install --user -y lotti-repo com.matthiasn.lotti"
echo ""
echo "To create a bundle:"
echo "  flatpak build-bundle repo lotti.flatpak com.matthiasn.lotti"
echo ""
