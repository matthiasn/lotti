...
def test_cmake_patches_ignore_existing_custom_patch(make_document):
    ...
    assert lotti["sources"] == [{"type": "patch", "path": "patches/custom-sqlite.patch"}]
...
def test_cargokit_patches_preserve_existing_entries(make_document):
    ...
    lotti["sources"] = [{"type": "patch", "path": "custom.patch", "dest": existing_dest}]
    ...
    assert any(dest for dest in dests if dest != existing_dest and dest.endswith("/cargokit"))
...
def test_cargokit_patches_handle_non_list_sources(make_document):
    ...
    assert any("cargokit" in src.get("dest", "") for src in lotti["sources"] if isinstance(src, dict))
