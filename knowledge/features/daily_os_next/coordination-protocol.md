...
2. `flushWakeIntents` waits until that intent is on disk in the settings
   ...
   scan cannot mistake an in-memory intent for a durable one. If the wake
   finishes meanwhile, its completion receipt prevents another fire until the
   record can be consumed (see [wake intents](../agents/wake-orchestration.md)).
