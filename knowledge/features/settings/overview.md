...
type: Feature Module
title: Settings
description: One declarative tree rendered two ways, the ownership boundary against feature-owned pages, and the shared list/detail kit every definition editor reuses.
resource: ../../../lib/features/settings
tags: [settings, navigation, tree, forms]
status: stable
generated: { by: claude-code/opus-5, at: 2026-07-26T02:15:00Z }
...
# One tree, two presentations

Settings no longer hand-maintains its own menu. The landing list and the section
hubs render from a **single declarative tree** — `buildSettingsTree(...)` in the
[`settings_v2`](../settings_v2/) feature — which the desktop tree-nav already
consumed.

**Mobile and desktop read the same structure**, so the two surfaces can never
disagree about which settings exist, how they are grouped, or which flags gate
them.
