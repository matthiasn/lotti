...
second dispatch changes nothing: a created entity's id is derived from the
item, and a field is set only while it holds the value the proposal was made
against. When both devices create the entity before either has received the
