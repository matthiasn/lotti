...
- image analysis creates an `AiResponseEntry` linked from the `JournalImage` — one per run, so an image accumulates multiple analyses (e.g. a brief summary and a full OCR extraction, distinguished by model). These are what the nested-AI-responses UI lists under the image — each rendered by `AiResponseSummary` (`ui/ai_response_summary.dart`) as a quiet, non-elevated AI surface (subtle `aiCard` wash + hairline, deliberately lighter than the accent-blended agent report cards); in the nested tree the cards are collapsible, so long analyses (full OCR) start as a faded preview with a localized Show more/Show less toggle while short summaries stay fully visible. `fetchAiResponsesForImages` (`util/image_ai_responses.dart`) resolves them in bulk for task context: `AiInputRepository.generate` nests them per image log entry as `aiResponses: [{model, generatedAt, text}]`, and the agent capture path (`renderTaskSources`) emits one `image_analysis` log source per analysis. After a task-linked analysis is stored, `runImageAnalysis` marks every parent task of the image dirty (all tasks linking to it, plus the resolved `linkedTaskId`; non-task parents are skipped) with the standard child-changed notification pairs (`taskId` + `PROPAGATED::taskId`) — the analysis write itself only notifies the image, never the tasks — so each parent task's agent picks it up on its normal throttled subscription wake. (When the attribution service is not registered — older tests/partial composition roots — the legacy compatibility path appends the text to the `JournalImage` entry instead.)
