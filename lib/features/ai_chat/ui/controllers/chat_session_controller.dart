...
class ChatSessionController extends Notifier<ChatSessionUiModel> {
  ...
  Future<bool> sendMessage(String content) async {
    ...
    // Persist the user's words before starting fallible network inference.
    // Voice capture may be consumed only after this local acceptance boundary.
    final accepted = await _saveCurrentSession(
      logSubDomain: 'sendMessage.accept',
    );
    if (!accepted) {
      if (ref.mounted) {
        state = state.copyWith(
  ...
  Future<bool> _saveCurrentSession({
    String logSubDomain = '_saveCurrentSession',
  }) async {
    if (!ref.mounted) return false;
    try {
      final chatRepository = ref.read(chatRepositoryProvider);
