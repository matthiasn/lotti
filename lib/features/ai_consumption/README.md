...
- **Shows what AI actually costs.** Per task, per goal agent, per area and per
  ...
  CO₂e.
- **Keeps the cost in sight.** The same leaf-and-amount indicator appears
  wherever the subject does — a task row, a task's details — rather than only
  on a dedicated analysis page.
