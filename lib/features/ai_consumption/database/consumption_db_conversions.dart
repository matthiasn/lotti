...
class ConsumptionDbConversions {
  ...
  static ConsumptionEventsCompanion toCompanion(AiConsumptionEvent event) {
    return ConsumptionEventsCompanion(
      ...
      sequenceIndex: Value(event.sequenceIndex),
      interactionKind: Value(event.interactionKind?.name),
      interactionStatus: Value(event.interactionStatus.name),
      completedAt: Value(event.completedAt),
      providerRequestId: Value(event.providerRequestId),
      errorCode: Value(event.errorCode),
      errorSummary: Value(event.errorSummary),
      payloadId: Value(event.payload?.id),
      costId: Value(event.cost?.id),
