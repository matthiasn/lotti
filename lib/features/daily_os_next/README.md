...
> **rolled out**: a session coordinates it, `AppScreen` auto-arms it (sequenced
...
> provider is connected. `dailyOsOnboardingEnabledFlag` now seeds **on** and is
> force-enabled once on pre-rollout installs (see
> [Onboarding › Rollout](../onboarding/README.md#rollout)); it is kept as a
> field kill-switch. Still **deferred**: the completion **celebration** beat and
...
| `dailyOsOnboardingProviderReadyProvider` + `hasResolvableDailyOsPlannerThinkingRoute` | `state/daily_os_planner_readiness.dart` | The readiness seam: resolves the exact thinking route a drafting wake would use (read-only — never creates the planner; before the first capture it mirrors `DayAgentService.getOrCreatePlannerAgent` with a transient config from the seeded template). Its own library because the onboarding rollout backfill reads it too, which would otherwise make the two gates import each other in a cycle. |
