// Deterministic danish copy used by the manual screenshot catalog.
// The catalog translates authored demo data that does not come from the
// application ARB files, keeping every localized screenshot coherent.
// ignore_for_file: avoid_escaping_inner_quotes, prefer_single_quotes

String? manualScreenshotDanishText(String english) => _copy[english];

const _copy = <String, String>{
  "Theming": "Tematisering",
  "Recording Style": "Indspilningsstil",
  "Modern — energy orb": "Moderne — energikugle",
  "Analogue — VU meter": "Analog — VU-måler",
  "Speech": "Tale",
  "Voice": "Stemme",
  "Male 3": "Mand 3",
  "Female": "Kvinde",
  "Female 1": "Kvinde 1",
  "Female 5": "Kvinde 5",
  "Keyboard shortcuts": "Tastaturgenveje",
  "Open command palette": "Åbn kommando-paletten",
  "General": "Generelt",
  "Command palette": "Kommando-palette",
  "Task": "Opgave",
  "more tokens today": "flere tokens i dag",
  "AI summary": "AI-oversigt",
  "Automatic updates": "Automatiske opdateringer",
  "Bundle task changes and update after two minutes.":
      "Saml ændringer i opgaver og opdater efter to minutter.",
  "Read more": "Læs mere",
  "Open agent internals": "Åbn agentens interne detaljer",
  "Show less": "Vis mindre",
  "\"Run zero-gravity sardine feeder test\"": "\"Kør vægtløs sardinfodertest\"",
  "Add: \"Run zero-gravity sardine feeder test\"":
      "Tilføj: \"Kør vægtløs sardinfodertest\"",
  "Estimate: 45m → 1h 15m": "Estimat: 45 m → 1 time 15 min",
  "45m → 1h 15m": "45 m → 1 time 15 min",
  "Automatic updates are off. Wake the agent when you want a fresh report.":
      "Automatiske opdateringer er slået fra. Væk agenten, når du vil have en frisk rapport.",
  "This summary is out of date": "Dette resumé er forældet",
  "The task changed after this summary was generated.":
      "Opgaven ændrede sig efter denne opsummering blev genereret.",
  "Wake agent": "Væk agenten",
  "Filter tasks": "Filtrér opgaver",
  "Filter projects": "Filtrér projekter",
  "Mission Control Mac": "Mission Control Mac",
  "Orbital Habitat Console": "Orbital Habitat-konsol",
  "Admiral Pebble’s Phone": "Admiral Pebbles telefon",
  "Inspect orbital penguin habitat before launch":
      "Inspicer orbital pingvinhabitat før opsendelse",
  "Launch orbital penguin habitat after seal inspection":
      "Opsend orbital pingvinhabitat efter tætningsinspektion",
  "Mission Control cleared 36 penguins. Recheck pressure seal C and hold the sardine cargo pod until the final emperor arrives.":
      "Mission Control godkendte 36 pingviner. Tjek tryktætningen C igen og hold sardin-lastkapslen, indtil den sidste kejser ankommer.",
  "Inspect orbital penguin habitat — before launch":
      "Inspicer orbital pingvinhabitat — før opsendelse",
  "Habitat Console cleared all 37 penguins and pressure seal C. Release the zero-gravity sardine cargo pod at 11:20.":
      "Habitat Console har godkendt alle 37 pingviner og tryktætning C. Frigiv den vægtløse sardin-lastkapsel kl. 11:20.",
  "Europa sardine cargo manifest revised by Admiral Pebble.":
      "Europa sardin-fragtmanifest revideret af admiral Pebble.",
  "Project Waddle node profile": "Project Waddle-nodeprofil",
  "Project Waddle private-mode flag": "Project Waddle private-mode flag",
  "Show private entries": "Vis private poster",
  "Habitat pressure-seal photo": "Habitatets trykforseglingsfoto",
  "Project Details": "Projektdetaljer",
  "Status": "Status",
  "Category": "Kategori",
  "Change status": "Ændre status",
  "Target Date": "Måldato",
  "Project health": "Projektets sundhed",
  "Time Analysis": "Tidsanalyse",
  "TOTAL": "SAMLET",
  "FOCUS": "FOKUS",
  "Running total": "Løbende total",
  "Running total over the range": "Løbende total over perioden",
  "Compare": "Sammenlign",
  "Change": "Forandring",
  "PREVIOUS": "TIDLIGERE",
  "Summary": "Resumé",
  "Photos": "Fotos",
  "Timeline": "Tidslinje",
  "Filter journal": "Filtrér journal",
  "Entry types": "Posttyper",
  "Event": "Begivenhed",
  "Text Entry": "Tekstindtastning",
  "Date & Time": "Dato og Tid",
  "Start time": "Starttid",
  "End time": "Sluttid",
  "Friday, July 17, 2026": "fredag den 17. juli 2026",
  "July 2026": "juli 2026",
  "Images": "Billeder",
  "Code": "Kode",
  "Filter & Sort": "Filter & Sort",
  "Newest first": "Nyeste første",
  "Oldest first": "Ældste første",
  "Show hidden entries": "Vis skjulte indgange",
  "Rate this session": "Bedøm denne session",
  "How productive was this session?": "Hvor produktiv var denne session?",
  "How energized did you feel?": "Hvor energisk følte du dig?",
  "How focused were you?": "Hvor fokuseret var du?",
  "Just right": "Lige tilpas",
  "Save": "Gem",
  "Europa sardine futures summit": "Europa sardin-futures-topmødet",
  "Project Waddle launch gala": "Project Waddle lanceringsgalla",
  "Voice memo: Europa sardines are colder than the diplomatic protocol requires.":
      "Talebesked: Europa-sardiner er koldere, end den diplomatiske protokol kræver.",
  "Project Waddle voice memo. Europa sardine cargo is holding at minus eighteen degrees. All 37 emperor penguins are present. Recalibrate the zero-gravity fish feeder before the launch review, and ask Sir Flaps-a-Lot to stop saluting the pressure gauge.":
      "Projekt Waddle stemmememo. Europa sardin-lasten holder sig ved minus atten grader. Alle 37 kejserpingviner er til stede. Kalibrer nul-tyngdekrafts fiskefoderautomaten før opsendelsesgennemgangen, og bed Sir Flaps-a-Lot om at stoppe med at hilse på trykmåleren.",
  "Europa cold-chain check: sardines stable, emperor penguin roll call complete, feeder calibration still pending.":
      "Europa koldkædetjek: sardinerne stabile, kejserpingvin-opråbet fuldført, foder-kalibreringen afventer stadig.",
  "All 37 emperor penguins": "Alle 37 kejserpingviner",
  "Generate…": "Generer...",
  "Skills": "Færdigheder",
  "pressure-gauge anomalies": "trykmåleranomalier",
  "Add": "Tilføj",
  "Checklist": "Tjekliste",
  "Audio Recording": "Lydoptagelse",
  "Timer": "Timer",
  "Select Reference Images": "Vælg referencebilleder",
  "Continue": "Fortsæt",
  "Choose your AI brain": "Vælg din AI-hjerne",
  "More options": "Flere muligheder",
  "Connection verified": "Forbindelse bekræftet",
  "Connection": "Forbindelse",
  "Edit": "Rediger",
  "Provider Type": "Udbydertype",
  "API Key": "API-nøgle",
  "Edit Model": "Rediger model",
  "Capabilities": "Kapaciteter",
  "Choose a model": "Vælg en model",
  "AI Impact": "AI-indflydelse",
  "Cost by category": "Omkostninger efter kategori",
  "Connect": "Forbind",
  "Get started": "Kom i gang",
  "How should recording feel?": "Hvordan skal det føles at optage?",
  "Where should your AI work?": "Hvor skal din AI fungere?",
  "Add your own": "Tilføj din egen",
  "Create your first task": "Lav din første opgave",
  "Record your thought": "Optag dine tanker",
  "Listening… tap when you're done": "Lytter... tryk, når du er færdig",
  "Rather type?": "Vil du hellere skrive?",
  "Your first task is ready": "Din første opgave er klar",
  "You've created your first AI task": "Du har skabt din første AI-opgave",
  "Talk. Lotti turns it into a plan.": "Tal. Lotti gør det til en plan.",
  "Inspect the Europa sardine relay before the emperor penguin roll call":
      "Inspicer Europa sardin-relæet før kejserpingvinens opråb",
  "Inspect the Europa sardine relay": "Inspicer Europa-sardin-relæet",
  "Verify relay pressure": "Bekræft relætryk",
  "Count all emperor penguins": "Tæl alle kejserpingviner",
  "Route the sardine cargo pods": "Led sardin-lastkapslerne",
  "Generating image...": "Genererer billede...",
  "Link existing task...": "Link eksisterende opgave...",
  "Go to Tasks": "Gå til opgaver",
  "rename": "omdøb",
  "Rename focused item": "Omdøb fokuseret element",
  "Animations": "Animationer",
  "Celebration animations": "Fejringsanimationer",
  "Completion haptics": "Haptik ved fuldførelse",
  "Style": "Stil",
  "Sparks": "Gnister",
  "Combine two": "Kombiner to",
  "Try it": "Prøv det",
  "Count emperor penguins": "Tæl kejserpingviner",
  "Route sardine cargo": "Dirigér sardin-last",
  "Brief Mission Control": "Brief Mission Control",
  "Changes save and apply everywhere instantly":
      "Ændringer gemmes og gælder overalt med det samme",
  "Shape": "Form",
  "Advanced Settings": "Avancerede indstillinger",
  "Config Flags": "Konfigurationsflag",
  "Show private entries?": "Vis private poster?",
  "Import Activity Data": "Importer aktivitetsdata",
  "Purge deleted items": "Rens slettede elementer",
  "Definitions": "Definitioner",
  "Categories": "Kategorier",
  "Labels": "Etiketter",
  "Habits": "Vaner",
  "Inspect habitat seals": "Inspicer habitatets tætninger",
  "Check the pressure seals before the colony wakes.":
      "Tjek trykpakningerne, før kolonien vågner.",
  "Log penguin roll call": "Log pingvin-navneopråbet",
  "Confirm all 37 emperor penguins are aboard.":
      "Bekræft at alle 37 kejserpingviner er ombord.",
  "Recalibrate fish feeder": "Rekalibrer fiskefoderautomaten",
  "Tune the zero-gravity feeder after the midday delivery.":
      "Afstem den vægtløse foderautomat efter middagsleveringen.",
  "Review sardine inventory": "Gennemgå sardinlager",
  "Reconcile consumed crates with the orbital manifest.":
      "Afstem forbrugte kasser med det orbitale manifest.",
  "Success": "Succes",
  "Skip": "Spring over",
  "Missed": "Missede",
  "Dashboards": "Dashboards",
  "Measurables": "Målbare ting",
  "Mission Control Router": "Mission Control Router",
  "Cloud routing for launch planning and high-stakes reasoning.":
      "Cloud-routing til lanceringsplanlægning og højrisiko-ræsonnement.",
  "Habitat Local Lab": "Habitat Local Lab",
  "Local models for private colony notes and sardine logistics.":
      "Lokale modeller til private koloninoter og sardinlogistik.",
  "Orbital Vision": "Orbital Vision",
  "Multimodal inspection for habitat imagery and cover art.":
      "Multimodal inspektion for habitatbilleder og coverkunst.",
  "Penguin Audio Bay": "Penguin Audio Bay",
  "Local transcription for mission briefings.":
      "Lokal transskription til missionsbriefinger.",
  "Waddle Command 70B": "Waddle Kommando 70B",
  "Fast tool-calling model for routine Project Waddle operations.":
      "Hurtig værktøjskaldsmodel til rutinemæssige Project Waddle-operationer.",
  "Emperor Reasoning XL": "Kejserens Ræsonnement XL",
  "Deliberate model for launch reviews and unusually formal penguins.":
      "Betænksom model til lanceringsgennemgange og usædvanligt formelle pingviner.",
  "Sardine Logistics 14B": "Sardine Logistics 14B",
  "Local planning model for cargo manifests and feeder calibration.":
      "Lokal planlægningsmodel for fragtmanifest og kalibrering af feeder.",
  "Habitat Vision Pro": "Habitat Vision Pro",
  "Checks pressure gauges, ice seals, and suspicious fish-shaped alerts.":
      "Tjekker trykmålere, ispakninger og mistænkelige fiskeformede alarmer.",
  "Voxtral Penguin Briefings": "Voxtral Penguin-briefinger",
  "Transcribes habitat voice memos with Project Waddle vocabulary.":
      "Transskriberer habitat-talememoer med Project Waddle-ordforråd.",
  "Project Waddle Cover Artist": "Project Waddle Coverkunstner",
  "Creates centered 16:9 mission art that survives square thumbnail crops.":
      "Skaber centreret 16:9 missionskunst, der overlever firkantede miniature-crops.",
  "Project Waddle Command": "Projekt Waddle-kommando",
  "Launch-critical planning, habitat vision, briefings, and cover art.":
      "Affyringskritisk planlægning, habitatvision, briefinger og coverkunst.",
  "Habitat Local-First": "Habitat lokal-først",
  "Keeps private colony notes and routine sardine logistics local.":
      "Holder private koloninoter og rutinemæssig sardinlogistik lokalt.",
  "Fish Diplomacy": "Fiskediplomati",
  "Extra deliberation for Europa sardine markets and passenger law.":
      "Ekstra overvejelse for Europa sardinmarkeder og passagerlovgivning.",
  "Transcribe habitat briefing": "Transskribér habitatbriefing",
  "Turn a Project Waddle voice memo into punctuated mission notes.":
      "Lav en Project Waddle-stemmenote om til missionsnoter med tegnsætning.",
  "Transcribe the mission briefing accurately.":
      "Transskribér missionsbriefingen nøjagtigt.",
  "Preserve Project Waddle names and terminology.":
      "Bevar navne og terminologi for Projekt Waddle.",
  "Inspect habitat photo": "Inspicer habitatfoto",
  "Find pressure-gauge anomalies and task-relevant seal damage.":
      "Find trykmåler-anomalier og opgaverelevante forseglingsskader.",
  "Inspect the habitat image for operational risks.":
      "Inspicer habitatbilledet for operationelle risici.",
  "Report only visible and actionable findings.":
      "Rapportér kun synlige og handlingsorienterede resultater.",
  "Generate Project Waddle cover art": "Generer Project Waddle-coverkunst",
  "Create centered 16:9 art for the task and its square thumbnail.":
      "Lav centreret 16:9-grafik til opgaven og dens firkantede miniaturebillede.",
  "Create memorable mission cover art.": "Skab mindeværdigt missionscover.",
  "Keep the penguin subject inside the square-safe area.":
      "Hold pingvin-motivet inden for det firkantede sikre område.",
  "Draft launch-review prompt": "Udkast til prompt til lanceringsgennemgang",
  "Prepare a complete AI prompt for the next Mission Control review.":
      "Forbered en komplet AI-prompt til næste Mission Control-gennemgang.",
  "Write a precise operational prompt.": "Skriv en præcis operationel prompt.",
  "Include the task context and outstanding risks.":
      "Inkluder opgavekonteksten og udestående risici.",
  "Penguin Operations": "Penguin-operationer",
  "Penguin Ops": "Penguin Ops",
  "Mission Control": "Mission Control",
  "Human Maintenance": "Menneskelig vedligeholdelse",
  "Tasks": "Opgaver",
  "Logbook": "Logbog",
  "Settings": "Indstillinger",
  "Manual": "Manual",
  "Opens in your browser": "Åbner i din browser",
  "Calendar": "Kalender",
  "Habitat critical": "Habitatkritisk",
  "37 tasks": "37 opgaver",
  "14 tasks": "14 opgaver",
  "Edit category": "Rediger kategori",
  "Project Waddle; Sir Flaps-a-Lot; sardine; Europa":
      "Projekt Waddle; Sir Flaps-a-Lot; sardin; Europa",
  "Launch-critical work for the first interplanetary penguin habitat.":
      "Opsendelseskritisk arbejde for det første interplanetariske pingvinhabitat.",
  "Must be resolved before emperor penguins enter the orbital habitat.":
      "Skal løses, før kejserpingviner går ind i det orbitale habitat.",
  "Awaiting Mission Control": "Afventer Mission Control",
  "Blocked until the lunar shift sends clearance.":
      "Blokeret indtil måneskiftet sender tilladelse.",
  "Sardine market sensitive": "Sardin-markedsfølsom",
  "Private negotiations with the Europa fish exchange.":
      "Private forhandlinger med Europa fiskebørsen.",
  "Checklist correction examples": "Eksempler på tjeklistekorrektion",
  "Speech recognition": "Talegenkendelse",
  "Choose an inference profile": "Vælg en inferensprofil",
  "Create category": "Opret kategori",
  "No categories yet": "Ingen kategorier endnu",
  "Edit label": "Rediger etiket",
  "Project Waddle — launch": "Projekt Waddle — lancering",
  "Emperor penguin roll call": "Kejserpingvinens navneopråb",
  "Account for all 37 expedition penguins before launch.":
      "Gør rede for alle 37 ekspeditionspingviner før lancering.",
  "Walk the habitat seals": "Gennemgå habitatets tætninger",
  "Inspect every pressure seal after the artificial sunrise.":
      "Inspicer hver trykpakning efter den kunstige solopgang.",
  "Review sardine forecast": "Gennemgå sardinprognosen",
  "Paused while the Europa exchange recalibrates its fish index.":
      "På pause, mens Europa-børsen rekalibrerer sit fiskeindeks.",
  "Edit habit": "Rediger vane",
  "Schedule": "Tidsplan",
  "Start date": "Startdato",
  "Show from": "Vis fra",
  "Show alert at": "Vis alarm på",
  "Habitat pressure": "Habitattryk",
  "Average pressure across the orbital habitat.":
      "Gennemsnitligt tryk over det orbitale habitat.",
  "Sardines consumed": "Spiste sardiner",
  "Daily fish consumption across the orbital habitat.":
      "Dagligt fiskeforbrug i hele det orbitale habitat.",
  "sardines": "sardiner",
  "sardine": "sardin",
  "project waddle habitat": "projekt waddle habitat",
  "Project Waddle habitat": "Project Waddle-habitat",
  "Dinner shift after the Europa cargo docked":
      "Middagsvagt efter Europa-lasten lagde til kaj",
  "Observed at": "Observeret ved",
  "Penguins accounted for": "Pingviner gjort rede for",
  "Edit measurable": "Rediger målbar",
  "Daily average": "Dagligt gennemsnit",
  "Colony operations": "Kolonioperationer",
  "Habitat pressure, sardine demand, and crew headcount.":
      "Habitattryk, efterspørgsel efter sardiner og antal besætningsmedlemmer.",
  "Mission readiness": "Missionsberedskab",
  "Private launch review for Project Waddle.":
      "Privat opsendelsesgennemgang for Projekt Waddle.",
  "Habitat pressure — Daily average": "Habitattryk — Dagligt gennemsnit",
  "Sardines consumed — Daily sum": "Sardiner indtaget — Daglig sum",
  "Penguins accounted for — Daily sum": "Pingviner gjort rede for — Daglig sum",
  "Add charts by type": "Tilføj diagrammer efter type",
  "Measurements": "Målinger",
  "Health": "Helbred",
  "The habitat passed inspection, all 37 emperor penguins saluted in roughly the same direction, and the sardine cargo arrived cold.":
      "Habitatet bestod inspektionen, alle 37 kejserpingviner hilste i omtrent samme retning, og sardinlasten ankom kold.",
  "Price discovery, cold-chain diplomacy, and formal fish hats.":
      "Prisopdagelse, koldkædediplomati og formelle fiskehatte.",
  "Emperor penguin roll-call gala": "Kejserpingvin-navneopråbsgalla",
  "Every penguin was present; two attended twice.":
      "Hver pingvin var til stede; to deltog to gange.",
  "Europa cold-chain grand opening": "Europa kølekædens store åbning",
  "The ribbon froze before anyone could cut it.":
      "Båndet frøs, før nogen kunne klippe det.",
  "Orbital ice-garden opening": "Åbning af orbital ishave",
  "One quiet lap, zero status meetings.": "En stille runde, nul statusmøder.",
  "Mission Control declares the ice-pad trajectory officially waddly.":
      "Mission Control erklærer officielt ispaddens bane vraltende.",
  "Pressure seals green. Tiny oxygen packs counted twice.":
      "Trykpakninger er grønne. Små iltpakker talt to gange.",
  "The orbital habitat, five minutes before the ceremonial sardines.":
      "Det orbitale habitat, fem minutter før de ceremonielle sardiner.",
  "Europa cargo pods arrive at a crisp and diplomatic temperature.":
      "Europa-lastkapsler ankommer ved en frisk og diplomatisk temperatur.",
  "Count every expedition penguin, check the tiny oxygen packs, and record any suspiciously formal salutes.":
      "Tæl hver ekspeditionspingvin, tjek de små iltpakker, og noter eventuelle mistænkeligt formelle hilsener.",
  "Inspect orbital penguin habitat": "Undersøg pingvinhabitatet i kredsløb",
  "Inspect pressure seals, confirm all 37 emperor penguins are present, and route the sardine cargo pods before the live Project Waddle demonstration.":
      "Inspicer trykpakningerne, bekræft at alle 37 kejserpingviner er til stede, og dirigér sardin-lastkapslerne før den live Project Waddle-demonstration.",
  "Project Waddle launch review": "Project Waddle lanceringsgennemgang",
  "Review the ice-pad trajectory, confirm the snack manifest, and make sure Mission Control has removed the fish-shaped cursor from the launch display.":
      "Gennemgå ispaddens bane, bekræft snackmanifestet, og sørg for, at Mission Control har fjernet den fiskeformede markør fra opsendelsesdisplayet.",
  "Lunch (coffee is not a vegetable)": "Frokost (kaffe er ikke en grøntsag)",
  "Eat something recognizable as food before the robot nutritionist files another orbital wellness incident.":
      "Spis noget, der kan genkendes som mad, før robot-ernæringseksperten indgiver endnu en orbital wellness-hændelse.",
  "Negotiate sardine futures": "Forhandle sardinfutures",
  "Lock the colony's Q3 sardine price before the Europa exchange discovers why the emergency fish ceiling is shaped like a penguin.":
      "Lås koloniens Q3-sardinpris, før Europa-børsen opdager, hvorfor nødfiskeloftet er formet som en pingvin.",
  "Recalibrate the zero-gravity fish feeder":
      "Genkalibrer den vægtløse fiskefoderautomat",
  "Run the low-orbit sardine test and stop the feeder from launching lunch toward Mission Control.":
      "Kør lavbane-sardintesten og stop feederen fra at sende frokost mod Mission Control.",
  "Confirm the interplanetary sardine cargo pods":
      "Bekræft de interplanetariske sardin-lastkapsler",
  "Reconcile the cold-chain manifest with the colony dashboard before the next supply shuttle leaves Europa.":
      "Forlig kølekædemanifestet med koloniens dashboard, inden næste forsyningsshuttle forlader Europa.",
  "Ask Legal whether a penguin is a passenger":
      "Spørg Legal, om en pingvin er passager",
  "Resolve whether Sir Flaps-a-Lot needs a boarding pass or a cargo declaration before launch.":
      "Afgør om Sir Flaps-a-Lot skal have boardingkort eller lastdeklaration før opsendelse.",
  "Walk without a headset": "Gå uden headset",
  "Take one quiet lap around the orbital ice garden without turning it into a briefing, podcast, or emergency call.":
      "Tag en stille runde rundt i den orbitale ishave uden at gøre det til en briefing, podcast eller nødopkald.",
  "Tomorrow starts with the orbital penguin habitat inspection":
      "I morgen starter med inspektionen af orbitalpingvinens habitat",
  "Tomorrow I need to inspect the orbital penguin habitat before Mission Control wakes up, run the emperor penguin roll call, and negotiate the sardine futures contract with Reykjavik. At eleven we have the Project Waddle launch review. Please protect lunch because apparently coffee is not a vegetable. In the afternoon I need ninety minutes for the zero-gravity fish feeder, a legal review called Is a penguin a passenger, and the board briefing. Add a buffer before the live habitat demo, then leave thirty minutes for a walk and a debrief with Sir Flaps-a-Lot. Nothing important should begin after five.":
      "I morgen skal jeg inspicere det orbitale pingvinhabitat, før Mission Control vågner, køre kejserpingvin-navneopråbet og forhandle sardinfutures-kontrakten med Reykjavik. Klokken elleve har vi Project Waddle-lanceringsgennemgang. Beskyt venligst frokosten, for åbenbart er kaffe ikke en grøntsag. Om eftermiddagen skal jeg bruge halvfems minutter til nul-tyngdekrafts fiskefoderautomaten, en juridisk gennemgang kaldet Er en pingvin en passager, og bestyrelsens briefing. Tilføj en buffer før levende habitat-demoen, og lad derefter en halv time være til en gåtur og en debriefing med Sir Flaps-a-Lot. Intet vigtigt bør begynde efter klokken fem.",
  "Orbital habitat inspection": "Inspektion af kredsløbshabitat",
  "Zero-gravity fish feeder": "Vægtløs fiskefoderautomat",
  "Best done before Mission Control and the penguins get chatty":
      "Det er bedst at gøre det, før Mission Control og pingvinerne bliver snakkesalige",
  "Buffer": "Buffer",
  "The fish are least suspicious immediately after lunch":
      "Fiskene er mindst mistænkelige lige efter frokost",
  "Legal: Is a penguin a passenger?": "Juridisk: Er en pingvin en passager?",
  "HIGH ENERGY": "HØJ ENERGI",
  "POST-LUNCH DIP": "DIP EFTER FROKOST",
  "SECOND WIND": "ANDEN VIND",
  "Habitat seals green; tiny helmets still pending":
      "Habitattætninger er grønne; små hjelme venter stadig",
  "Locked Q3 sardines below the emergency fish ceiling":
      "Låste Q3-sardiner under nødfiskeloftet",
  "Prototype ready for the live habitat demo":
      "Prototype klar til live habitat-demonstrationen",
  "Retrieve penguin from ventilation duct":
      "Hent pingvinen fra ventilationskanalen",
  "Lunch, technically": "Frokost, teknisk set",
  "Habitat work lands before Mission Control wakes":
      "Habitatarbejdet lander, før Mission Control vågner",
  "Habitat inspections started before the first call run 30% longer before a penguin or executive finds the red button.":
      "Habitatinspektioner startede før det første kald varede 30 % længere, før en pingvin eller leder fandt den røde knap.",
  "Walks get skipped after penguin escapes":
      "Gåture bliver sprunget over, efter pingvinen er undsluppet",
  "When a penguin enters the ventilation system, the planned walk is usually dropped — protect it explicitly.":
      "Når en pingvin træder ind i ventilationssystemet, bliver den planlagte gåtur som regel droppet — beskyt den eksplicit.",
  "No briefings before 10:00": "Ingen briefinger før kl. 10:00",
  "Keep mornings briefing-free until 10:00 — stated preference.":
      "Hold morgenerne uden briefing indtil kl. 10:00 — angivet præference.",
  "Move sardine negotiations 30 minutes later":
      "Flyt sardinforhandlinger 30 minutter senere",
  "Protect the launch review and give Mission Control a buffer.":
      "Beskyt opsendelsesgennemgangen og giv Mission Control en buffer.",
  "Start the fish-feeder demo after the new buffer":
      "Start fiskefoder-demoen efter den nye buffer",
  "Keep the zero-gravity demo clear of the sardine negotiation.":
      "Hold nul-tyngdekraft-demoen fri for sardinforhandlingerne.",
  "Seals green; all 37 emperor penguins accounted for.":
      "Tætninger grønne; alle 37 kejserpingviner blev gjort rede for.",
  "Mission Control approved the revised habitat checklist.":
      "Mission Control godkendte den reviderede habitat-tjekliste.",
  "Q3 supply secured below the emergency fish ceiling.":
      "Q3-forsyning sikret under nød-fiskeloftet.",
  "Prototype calibrated; live habitat demo still pending.":
      "Prototype kalibreret; live habitat-demoen afventer stadig.",
  "→ tomorrow morning": "→ i morgen tidlig",
  "Mission Control review ran long.": "Mission Control-gennemgangen trak ud.",
  "→ tomorrow afternoon": "→ i morgen eftermiddag",
  "Start with the live fish-feeder demo at 09:00, then resolve the penguin passenger question before the launch window opens.":
      "Start med den live fiskefoder-demonstration kl. 09:00, og løs derefter spørgsmålet om pingvinpassagerer, inden opsendelsesvinduet åbner.",
  "Protect the launch review, move sardine negotiations later, and keep the fish-feeder demo clear.":
      "Beskyt opsendelsesgennemgangen, flyt sardinforhandlingerne senere, og hold fiskefoder-demonstrationen fri.",
  "Send Project Waddle briefing": "Send Project Waddle-briefing",
  "the Project Waddle launch review": "Project Waddle lanceringsgennemgang",
  "Project Waddle launch briefing": "Project Waddle lanceringsbriefing",
  "In progress · 2 sessions": "I gang · 2 sessioner",
  "before Mission Control wakes up": "før Mission Control vågner",
  "run the emperor penguin roll call": "kør kejserpingvin-navneopråbet",
  "Recurring · weekdays": "Tilbagevendende · Hverdage",
  "Mark complete after all 37 answer":
      "Markér som færdig, når alle 37 har svaret",
  "Replace the diplomatic fish bucket": "Udskift den diplomatiske fiskespand",
  "Started Friday, dignity not included":
      "Startede fredag, værdighed ikke inkluderet",
  "Renew orbital wildlife permit": "Forny tilladelsen til det orbitale dyreliv",
  "The penguins are currently technically cargo":
      "Pingvinerne er teknisk set last",
  "Practice calm during surprise flapping": "Øv ro under overraskelsesflag",
  "Last skipped Thursday": "Sidst sprunget torsdag over",
  "Habitat Watcher": "Habitat Watcher",
  "All 37 emperor penguins are accounted for. Habitat pressure held at 101.3 kPa overnight; the remaining launch risk is the zero-gravity sardine feeder calibration.":
      "Alle 37 kejserpingviner er til stede. Habitattrykket holdt sig på 101,3 kPa natten over; den resterende opsendelsesrisiko er kalibreringen af den vægtløse sardinfoderautomat.",
  "Habitat stable; zero-gravity sardine feeder blocks sign-off.":
      "Habitat stabilt; den vægtløse sardinfoderautomat blokerer for godkendelsen.",
  "## Latest assessment\n\n- Pressure seals A–F stayed stable across the night shift.\n- 840 sardines are loaded; feeder calibration still blocks sign-off.\n- Mission Control clearance is due before the 06:30 roll call.\n\n## Recommended next step\n\nRun the feeder test, attach the telemetry image, then request launch approval.\n":
      "## Seneste vurdering\n\n- Trykpakninger A–F forblev stabile gennem natteholdet.\n- 840 sardiner er læsset; feeder-kalibreringen blokerer stadig for godkendelsen.\n- Mission Control-godkendelse skal leveres før opråbet kl. 06:30.\n\n## Anbefalet næste skridt\n\nKør feeder-testen, vedhæft telemetribilledet, og anmod derefter om godkendelse af lanceringen.\n",
  "Latest assessment": "Seneste vurdering",
  "Recommended next step": "Anbefalet næste skridt",
  "Run zero-gravity sardine feeder test": "Kør vægtløs sardin-feedertest",
  "Orbital Habitat Sentinel": "Orbital Habitat Sentinel",
  "Project Waddle Day Planner": "Projekt Waddle Day Planner",
  "Sardine Supply Watch": "Sardinforsyningsvagt",
  "Fish Diplomacy Coach": "Fiskediplomatitræner",
  "Habitat Seal Inspector": "Habitat-tætningsinspektør",
  "Project Waddle Planner": "Projekt Waddle Planlægger",
  "Sardine Cargo Coordinator": "Sardin-lastkoordinator",
  "Project Waddle Operations": "Projekt Waddle-operationer",
  "Cargo Bay": "Lastrum",
  "Interplanetary Relations": "Interplanetariske relationer",
  "Habitat Science": "Habitatvidenskab",
  "Protect Project Waddle by checking pressure seals, habitat telemetry, and every suspiciously cheerful penguin before launch.":
      "Beskyt Project Waddle ved at tjekke trykforseglinger, habitat-telemetri og alle mistænkeligt glade pingviner før opsendelse.",
  "End with a concise go/no-go recommendation and list any seals that need a human inspection.":
      "Afslut med en kortfattet go/no-go anbefaling og oplist alle pakninger, der kræver menneskelig inspektion.",
  "Build a realistic day around energy, fixed launch windows, and the colony rule that lunch still counts even during a sardine emergency.":
      "Byg en realistisk dag op omkring energi, faste opsendelsesvinduer og kolonireglen om, at frokost stadig tæller, selv under en sardin-nødsituation.",
  "Call out over-capacity plans before asking for commitment.":
      "Nævn overkapacitetsplaner, før du beder om forpligtelse.",
  "Track sardine cargo pods, cold-chain handoffs, and zero-gravity feeder stock without sending private manifests to a cloud model.":
      "Følg sardin-lastkapsler, koldkæde-overleveringer og lageret i den vægtløse foderautomat uden at sende private manifester til en skymodel.",
  "Report shortages by habitat, pod, and expected penguin impact.":
      "Rapportér mangel efter habitat, kapsel og forventet pingvinpåvirkning.",
  "Coach agent templates to be clear, skeptical, and gracious during high-stakes fish negotiations.":
      "Coach agent-skabeloner til at være klare, skeptiske og imødekommende under højrisiko-fiskeforhandlinger.",
  "Explain each proposed wording change without flattery.":
      "Forklar hver foreslået formuleringsændring uden smiger.",
  "Speak like a calm flight director who respects both evidence and penguins. Put the operational decision first.":
      "Tal som en rolig flyveleder, der respekterer både beviser og pingviner. Sæt den operationelle beslutning først.",
  "Dry warmth is welcome. Never turn a safety warning into a joke.":
      "Tør varme er velkommen. Gør aldrig en sikkerhedsadvarsel til en joke.",
  "Ask one clarifying question when telemetry is ambiguous, then propose the smallest safe next step.":
      "Stil et opklarende spørgsmål, når telemetrien er tvetydig, og foreslå derefter det mindste sikre næste skridt.",
  "Challenge optimistic launch assumptions and cite the observation that changed your conclusion.":
      "Udfordr optimistiske lanceringsantagelser og citer den observation, der ændrede din konklusion.",
  "Be curious, precise, and delighted by useful anomalies.":
      "Vær nysgerrig, præcis og begejstret for nyttige anomalier.",
  "No alarmism; distinguish measurements from hypotheses.":
      "Ingen alarmisme; skelne målinger fra hypoteser.",
  "Turn vague concerns into an observable test.":
      "Gør vage bekymringer til en observerbar test.",
  "Prefer an awkward fact over an elegant story.":
      "Foretræk en akavet kendsgerning frem for en elegant historie.",
  "Be brisk, practical, and exact about quantities.":
      "Vær hurtig, praktisk og præcis med mængderne.",
  "Never shame a penguin for an empty feeder.":
      "Skam aldrig en pingvin for en tom foderautomat.",
  "Name the blocked handoff and the next owner.":
      "Nævn den blokerede overdragelse og den næste ejer.",
  "Do not declare cargo healthy without pod counts.":
      "Lad være med at erklære lasten sund uden pod-tællinger.",
  "Project Waddle launch day · July 18":
      "Project Waddle lanceringsdag · 18. juli",
  "Project Waddle launch day": "Project Waddle lanceringsdag",
  "The sentinel caught the pressure anomaly, but its launch report buried the go/no-go decision beneath six paragraphs of fish trivia.":
      "Sentinelen opdagede trykanomalien, men dens opsendelsesrapport begravede go/no-go-beslutningen under seks afsnit fisketrivia.",
  "Make seal evidence easier to scan.":
      "Gør det lettere at scanne forseglingsbeviser.",
  "Put seal anomalies first and make the final launch call explicit.":
      "Sæt forseglingsanomalier først og gør den endelige opsendelsesbeslutning eksplicit.",
  "## Result\n\n- Evidence now leads each finding.\n- Reports end with a plain-language launch recommendation.":
      "## Resultat\n\n- Beviser leder nu hvert fund.\n- Rapporter slutter med en anbefaling om lancering i et klart sprog.",
  "Promoted pressure evidence and added an explicit go/no-go ending.":
      "Fremmet trykbeviser og tilføjet en eksplicit go/no-go slutning.",
  "Admiral Pebble is appropriately skeptical, but should ask fewer questions when a launch hold is already obvious.":
      "Admiral Pebble er passende skeptisk, men burde stille færre spørgsmål, når en affyringspause allerede er åbenlys.",
  "pressure anomaly": "trykanomali",
  "calm flight director": "rolig flyveleder",
  "launch hold": "affyringspause",
  // Demo-world tutorial task ("Your first mission").
  "Your first mission": "Din første mission",
  "Work through the checklist below to learn the basics — everything in this demo world is safe to try.":
      "Arbejd dig gennem tjeklisten nedenfor for at lære det grundlæggende — i denne demoverden kan du trygt prøve alt.",
  "Learn the ropes": "Lær det grundlæggende",
  "Check this item off": "Sæt flueben ved dette punkt",
  "Start the timer on this task": "Start timeren på denne opgave",
  "Add your own checklist item": "Tilføj dit eget punkt til tjeklisten",
  "Create a brand-new task": "Opret en helt ny opgave",
  "Record a voice note": "Optag en talebesked",
  "Walk pressure seals A–F": "Gennemgå tryktætninger A–F",
  "Count all 37 emperor penguins": "Tæl alle 37 kejserpingviner",
  "Request Mission Control clearance":
      "Anmod om godkendelse fra Mission Control",
  "Pre-launch checks": "Tjek før opsendelse",
  "Seal walk complete: A–F held at 101.3 kPa overnight. Roll call confirmed all 37 penguins, including the one asleep in the cargo netting.":
      "Tætningsrunde fuldført: A–F holdt 101,3 kPa natten over. Navneopråbet bekræftede alle 37 pingviner, inklusive den der sov i fragtnettet.",
  // Demo-world expansion: linked clusters, notes, logged time, checklists.
  "Blocked": "Blokeret",
  "Waiting on": "Venter på",
  "Research": "Research",
  "Brief the boarding crew": "Brief indstigningsholdet",
  "Time the hatch sequence": "Tag tid på lugesekvensen",
  "Test the intercom": "Test samtaleanlægget",
  "Rehearse the abort call": "Øv afbrydelseskaldet",
  "Vent Bay A": "Udluft rum A",
  "Swap cartridges A1–A4": "Skift patroner A1–A4",
  "Log the CO2 baseline": "Notér CO2-udgangsværdien",
  "Return the used cartridges": "Returnér de brugte patroner",
  "Check bay two": "Tjek rum to",
  "Check the cold ring": "Tjek køleringen",
  "Ask the dock crew": "Spørg dokholdet",
  "File a loss report": "Indsend en tabsanmeldelse",
  "Colony news": "Nyt fra kolonien",
  "Launch update": "Nyt om opsendelsen",
  "Chick of the month": "Månedens unge",
  "Sardine recipe": "Sardinopskrift",
  "Pull the log exports": "Hent logeksporterne",
  "Flag every gap": "Markér hvert hul",
  "Recheck freezer 3": "Tjek fryser 3 igen",
  "Sign off the audit": "Godkend gennemgangen",
  "Count pods on the dock": "Tæl kapsler på dokken",
  "Match against the manifest": "Sammenhold med fragtlisten",
  "Confirm the cold-chain seals": "Bekræft koldkædeplomberne",
  "Send the corrected list": "Send den rettede liste",
  "Rehearsal script": "Prøvedrejebog",
  "Scrubber swap": "Filterskift",
  "Pallet search": "Palleeftersøgning",
  "Newsletter sections": "Afsnit i kolonibrevet",
  "Freezer log audit": "Gennemgang af fryserlogge",
  "Manifest checks": "Tjek af fragtlisten",
  "Bay A seals held 101.3 kPa all night.":
      "Pakningerne i rum A holdt 101,3 kPa hele natten.",
  "Cartridge order confirmed, arrives on the next shuttle.":
      "Patronbestillingen er bekræftet, den kommer med næste shuttle.",
  "Bay C is at 78% humidity, nine points up since Tuesday.":
      "Rum C ligger på 78 % luftfugtighed, ni point op siden tirsdag.",
  "The feeder still aims lunch at Mission Control.":
      "Foderautomaten sigter stadig frokosten mod Mission Control.",
  "Pallet 14 is not in bay two. Checking the cold ring next.":
      "Palle 14 er ikke i rum to. Næste stop er køleringen.",
  "Europa Krill quoted 12% below our current supplier.":
      "Europa Krill bød 12 % under vores nuværende leverandør.",
  "Customs wants the pod seal certificates before Friday.":
      "Tolden vil have certifikaterne på kapselpakningerne før fredag.",
  "The ice pad clears at 06:40 with a light crosswind.":
      "Isrampen er fri kl. 06:40 med let sidevind.",
  "Rehearsal ran nine minutes long on the boarding step.":
      "Prøven trak ni minutter over ved indstigningen.",
  "Three flight suits need a wider flipper cut.":
      "Tre flyvedragter skal have bredere luffesnit.",
  "The draft is done except for the launch section.":
      "Udkastet er færdigt på nær opsendelsesafsnittet.",
  "Two volunteers dropped out of the Thursday slot.":
      "To frivillige er sprunget fra torsdagsvagten.",
  "The colony voted for the documentary about ice.":
      "Kolonien stemte for dokumentaren om is.",
  "One sprained flipper, so we need softer landings.":
      "En forstuvet luffe, så vi skal have blødere landinger.",
  "Tilt is four degrees off after the last burn.":
      "Hældningen er fire grader ved siden af efter sidste motorbrænding.",
  "The recycler filter was clogged with feather down.":
      "Genbrugsanlæggets filter var stoppet af fjerdun.",
  "Freezer 3 logged a two-hour gap on Sunday.":
      "Fryser 3 loggede et hul på to timer søndag.",
  "The manifest says 40 pods, the dock counted 39.":
      "Fragtlisten siger 40 kapsler, dokken talte 39.",
  "Mission Control wants fewer fish puns in the launch script.":
      "Mission Control vil have færre fiskeordspil i opsendelsesdrejebogen.",
  "Sir Flaps-a-Lot answered roll call from the cargo netting.":
      "Sir Flaps-a-Lot svarede på navneopråbet fra fragtnettet.",
  "Swapped cartridges in Bay A and B.": "Skiftede patroner i rum A og B.",
  "Two hours chasing the humidity leak, no source yet.":
      "To timers jagt på fugtlækagen, ingen kilde endnu.",
  "Walked the whole cold ring looking for pallet 14.":
      "Gik hele køleringen igennem efter palle 14.",
  "Full rehearsal run with the boarding crew.":
      "Fuld gennemgang med indstigningsholdet.",
  "Reconciled two weeks of freezer logs.": "Afstemte to ugers fryserlogge.",
  "Wrote the colony newsletter draft.": "Skrev udkastet til kolonibrevet.",
  "Measured the array tilt against the sun sensor.":
      "Målte solpanelernes hældning mod solsensoren.",
  "Cleaned the recycler filter housing.":
      "Rensede genbrugsanlæggets filterhus.",
  "Counted pods on the dock with the shuttle crew.":
      "Talte kapsler på dokken med shuttle-holdet.",
  "Rewrote the launch script intro.":
      "Skrev indledningen til opsendelsesdrejebogen om.",
  "Draft the launch comms plan":
      "Lav udkast til opsendelsens kommunikationsplan",
  "Agree who announces what on launch day, and in which order.":
      "Bliv enige om, hvem der melder hvad ud på opsendelsesdagen — og i hvilken rækkefølge.",
  "Check the ice-pad weather window": "Tjek vejrvinduet ved isrampen",
  "Confirm the crosswind stays under limits for the launch slot.":
      "Bekræft, at sidevinden holder sig under grænsen i opsendelsesvinduet.",
  "Audit the cold-chain freezer logs": "Gennemgå koldkædens fryserlogge",
  "Find every gap in the freezer logs before the sardines ship.":
      "Find hvert eneste hul i fryserloggene, før sardinerne sendes af sted.",
  "Run the launch-day rehearsal": "Afhold generalprøven på opsendelsen",
  "Walk the whole launch morning once, at full speed, with the crew.":
      "Gå hele opsendelsesmorgenen igennem én gang, i fuldt tempo, med holdet.",
  "Fit the penguin flight suits": "Tilpas pingvinernes flyvedragter",
  "Measure every flyer and send the three wide-flipper suits back.":
      "Mål hver flyver, og send de tre dragter med brede luffer retur.",
  "Replace the air scrubber cartridges": "Skift luftrensepatronerne",
  "Swap all four cartridges in Bay A before the CO2 alarm gets bored.":
      "Skift alle fire patroner i rum A, før CO2-alarmen keder sig.",
  "Trace the humidity spike in Bay C": "Spor fugtspidsen i rum C",
  "Nine points in three days is a leak, not weather.":
      "Ni point på tre dage er en lækage, ikke vejr.",
  "Waiting on the Bay C sensor swap": "Venter på sensorskiftet i rum C",
  "Resurface the habitat ice rink": "Læg ny is på habitatets skøjtebane",
  "The colony rink has more grooves than ice. Book the resurfacer.":
      "Koloniens bane har flere riller end is. Book ismaskinen.",
  "Retune the solar array tilt": "Justér solpanelernes hældning",
  "Four degrees of drift is costing the habitat a third of its power.":
      "Fire graders afvigelse koster habitatet en tredjedel af strømmen.",
  "Service the water recycler": "Servicér vandgenbrugsanlægget",
  "Clean the filter housing and log the throughput afterwards.":
      "Rens filterhuset, og notér gennemstrømningen bagefter.",
  "Find the missing squid pallet": "Find den forsvundne blækspruttepalle",
  "Pallet 14 left Europa and never reached the cold ring.":
      "Palle 14 forlod Europa og nåede aldrig køleringen.",
  "Shortlist a second krill supplier": "Udvælg en ekstra krill-leverandør",
  "One supplier for the whole colony is one storm away from trouble.":
      "Én leverandør til hele kolonien er én storm fra ballade.",
  "Reconcile the shuttle manifest": "Afstem shuttlens fragtliste",
  "The manifest and the dock disagree by one pod. Find out which.":
      "Fragtlisten og dokken er uenige om én kapsel. Find ud af hvilken.",
  "Order replacement pod seals": "Bestil nye kapselpakninger",
  "Customs will not clear a pod whose seal certificate has expired.":
      "Tolden frigiver ikke en kapsel, hvis pakningscertifikat er udløbet.",
  "Clear customs on Europa": "Kom gennem tolden på Europa",
  "File the seal certificates and the passenger question together.":
      "Indsend pakningscertifikaterne og passagerspørgsmålet samlet.",
  "Europa customs is closed until Monday":
      "Tolden på Europa har lukket til mandag",
  "Write the colony newsletter": "Skriv kolonibrevet",
  "Four sections, one photo, and no more than one fish pun.":
      "Fire afsnit, ét foto og højst ét fiskeordspil.",
  "Refill the chick daycare rota": "Fyld vagtplanen for ungepasningen op",
  "Thursday lost two volunteers and the chicks noticed immediately.":
      "Torsdagsvagten mistede to frivillige, og ungerne opdagede det med det samme.",
  "Pick the film for colony night": "Vælg filmen til kolonieaftenen",
  "The colony voted for the ice documentary. Book the dome.":
      "Kolonien stemte for is-dokumentaren. Book kuplen.",
  "Restart the tobogganing league": "Genstart kælkeligaen",
  "Softer landings first, then a rematch against Bay C.":
      "Først blødere landinger, så en omkamp mod rum C.",
  "Habitat Engineering": "Habitatteknik",
  "Logistics & Supply": "Logistik & forsyning",
  "Check cold-chain telemetry": "Tjek kølekædens telemetri",
  "Scan overnight freezer readings before the first cargo handoff.":
      "Gennemgå nattens frysermålinger før den første fragtoverdragelse.",
  "Reconcile outbound cargo manifest": "Afstem manifestet for udgående fragt",
  "Match every pod, pallet, and penguin signature before departure.":
      "Match hver kapsel, palle og pingvinunderskrift før afgang.",
  "Send the end-of-shift handoff": "Send vagtskifteoverdragelsen",
  "Leave the next specialist a crisp update, including any drifting fish.":
      "Giv den næste specialist en klar opdatering, inklusive eventuelle fisk "
      "på afveje.",
  "Zero-gravity flipper mobility": "Luffemobilitet i vægtløshed",
  "Complete three gentle mobility sessions before the weekly cargo sprint.":
      "Gennemfør tre rolige mobilitetspas før ugens fragtsprint.",
  // Demo-world expansion: launch-prep checklist items and task descriptions.
  "List every launch-day announcement":
      "Oplist alle udmeldinger på opsendelsesdagen",
  "Name a speaker for each slot": "Udpeg en stemme til hvert slot",
  "Write the fallback script for a hold":
      "Skriv reservedrejebogen til en affyringspause",
  "Get sign-off from Flight Direction": "Indhent godkendelse fra flyveledelsen",
  "Pull the morning soundings": "Hent morgensonderingerne",
  "Compare the three forecast models": "Sammenlign de tre vejrmodeller",
  "Check the crosswind against the 18-knot limit":
      "Tjek sidevinden mod grænsen på 18 knob",
  "Send the go or no-go call to Flight Direction":
      "Meld go eller no-go til flyveledelsen",
  "Remeasure all twelve flyers": "Mål alle tolv flyvere igen",
  "Tag the three wide-flipper returns":
      "Mærk de tre returdragter med brede luffer",
  "Book the courier pickup": "Book kurérafhentningen",
  "Confirm the replacement delivery date":
      "Bekræft erstatningens leveringsdato",
  "Chart the humidity readings by day": "Optegn fugtmålingerne dag for dag",
  "Wait for the Bay C sensor swap": "Vent på sensorskiftet i rum C",
  "Walk the seam line with the thermal camera":
      "Gå sømlinjen efter med varmekameraet",
  "Report the leak or clear the bay": "Meld lækagen eller frigiv rummet",
  "Book the resurfacer for an evening slot": "Book ismaskinen til en aftentid",
  "Announce the one-session closure": "Varsl lukningen i én session",
  "Resurface and inspect the ice": "Læg ny is, og efterse den",
  "Post the new ice rules": "Hæng de nye isregler op",
  "Recalibrate against the noon reference": "Rekalibrer mod middagsreferencen",
  "Verify the tracking curve for a full day":
      "Kontrollér følgekurven en hel dag",
  "Log the recovered wattage": "Notér den genvundne effekt",
  "Close the storm-damage ticket": "Luk stormskadesagen",
  "Flush the recycler loop": "Skyl genbrugsanlæggets kredsløb",
  "Clean the filter housing": "Rens filterhuset",
  "Swap the pre-filter": "Skift forfilteret",
  "Log the throughput numbers": "Notér gennemstrømningstallene",
  "Write down the selection criteria": "Skriv udvælgelseskriterierne ned",
  "Collect quotes from three candidates": "Indhent tilbud fra tre kandidater",
  "Order sample crates from the best two":
      "Bestil prøvekasser hos de to bedste",
  "Put a recommendation to the council": "Fremlæg en anbefaling for rådet",
  "Confirm seal sizes against the pod register":
      "Bekræft pakningsstørrelserne mod kapselregistret",
  "Place the rush order": "Afgiv hasteordren",
  "Chase the delivery confirmation": "Ryk for leveringsbekræftelsen",
  "Warn Logistics about the affected pods":
      "Advar logistikken om de berørte kapsler",
  "Assemble the certificate folder": "Saml certifikatmappen",
  "Draft the passenger ruling request":
      "Lav udkast til anmodningen om passagerspørgsmålet",
  "Copy the current manifest": "Kopiér den aktuelle fragtliste",
  "Book the Monday customs slot": "Book toldtiden mandag",
  "Ask the reserve list for Thursday": "Spørg reservelisten om torsdagsvagten",
  "Post the open shifts on the colony board":
      "Hæng de ledige vagter op på koloniens opslagstavle",
  "Confirm two names per shift": "Bekræft to navne pr. vagt",
  "Publish the full week by Friday": "Hæng hele ugeplanen op senest fredag",
  "Request the screening licence": "Anmod om visningslicensen",
  "Reserve the dome for the evening": "Reservér kuplen til aftenen",
  "Schedule the heater shutdown": "Planlæg slukningen af varmen",
  "Post the start time on the colony board":
      "Skriv starttidspunktet på koloniens opslagstavle",
  "Pile fresh snow on the run-out": "Skovl frisk sne i opløbet",
  "Set the doubles-sled weight limit": "Fastsæt vægtgrænsen for dobbeltkælken",
  "Draft the new season schedule": "Lav udkast til den nye sæsonplan",
  "Send the rematch challenge to Bay C": "Send omkampsudfordringen til rum C",
  "Agree who announces what on launch day, and in which order. Mission Control wants one voice on the public feed, the colony wants updates in plain penguin, and the sponsors expect their logo mentioned exactly twice. Draft the running order, name a speaker per slot, and get sign-off from Flight Direction before the rehearsal.":
      "Bliv enige om, hvem der melder hvad ud på opsendelsesdagen — og i hvilken rækkefølge. Mission Control vil have én stemme på den offentlige kanal, kolonien vil have opdateringer på klart pingvinsk, og sponsorerne forventer deres logo nævnt præcis to gange. Lav udkast til programrækkefølgen, udpeg en stemme til hvert slot, og indhent godkendelse fra flyveledelsen før generalprøven.",
  "Confirm the crosswind stays under limits for the launch slot. The ice pad tolerates 18 knots across the strip and the forecast has been flirting with 20 all week. Pull the morning soundings, compare the three forecast models, and give Flight Direction a clear go or no-go with a confidence note by 15:00.":
      "Bekræft, at sidevinden holder sig under grænsen i opsendelsesvinduet. Isrampen tåler 18 knob på tværs af banen, og prognosen har flirtet med 20 hele ugen. Hent morgensonderingerne, sammenlign de tre vejrmodeller, og giv flyveledelsen et klart go eller no-go med en konfidensnote senest kl. 15:00.",
  "Find every gap in the freezer logs before the sardines ship. Freezer 3 dropped out of range twice last month, and customs wants an unbroken cold-chain record for every pod on the manifest. Export the logs, flag every excursion with its duration, and sign the audit so the shipment can clear.":
      "Find hvert eneste hul i fryserloggene, før sardinerne sendes af sted. Fryser 3 røg uden for sit måleområde to gange i sidste måned, og tolden vil have en ubrudt kølekædedokumentation for hver kapsel på fragtlisten. Eksportér loggene, markér hver afvigelse med dens varighed, og godkend gennemgangen, så forsendelsen kan frigives.",
  "Walk the whole launch morning once, at full speed, with the crew. Every handoff gets timed, every call gets spoken out loud, and nobody pauses to explain — that is what the debrief is for. Reserve the pad for three hours and rehearse the abort call until it sounds boring.":
      "Gå hele opsendelsesmorgenen igennem én gang, i fuldt tempo, med holdet. Der tages tid på hver overdragelse, hvert kald siges højt, og ingen stopper op for at forklare — det er det, debriefingen er til. Reservér rampen i tre timer, og øv afbrydelseskaldet, indtil det lyder kedeligt.",
  "Measure every flyer and send the three wide-flipper suits back. The supplier mixed up the flipper gauges, so half the squad is wearing suits that whistle above Mach 0.2. Remeasure all twelve flyers, tag the returns, and confirm the replacement delivery lands before the rehearsal.":
      "Mål hver flyver, og send de tre dragter med brede luffer retur. Leverandøren byttede rundt på luffemålene, så halvdelen af eskadrillen går i dragter, der fløjter over Mach 0,2. Mål alle tolv flyvere igen, mærk returneringerne, og bekræft, at erstatningen lander før generalprøven.",
  "Swap all four cartridges in Bay A before the CO2 alarm gets bored. The current set is 12 percent over its rated hours and the alarm has started clearing its throat at night. Vent the bay first, swap A1 through A4 in order, and log the new CO2 baseline so Habitat Engineering can close the maintenance ticket.":
      "Skift alle fire patroner i rum A, før CO2-alarmen keder sig. Det nuværende sæt er 12 procent over sine normerede timer, og alarmen er begyndt at rømme sig om natten. Udluft rummet først, skift A1 til A4 i rækkefølge, og notér den nye CO2-udgangsværdi, så habitatteknikken kan lukke vedligeholdelsessagen.",
  "Nine points in three days is a leak, not weather. Bay C sits between the ice rink and the nursery, so a hidden leak there ends up in every feather on the station. The wall sensor is being swapped; once it reports again, walk the seam line with the thermal camera and either find the leak or clear the bay.":
      "Ni point på tre dage er en lækage, ikke vejr. Rum C ligger mellem skøjtebanen og ungepasningen, så en skjult lækage dér ender i hver eneste fjer på stationen. Vægsensoren er ved at blive skiftet — så snart den melder igen, skal I gå sømlinjen efter med varmekameraet og enten finde lækagen eller frigive rummet.",
  "The colony rink has more grooves than ice. Book the resurfacer. Tobogganing practice starts again next month and the juniors keep catching their flippers in the ruts. Reserve the resurfacer for a quiet evening, close the rink for one session, and post the new ice rules before reopening.":
      "Koloniens bane har flere riller end is. Book ismaskinen. Kælketræningen starter igen næste måned, og juniorerne bliver ved med at hænge fast med lufferne i rillerne. Reservér ismaskinen til en stille aften, luk banen i én session, og hæng de nye isregler op før genåbningen.",
  "Four degrees of drift is costing the habitat a third of its power. The tracking motor lost its calibration in the last dust storm and the batteries have dipped below reserve every night since. Recalibrate the tilt against the noon reference, verify the tracking curve over a full day, and log the recovered wattage.":
      "Fire graders afvigelse koster habitatet en tredjedel af strømmen. Følgemotoren mistede sin kalibrering i den seneste støvstorm, og batterierne er siden røget under reserven hver nat. Rekalibrer hældningen mod middagsreferencen, kontrollér følgekurven over en hel dag, og notér den genvundne effekt.",
  "Clean the filter housing and log the throughput afterwards. Routine service, but the last crew skipped the throughput log and Habitat Engineering had to guess the filter age. Do the full sequence this time: flush the loop, clean the housing, swap the pre-filter, and write the numbers down.":
      "Rens filterhuset, og notér gennemstrømningen bagefter. Rutineservice — men det sidste hold sprang gennemstrømningsloggen over, og habitatteknikken måtte gætte filterets alder. Tag hele sekvensen denne gang: skyl kredsløbet, rens huset, skift forfilteret, og skriv tallene ned.",
  "Pallet 14 left Europa and never reached the cold ring. The dock scanner shows it entering bay two at 04:12, then nothing — and the squid inside has a five-day cold rating that runs out on Thursday. Trace the scan trail, search both bays, and either find the pallet or file the loss report before the insurance window closes.":
      "Palle 14 forlod Europa og nåede aldrig køleringen. Dokscanneren viser den på vej ind i rum to kl. 04:12, derefter ingenting — og blæksprutten indeni har en kølegodkendelse på fem dage, der udløber torsdag. Følg scanningssporet, gennemsøg begge rum, og find pallen — eller indsend tabsanmeldelsen, før forsikringsvinduet lukker.",
  "One supplier for the whole colony is one storm away from trouble. Procurement wants a second source signed before the winter contracts renew. Write down the criteria that actually matter — delivery time, cold-chain rating, price per tonne — collect quotes from at least three candidates, order a sample crate from the best two, and put a recommendation in front of the council.":
      "Én leverandør til hele kolonien er én storm fra ballade. Indkøb vil have en ekstra kilde på kontrakt, før vinterkontrakterne fornys. Skriv de kriterier ned, der faktisk betyder noget — leveringstid, kølekædevurdering, pris pr. ton — indhent tilbud fra mindst tre kandidater, bestil en prøvekasse hos de to bedste, og fremlæg en anbefaling for rådet.",
  "The manifest and the dock disagree by one pod. Find out which. Twenty-three pods on paper, twenty-four on the dock — and launch clearance needs the two lists to match to the pod. Count what is actually standing there, reconcile against the manifest line by line, and send the corrected list to Europa before the shuttle is loaded.":
      "Fragtlisten og dokken er uenige om én kapsel. Find ud af hvilken. Treogtyve kapsler på papiret, fireogtyve på dokken — og opsendelsestilladelsen kræver to lister, der stemmer ned til den enkelte kapsel. Tæl, hvad der faktisk står der, afstem linje for linje mod fragtlisten, og send den rettede liste til Europa, før shuttlen lastes.",
  "Customs will not clear a pod whose seal certificate has expired. Eight pods run out of certificate next week and the seal supplier quotes ten days for delivery — the order is already five days late. Confirm the sizes against the pod register, place the rush order, and warn Logistics which pods will miss the next shuttle either way.":
      "Tolden frigiver ikke en kapsel, hvis pakningscertifikat er udløbet. Otte kapslers certifikater udløber i næste uge, og pakningsleverandøren melder ti dages leveringstid — bestillingen er allerede fem dage forsinket. Bekræft størrelserne mod kapselregistret, afgiv hasteordren, og advar logistikken om, hvilke kapsler der under alle omstændigheder misser næste shuttle.",
  "File the seal certificates and the passenger question together. Europa customs reopens Monday, and they process bundled filings faster than loose paperwork. Have the certificate folder, the passenger ruling request, and the manifest copy ready in one envelope so the whole stack clears in a single visit.":
      "Indsend pakningscertifikaterne og passagerspørgsmålet samlet. Tolden på Europa åbner igen mandag, og de behandler samlede indsendelser hurtigere end løse papirer. Hav certifikatmappen, anmodningen om passagerspørgsmålet og kopien af fragtlisten klar i én kuvert, så hele stakken går igennem ved ét besøg.",
  "Four sections, one photo, and no more than one fish pun. The colony reads it over breakfast, so keep it short and warm: what happened, what is coming, who hatched, and what the kitchen is planning. The launch update must match what Comms announces — check with them before printing.":
      "Fire afsnit, ét foto og højst ét fiskeordspil. Kolonien læser det over morgenmaden, så hold det kort og varmt: hvad der er sket, hvad der kommer, hvem der er klækket, og hvad køkkenet planlægger. Nyt om opsendelsen skal stemme med det, Comms melder ud — tjek med dem, før der trykkes.",
  "Thursday lost two volunteers and the chicks noticed immediately. The rota needs two names per shift or the nursery closes early, and early closing means chicks at the launch briefing again. Ask the reserve list first, then post the open shifts on the colony board, and confirm the full week by Friday.":
      "Torsdagsvagten mistede to frivillige, og ungerne opdagede det med det samme. Vagtplanen kræver to navne pr. vagt, ellers lukker ungepasningen tidligt — og tidlig lukning betyder unger til opsendelsesbriefingen igen. Spørg reservelisten først, hæng derefter de ledige vagter op på koloniens opslagstavle, og bekræft hele ugen senest fredag.",
  "The colony voted for the ice documentary. Book the dome. The distributor needs three days of notice for the screening licence, and the dome heater has to be off for two hours before anyone brings blankets. Book the licence, reserve the dome, and put the start time on the colony board.":
      "Kolonien stemte for is-dokumentaren. Book kuplen. Distributøren skal have tre dages varsel til visningslicensen, og varmen i kuplen skal være slukket to timer, før nogen kommer med tæpper. Book licensen, reservér kuplen, og skriv starttidspunktet på koloniens opslagstavle.",
  "Softer landings first, then a rematch against Bay C. The league stopped after two bruised tails and one very formal complaint. Pile fresh snow on the run-out, set a weight limit for the doubles sled, and only then challenge Bay C — they have been practising.":
      "Først blødere landinger, så en omkamp mod rum C. Ligaen stoppede efter to forslåede haler og én meget formel klage. Skovl frisk sne i opløbet, fastsæt en vægtgrænse for dobbeltkælken, og udfordr først derefter rum C — de har trænet.",
};
