  ...
  Widget build(BuildContext context) {
    ...
    if (identity == null) {
      if (widget.hasProjectAgent) {
        return _ProjectAgentReadOnlySummary(
    ...
    final effectiveIdentity = identity;
    final report = ref
    ...
    final additionalReport = summary.isNotEmpty && content != summary
        ? content
        : null;
    final hasReportContent = tldr.isNotEmpty;
    ...
    return AgentSummaryCardSurface(
      children: [
        TldrHeader(agentName: agentName, onAgentTap: openInternals),
        ...
        if (!widget.isMutating) ?widget.actions,
        footer,
      ],
    );
  }
...
class _ProjectAgentReadOnlySummary extends StatefulWidget {
  const _ProjectAgentReadOnlySummary({
    required this.record,
  ...
  @override
  State<_ProjectAgentReadOnlySummary> createState() =>
      _ProjectAgentReadOnlySummaryState();
}

class _ProjectAgentReadOnlySummaryState
    extends State<_ProjectAgentReadOnlySummary> {
  bool _expanded = false;
  ...
  Widget build(BuildContext context) {
    ...
    return AgentSummaryCardSurface(
      children: [
        const TldrHeader(agentName: null),
        if (widget.record.healthMetrics != null)
          Padding(
            padding: EdgeInsets.fromLTRB(
              tokens.spacing.cardPadding,
