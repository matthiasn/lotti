  ...
  Future<void> arm({
    ...
    await _bestEffort('arm', () async {
      ...
      await _notifications.createRelationshipCheckIn(
        ...
        // leaves the app sandbox and lands on a lock screen, so it carries
        // the person's name and nothing else about them — no cadence, no
        // recency, nothing from a check-in.
        title: messages.relationshipCheckInReminderTitle(
          relationship.data.title,
        ),
        body: messages.relationshipCheckInReminderBody,
        scheduledFor: _reminderInstant(derivation),
