...
/// left column shows [_TreeSlotPlaceholder] and the right column
/// shows a neutral empty-state via [SettingsV2DetailPlaceholder].
          ...
          Expanded(
            child: Row(
              ...
              children: [
                SizedBox(
                  width: treeWidth,
                  child: const _TreeSlotPlaceholder(),
                ),
...
/// Empty left-column stand-in. The real `SettingsTreeView` lands in
/// the next PR (plan step 4); until then the slot renders as a plain
/// surface so the page chrome, resize handle, and detail placeholder
/// can be exercised end-to-end behind `enableSettingsTreeFlag`.
class _TreeSlotPlaceholder extends StatelessWidget {
  const _TreeSlotPlaceholder();

  @override
  Widget build(BuildContext context) {
    final tokens = context.designTokens;
    return ColoredBox(color: tokens.colors.background.level01);
  }
}
