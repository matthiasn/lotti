...
- `SavedTaskFilterRail` — a conditional band rendered only when ≥1 saved filter exists (otherwise `SizedBox.shrink`, so the layout is unchanged for users without saved filters). Left→right: a chip-chromed "Saved (N)" button (a neutral filled+bordered `DsPill` led by a bookmark glyph and closed by a disclosure chevron, so it reads as a menu-opening button rather than a static label; opens the sheet), an "All" pill that clears to the default view, the active saved pill (or a "Custom" pill for an ad-hoc filter that matches no saved filter), as many most-recently-used quick-jump pills as fit, and a trailing teal "+ Save" call-to-action — a `DsPill(outline, interactive.enabled)` with a leading `+`, NOT the muted dashed `DsGhostChip` skin (that is reserved for true empty states) — shown only when `tasksFilterHasUnsavedClausesProvider` is true. The pill run is hard-capped and never scrolls — overflow lives in the sheet; MRU fit is computed deterministically in `LayoutBuilder` from token-derived width heuristics. The selection is tri-state (a saved pill, "All", or "Custom") so exactly one anchor always reads. At large text (`MediaQuery.textScalerOf(context).scale(1) ≥ 1.3`) the rail collapses to just the "Saved (N)" button + the single active anchor pill (dropping "All" and the MRU quick-jumps) so the active filter's name + count stay readable. Counts use stale-while-revalidate (`AsyncValue.value`) so a background sync never flashes pills back to `–`.
- `SavedTaskFilterPill` — a `DsPill`-based pill (leading selection check + optional category dot, ellipsizing name, min-width tabular count slot capped at `999+`, dimmed `0`, `–` on cold start) wrapped in a ≥48dp tap target. The count slot reserves a `step7` min-width for column alignment but is free to grow, so the digits are never width-clipped — at large text the **name** ellipsizes while the full count still renders. The active pill carries a chevron cue and opens the sheet; inactive pills apply their filter. Selection draws on the orthogonal `DsPill.selected` flag (teal `interactive.enabled` border + `surface.selected` tint + bold label); on the mint selected fill the check, count, and chevron render in `text.highEmphasis` (not the teal accent) so they stay legible in light theme.
- `SavedTaskFiltersSheet` (`showSavedTaskFiltersSheet`) — the complete switcher + manager: full-width ≥48dp single-select rows with a right-aligned tabular count column (same min-width-but-growable slot as the rail pill), an "All tasks" row showing `allTasksTotalCountProvider`, an Edit toggle (teal `interactive.enabled` foreground — the sheet's one accent) revealing per-row Rename (text modal) / Delete, where Rename and Delete are each ≥48dp targets separated by a clear gap and Delete routes through `showConfirmationModal`, and a bottom "Save current filter as…" create row. Tapping a row applies + closes.
