...
- `TaskActionBar` — a sticky frosted-glass bar hosted in the page's
  ...
  - Tracking-this-task: the live elapsed time replaces the label, with
    ...
    navigates to the running timer entry (mirrors the desktop activity
    detail); only the inset stop circle stops the timer. The
  ...
  shell has no floating recording indicator; the desktop sidebar exposes the
  running session through `SidebarActivitySummary` (see "Sidebar activity
  coordination" below).
...
navigation. The stream is seeded with `TimeService.getCurrent()` as
`initialData` so an already-running session renders on the first frame.
...
- `AiSummaryCard` — a single deep-teal-tinted-navy surface that hosts the agent's TLDR + expandable inline report, the unified open-proposal list with swipe / button confirm-or-reject + collapsible history, the recent-activity footer (inline expand), and the automation band (an always-available `Update now` trigger, a freshness word, a `Skip` that cancels only the pending automatic run, the automatic-updates switch, and a countdown that switches from `m:ss` to `h:mm:ss` once an hour cell is needed and shortens its wording rather than truncating the time as width runs out). Tapping the agent name (or the avatar / "Open agent internals" pill) opens `AgentInternalsPanel`, a right-side overlay (600–800px wide) that re-houses the existing `AgentInternalsBody` (Stats / Reports / Conversations / Observations / Activity tabs) without page navigation
- `LinkedTasksWidget`
- `ChecklistsWidget`
