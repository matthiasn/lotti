...
- `SavedTaskFilterRail` — a conditional band rendered only when ≥1 saved filter exists (otherwise `SizedBox.shrink`, so the layout is unchanged for users without saved filters). Left→right: a chip-chromed "Saved (N)" button (a neutral filled+bordered `DsPill` led by a bookmark glyph and closed by the rail's **single** disclosure chevron, so it reads as a menu-opening button rather than a static label; opens the sheet). The `(N)` saved-filter count is restored as a **subordinate** numeral — rendered in `text.lowEmphasis` (dimmer than the `text.mediumEmphasis` task-count pills) and parenthetical via the `tasksSavedFiltersRailButton(count)` placeholder, so it reads as "N saved filters" rather than a third peer task-count beside the pill numbers; the label word keeps the filled pill's `text.highEmphasis` and its glyphs use `text.highEmphasis` so they stay legible over the light-theme pill fill. Then an "All" pill that clears to the default view, the active saved pill (or a "Custom" pill carrying the live filtered count for an ad-hoc filter that matches no saved filter — sourced from `currentTasksFilterCountProvider`, which mirrors the live list's empty-status→all-statuses expansion so the number agrees with the list), as many most-recently-used quick-jump pills as fit, and a trailing teal "+ Save" call-to-action — a `DsPill(outline, interactive.enabled)` with a leading `+`, NOT the muted dashed `DsGhostChip` skin (that is reserved for true empty states) — shown only when `tasksFilterHasUnsavedClausesProvider` is true. The chevron lives on the "Saved" button **only**: the active and "Custom" pills carry none, so every pill is one predictable whole-pill tap target (inactive pills apply/switch; the active and "Custom" pills open the sheet). The pill run is hard-capped and never scrolls — overflow lives in the sheet; MRU fit is computed deterministically in `LayoutBuilder` from token-derived width heuristics. The selection is tri-state (a saved pill, "All", or "Custom") so exactly one anchor always reads. At large text (`MediaQuery.textScalerOf(context).scale(1) ≥ 1.3`) the rail collapses to the single active anchor pill + a compact "All" reset (those two share an `Expanded` horizontal `SingleChildScrollView`) with the "Saved" button (and the Save CTA) **pinned outside the scroll** at the trailing edge so the sheet opener is always visible — it must never scroll off into an "S" sliver; only the MRU quick-jumps are dropped. "All" is kept because return-to-unfiltered is the most common escape hatch, and when "All" *is* the active selection it doubles as the anchor rather than rendering twice. Counts use stale-while-revalidate (`AsyncValue.value`) so a background sync never flashes pills back to `–`.
- `SavedTaskFilterPill` — a `DsPill`-based pill (optional leading category dot, ellipsizing name, min-width tabular count slot capped at `999+`, dimmed `0`, `–` on cold start) wrapped in a ≥48dp tap target. There is deliberately **no** in-pill selection check and **no** chevron: the active state is already encoded by the border/fill/bold name (and the category dot), and the disclosure caret lives only on the "Saved" button, so each pill stays a single unambiguous tap target. The freed width goes to the name, and a name that still overflows truncates its leading "Category · " prefix *before* its trailing "· Status" segment (`_PillLabel` lays the prefix out in a `Flexible` so it ellipsizes first while the `·`-led status segment is pinned beside it; names without a `·` fall back to ordinary trailing ellipsis) — the dot already conveys the category, so the status is the higher-value half to keep. The active pill is the rail's `Flexible` (priority-width) element while "All" stays content-sized. The count is rendered by the shared `SavedFilterCountText` widget (see below). Selection draws on the orthogonal `DsPill.selected` flag (teal `interactive.enabled` border + `surface.selected` tint + bold label).
- `SavedFilterCountText` (`saved_task_filter_pill.dart`) — the **single** count renderer shared by the rail pill and the sheet rows, so the same number never changes type, weight, or emphasis between surfaces. One type token (`others.caption`), tabular figures, fixed `w600` weight; the count is a *secondary* datum reading `text.mediumEmphasis`, with a dimmed `0` and a cold-start / loading `–` (a null count) dropping to `text.lowEmphasis`. `minWidth` reserves a stable column start (rail pill `step7`, sheet `step8`) but is only a MIN — the slot grows so at large text the name ellipsizes while the full count is never width-clipped.
- `SavedTaskFiltersSheet` (`showSavedTaskFiltersSheet`) — the complete switcher + manager: full-width ≥48dp single-select rows whose **active** row carries a token-backed `colors.surface.selected` background tint (the same mint the rail's selected pill uses) so selection is multi-channel (tinted surface + teal radio + bold name) rather than leaning on the radio alone, plus a right-aligned `SavedFilterCountText` column (the same shared renderer + min-width-but-growable slot as the rail pill, so the same number never changes colour between rail and sheet). An "All tasks" row shows `allTasksTotalCountProvider`; an Edit toggle (teal `interactive.enabled` foreground — the sheet's one accent) reveals per-row Rename (text modal) / Delete, each a ≥48dp target separated by a clear `step3` gap, with Delete routing through `showConfirmationModal`. Edit-mode rows keep the **same 48dp row height** as normal rows (the ≥48dp action targets define the height; no extra vertical padding is added) so toggling Edit swaps the count for the action pair without the list jumping, and "All tasks" drops its count in Edit mode so a lone count is never mixed against the other rows' action-pairs. Deleting the *currently-active* saved filter falls back to the default "All" view (`SavedTaskFilterActivator.clearToDefault()`) so the live filter is never left on an orphaned shape; "All tasks" itself is non-deletable. A bottom "Save current filter as…" create row closes the sheet. Tapping a row applies + closes.
...
The "Saved" button is a plain menu-opener with no number — there is intentionally no saved-*definitions* count on the rail, so the only numbers a user sees are per-pill/per-row task counts, all in the same secondary tabular treatment.
