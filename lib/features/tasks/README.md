...
- `SavedTaskFilterRail` — a conditional band rendered only when ≥1 saved filter exists (otherwise `SizedBox.shrink`, so the layout is unchanged for users without saved filters). Left→right: a chip-chromed "Saved" button (a neutral filled+bordered `DsPill` led by a bookmark glyph and closed by a disclosure chevron, so it reads as a menu-opening button rather than a static label; opens the sheet — it carries **no** filter-count badge, since "Saved (N)" read like a third task-count beside the pill numbers, and its glyphs use `text.highEmphasis` so they stay legible over the light-theme pill fill), an "All" pill that clears to the default view, the active saved pill (or a "Custom" pill carrying the live filtered count for an ad-hoc filter that matches no saved filter — sourced from `currentTasksFilterCountProvider`, which mirrors the live list's empty-status→all-statuses expansion so the number agrees with the list), as many most-recently-used quick-jump pills as fit, and a trailing teal "+ Save" call-to-action — a `DsPill(outline, interactive.enabled)` with a leading `+`, NOT the muted dashed `DsGhostChip` skin (that is reserved for true empty states) — shown only when `tasksFilterHasUnsavedClausesProvider` is true. The pill run is hard-capped and never scrolls — overflow lives in the sheet; MRU fit is computed deterministically in `LayoutBuilder` from token-derived width heuristics. The selection is tri-state (a saved pill, "All", or "Custom") so exactly one anchor always reads. At large text (`MediaQuery.textScalerOf(context).scale(1) ≥ 1.3`) the rail collapses to the "Saved" button + a compact "All" reset + the single active anchor pill (only the MRU quick-jumps are dropped); "All" is kept because return-to-unfiltered is the most common escape hatch, and when "All" *is* the active selection it doubles as the anchor rather than rendering twice. Counts use stale-while-revalidate (`AsyncValue.value`) so a background sync never flashes pills back to `–`.
- `SavedTaskFilterPill` — a `DsPill`-based pill (optional leading category dot, ellipsizing name, min-width tabular count slot capped at `999+`, dimmed `0`, `–` on cold start) wrapped in a ≥48dp tap target. There is deliberately **no** in-pill selection check: the active state is already encoded by the border/fill/bold name (and the category dot), so a redundant check would only steal width from the name. The freed width goes to the name, and a name that still overflows truncates its leading "Category · " prefix *before* its trailing "· Status" segment (`_PillLabel` lays the prefix out in a `Flexible` so it ellipsizes first while the `·`-led status segment is pinned beside it; names without a `·` fall back to ordinary trailing ellipsis) — the dot already conveys the category, so the status is the higher-value half to keep. The active pill is the rail's `Flexible` (priority-width) element while "All" stays content-sized. The count slot reserves a `step7` min-width for column alignment but is free to grow, so the digits are never width-clipped — at large text the **name** ellipsizes while the full count still renders. The count is a *secondary* datum: it reads `text.mediumEmphasis` in **every** state (selected or not), identical to the sheet's count — the active state is never conveyed by re-colouring the number. The active pill carries a chevron cue (a `text.highEmphasis` disclosure affordance) and opens the sheet; inactive pills apply their filter. Selection draws on the orthogonal `DsPill.selected` flag (teal `interactive.enabled` border + `surface.selected` tint + bold label).
- `SavedTaskFiltersSheet` (`showSavedTaskFiltersSheet`) — the complete switcher + manager: full-width ≥48dp single-select rows with a right-aligned tabular count column (same min-width-but-growable slot as the rail pill, and the same secondary `text.mediumEmphasis` count treatment so the same number never changes colour between rail and sheet), an "All tasks" row showing `allTasksTotalCountProvider`, an Edit toggle (teal `interactive.enabled` foreground — the sheet's one accent) revealing per-row Rename (text modal) / Delete, where Rename and Delete are each ≥48dp targets separated by a clear `step3` gap and Delete routes through `showConfirmationModal`. Deleting the *currently-active* saved filter falls back to the default "All" view (`SavedTaskFilterActivator.clearToDefault()`) so the live filter is never left on an orphaned shape; "All tasks" itself is non-deletable. A bottom "Save current filter as…" create row closes the sheet. Tapping a row applies + closes.
