...
- **Priority section bands.** A priority-sorted section header sits in a filled, priority-tinted band whose fill is *lightly graduated by rank* (`bandAlpha`: P0 `0.13` → P3 `0.06`). The bands stay deliberately calm — they only *label* their group; the urgency *anchor* is the single lead-card rail plus the priority glyph and the due chips, so even the urgent band does not need a loud "alert" fill. The label reads as the plain priority word (`Urgent` / `High` / `Medium` / `Low`) — the opaque `P{n}` code reads as developer shorthand to everyday users, and the glyph plus band colour already encode severity; the `P{n}` handle still lives in the filter chips and AI context. Date-sorted sections get no band. The band's horizontal padding (`step4`) matches the card content inset so the header text shares the cards' left edge; the band has a generous `step6` gap above (separating groups) and a tight `step3` below (binding it to its first card).
