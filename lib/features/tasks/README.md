...
- **Priority section bands.** A priority-sorted section header sits in a filled, priority-tinted band whose fill is *steeply graduated by rank* (`bandAlpha`: P0 `0.26` → P1 `0.15` → P2 `0.10` → P3 `0.06`) so the urgent (red) band visibly outweighs the high (orange) band — orange is intrinsically brighter, so red needs the markedly higher alpha to read as the stronger of the two. The label itself uses `heading.heading3` (20px) at the heaviest weight — a clear step *larger* than the 16px card titles — so the group header out-ranks its members on the type ramp (header 20 > title 16 > body 14), not just via the band fill. The label reads as the plain priority word (`Urgent` / `High` / `Medium` / `Low`) — the opaque `P{n}` code reads as developer shorthand to everyday users, and the glyph plus band colour already encode severity; the `P{n}` handle still lives in the filter chips and AI context. Date-sorted sections get no band. The band's horizontal padding (`step4`) matches the card content inset so the header text shares the cards' left edge; the band has a generous `step6` gap above (separating groups) and a tight `step3` below (binding it to its first card).
- **Grouped container.** Cards in a section share one rounded container: a neutral `TaskShowcasePalette.containerBorder` (`decorative.level02`) outer hairline lifts the group off the darker page (`background.level01`), while within-group dividers are inset to the content's left edge so the outer border — not the dividers — defines the group. Each divider sits in a `step5`-tall box (line centred) so the card-to-card gap decisively beats the intra-card gaps and the rows read as discrete cards rather than one run-on block — without un-merging the container.
- **Single "do this first" anchor.** Only the first card of the top bucket after the urgency sort (`isGlobalLead`) is marked, with a full-height priority-coloured rail down its leading edge, so the list has exactly one unambiguous starting point rather than one competing anchor per bucket. The rail alone carries the anchor — an earlier whole-card colour wash was dropped because it read as anxiety-inducing "alarm" tint. The rail is rendered inside the row shell and clipped to the card's rounded rectangle (`_withLeadRail`): a 6px-wide rail cannot carry the card's 16px corner radius itself without rounding off into a blob, so the card corner does the clipping and the rail stays flush.
- **Card body.** A bold `subtitle1` (16px) title, a `mediumEmphasis` AI one-liner shown across up to two lines (`maxLines: 2` — enough to read the task's current status without letting a long summary balloon the card height), then one unified metadata line. That line folds tracked time, the category chip, and the former footer (due date, created date, vector distance) into one `Wrap`, so each card has a single meta row instead of two stacked lines. The metadata chips share a 24px height with 14px text and a `decorative.level02` hairline so each chip's boundary is legible against the near-same-tone surface for low-vision users.
- **Conditional chips.** `TaskRowContent` drops the category chip when every loaded task shares one category — `showCategoryChip` is derived in `TasksTabPage` from the distinct `categoryId`s of the loaded items, so it holds however the list was scoped (a category filter, a project filter, or a saved view), not only an explicit category selection. The category chip itself is a calm neutral surface pill matching the other metadata chips, carrying the category colour only in a small leading icon rather than a fully saturated fill. The trailing status pill follows the *same* loaded-items rule — `showStatus` is true only when 2+ distinct status kinds are visible — so a list where every task is "In Progress" drops the repeated, zero-signal pill. Tracked time is suppressed when it is zero, and the metadata row collapses entirely when it would be empty.
- **Deadline emphasis.** `DueDateText` renders every due date as a calm surface chip the same height as the other metadata chips; urgency is carried only by the text/icon *colour* (overdue red, due-today amber, upcoming neutral) and a *weight ladder*, not a filled pill. The overdue/today colours are the lighter on-surface `taskIconColorRed` (#E57373) / `taskIconColorOrange` tints rather than fully saturated `Colors.red`/`orange`: the saturated red measured ~3.9:1 as text on the dark card (below WCAG AA) and read as an alarm, while the lighter red clears AA (~5.4:1) and reads as a calmer "attention" cue that no longer out-shouts the lead-card rail. The ladder makes overdue the loudest state (`w700`), then due-today (`w600`), then a quiet upcoming date (`w500`) — triage needs "what's late?" to out-rank "what's due today", and amber (today) is intrinsically brighter than red (overdue), so overdue earns the extra weight. The chip defaults to a **relative** phrasing whenever it triages faster: *overdue* dates are always relative ("Overdue by N days") however long ago — a bare absolute date disguises the most urgent item as a far-off one and drops the redundant text cue colour-blind users rely on — and *future* dates are relative within two weeks (`_relativeWindowDays` = 14: "Due Today", "Due in 10 days") and the **absolute** date beyond it, where an exact date aids planning. The two-week window keeps the whole near-term horizon in one relative register so a "Due in 10 days" chip never sits next to a bare date the eye has to do math on. Because overdue days are negative they always satisfy `days <= _relativeWindowDays`, so a single threshold covers both; tapping flips to the other representation. Upcoming (non-urgent) dates use the same medium-emphasis grey as the other chips so they stay legible rather than fading to the faintest text on the card. Tracked time renders as the same bordered surface chip as the rest of the metadata row, and numeric meta (tracked time, section counts) uses tabular figures.
