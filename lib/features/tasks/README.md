...
- **Single "do this first" anchor.** Only the first card of the top bucket after the urgency sort (`isGlobalLead`) is marked, with a full-height priority-coloured rail down its leading edge plus a faint whole-card priority tint, so the list has exactly one unambiguous starting point rather than one competing anchor per bucket. The rail is rendered inside the row shell and clipped to the card's rounded rectangle (`_withLeadRail`): a 6px-wide rail cannot carry the card's 16px corner radius itself without rounding off into a blob, so the card corner does the clipping and the rail stays flush.
- **Card body.** A bold `subtitle2` title, a `mediumEmphasis` AI one-liner shown *in full* (it wraps when the model emits more than a line, since the summary carries the task's current status), then one unified metadata line. That line folds tracked time, the category chip, and the former footer (due date, created date, vector distance) into one `Wrap`, so each card has a single meta row instead of two stacked lines.
...
- **Deadline emphasis.** `DueDateText` renders every due date as a calm surface chip the same height as the other metadata chips; urgency is carried only by the text/icon *colour* (overdue red, due-today amber, upcoming neutral) and a *weight ladder*, not a filled pill. The ladder makes overdue the loudest state (`w700`), then due-today (`w600`), then a quiet upcoming date (`w500`) — triage needs "what's late?" to out-rank "what's due today", and amber (today) is intrinsically brighter than red (overdue), so overdue earns the extra weight. The deadline stays scannable without reading as an angry red block, and a dated card is still spottable among undated ones. Numeric meta (tracked time, section counts) uses tabular figures.
