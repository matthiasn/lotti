...
- **Deadline emphasis.** `DueDateText` renders overdue / due-today dates (`DueDateStatus.isUrgent`) as a filled, bordered pill so the one time-critical card in a group is spottable at a glance and is distinguished from the priority band by *form*, not just hue; future dates stay quiet neutral text. Numeric meta (tracked time, section counts) uses tabular figures.
