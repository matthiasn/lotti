...
- **Finds the right tasks fast.** Filter by status, priority, label, category or
  ...
  On phones the search-and-filter header folds into a slim bar while scrolling
  down the list and returns on the first scroll up, so small screens show tasks
  rather than chrome.
