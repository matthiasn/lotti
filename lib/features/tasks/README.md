...
- **Priority section bands.** A priority-sorted section header sits in a filled, priority-tinted band whose fill is *steeply graduated by rank* (`bandAlpha`: P0 `0.26` → P1 `0.15` → P2 `0.10` → P3 `0.06`) so the urgent (red) band visibly outweighs the high (orange) band — orange is intrinsically brighter, so red needs the markedly higher alpha to read as the stronger of the two. The label itself uses `subtitle.subtitle1` (16px) at the heaviest weight — a step *larger* than the 14px card titles — so the group header out-ranks its members rather than tying with them. The label reads as the plain priority word (`Urgent` / `High` / `Medium` / `Low`) — the opaque `P{n}` code reads as developer shorthand to everyday users, and the glyph plus band colour already encode severity; the `P{n}` handle still lives in the filter chips and AI context. Date-sorted sections get no band. The band's horizontal padding (`step4`) matches the card content inset so the header text shares the cards' left edge; the band has a generous `step6` gap above (separating groups) and a tight `step3` below (binding it to its first card).
