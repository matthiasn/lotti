...
/// view), the active saved pill (or a "Custom" pill carrying the live filtered
...
/// normal rail never scrolls. The disclosure glyph lives ONLY on the "Views"
...
/// horizontally scrolling run — [active anchor] [All] [Views] [+Save?] — sharing
...
/// is the most common escape hatch, and when "All" *is* the active selection it
...
class SavedTaskFilterRail extends ConsumerWidget {
  ...
  /// after reserving the fixed head (Views button + All pill + optional Save
  ...
  int _fitMruCount({
    ...
    // The Views button carries the shared count slot + disclosure glyph, so
...
/// The band-leading "Views" button — the rail's single explicit sheet opener,
...
/// tabular, min-width-growable) so it reads "Views  6" consistent with
...
class _SavedButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // the glyphs: at medium emphasis the bookmark + glyph read gray-on-gray
