...
class AudioProcessor:
    ...
    async def process_audio_base64(
        ...
        try:
            ...
            if size_mb > self.max_size_mb:
                raise ValueError(f"Audio file too large: {size_mb:.1f}MB " f"(max: {self.max_size_mb}MB)")
            ...
            if duration > self.max_duration:
                raise ValueError(f"Audio too long: {duration:.1f}s " f"(max: {self.max_duration}s)")
    ...
    def _load_from_temp_file(self, audio_bytes: bytes) -> Tuple[NDArray[np.float32], int]:
        ...
        try:
            ...
            # Prefer ffmpeg decode for formats librosa/soundfile
            # struggle with (e.g., m4a)
            try:
                ...
                logger.warning(f"ffmpeg decoding failed, falling back to librosa. " f"Error: {e}")
            ...
            raise ValueError(f"Unable to load audio from bytes. " f"Temp file method failed: {e}")
        finally:
            # Clean up temp file
    ...
    def _resample_audio(self, audio_array: NDArray[np.float32], original_sr: int) -> Tuple[NDArray[np.float32], float]:
        ...
        try:
            ...
            resampled = librosa.resample(audio_array, orig_sr=original_sr, target_sr=self.sample_rate)
        return resampled, (time.perf_counter() - t_rs0)

    def prepare_for_model(self, audio_array: NDArray[np.float32], processor=None) -> Union[torch.Tensor, dict]:
        """
        ...
        if processor and hasattr(processor, "feature_extractor"):
            ...
            inputs = processor(audio=audio_array, sampling_rate=self.sample_rate, return_tensors="pt")
            return inputs
        else:
            # Convert to tensor
    ...
    def chunk_audio_optimized(
        ...
        # Max 30s chunks for Gemma 3N
        chunk_duration = min(chunk_duration, 30)
        ...
        # Return original if no valid chunks
        return chunks if chunks else [audio_array]

    # Legacy method for backwards compatibility
    def chunk_audio(
        self, audio_array: NDArray[np.float32], chunk_duration: float = 30.0, overlap: float = 2.0
    ) -> List[NDArray[np.float32]]:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_cached_duration": sum(entry["duration"] for entry in self._feature_cache.values()),
        }
