...
class AudioProcessor:
    ...
    async def process_audio_base64(
        ...
        try:
            ...
            # Decode base64
            logger.info(f"Decoding base64 audio data: {len(audio_base64)} characters")
            logger.info(f"Base64 prefix: {audio_base64[:50]}...")
            ...
            logger.info(f"Audio size OK: {size_mb:.2f}MB")
            ...
            if use_chunking and duration > self.chunk_duration:
                logger.info(f"Chunking audio into {self.chunk_duration}s segments")
    ...
    def _load_audio_from_bytes(self, audio_bytes: bytes) -> Tuple[np.ndarray, int]:
        ...
        """
        logger.info(f"Loading audio from {len(audio_bytes)} bytes")
        ...
        try:
            ...
            logger.info(f"Soundfile success: shape={audio_array.shape}, sr={sample_rate}")
            if len(audio_array.shape) > 1:
                # Convert to mono
                audio_array = np.mean(audio_array, axis=1)
                logger.info(f"Converted to mono: shape={audio_array.shape}")
            return audio_array, sample_rate
        except Exception as e:
            logger.info(f"Soundfile failed: {type(e).__name__}: {e}")
        ...
        try:
            ...
            logger.info(f"Torchaudio success: shape={waveform.shape}, sr={sample_rate}")
            # Convert to numpy and mono
            audio_array = waveform.mean(dim=0).numpy()
            logger.info(f"Converted to numpy mono: shape={audio_array.shape}")
            return audio_array, sample_rate
        except Exception as e:
            logger.info(f"Torchaudio failed: {type(e).__name__}: {e}")
    ...
    def _load_from_temp_file(self, audio_bytes: bytes) -> Tuple[np.ndarray, int]:
        ...
        try:
            ...
            logger.info(f"Librosa temp file success: shape={audio_array.shape}, sr={sample_rate}")
            return audio_array, sample_rate
    ...
    def chunk_audio_optimized(
        ...
        logger.info(f"Splitting {total_duration:.1f}s audio into {chunk_duration}s chunks with {overlap}s overlap")
        ...
        logger.info(f"Created {len(chunks)} audio chunks for processing")
    ...
    def get_cached_features(self, audio_array: np.ndarray) -> Optional[Any]:
        ...
        if cache_entry:
            logger.debug(f"Cache hit for audio hash {audio_hash}")
