...
async def test_model_download():
    ...
    async with httpx.AsyncClient(timeout=600.0) as client:
        request_data = {"model_name": "gemma-3n-E2B-it", "stream": True}

        print("Starting model download...")

        async with client.stream("POST", "http://localhost:11343/v1/models/pull", json=request_data) as response:
            async for chunk in response.aiter_lines():
                if chunk.startswith("data: "):
                    ...
                    try:
                        ...
                        if "progress" in data:
                            print(
                                f"\rProgress: {data['progress']:.1f}% - {data['status']}",
                                end="",
                                flush=True,
                            )
        ...
        response = await client.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": "Context: This is a test audio file containing a tone.\n\nTranscribe this audio",
                    }
                ],
                "audio": audio_base64,
                "temperature": 0.1,
...
async def test_streaming_chat():
    ...
    async with httpx.AsyncClient() as client:
        ...
        async with client.stream("POST", "http://localhost:11343/v1/chat/completions", json=request_data) as response:
            async for chunk in response.aiter_lines():
                if chunk.startswith("data: "):
                    data_str = chunk[6:]  # Remove "data: " prefix
                    if data_str == "[DONE]":
