...
async def chat_completion(request: ChatRequest) -> Union[Dict[str, Any], StreamingResponse]:
    """
...
async def pull_model(request: Dict[str, Any]) -> Union[StreamingResponse, Dict[str, Any]]:
    ...
    async def generate() -> Any:
        ...
        try:
            ...
            error_event = {"status": STATUS_ERROR, "error": "An internal error occurred."}
    ...
    if request.get("stream", True):
        ...
        try:
            ...
            async for progress in model_downloader.download_model(model_name, False):
                if progress.status.value == "complete":
                    result = {"status": STATUS_SUCCESS, "message": progress.message}
                elif progress.status.value == "error":
                    raise HTTPException(status_code=500, detail=progress.error)

            return result or {"status": STATUS_SUCCESS, "message": "Download completed"}
