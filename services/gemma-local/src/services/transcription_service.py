...
from ..core.interfaces import (
    ITranscriptionService, IModelManager, IAudioProcessor, IModelValidator
)
...
class TranscriptionService(ITranscriptionService):
    ...
    async def transcribe_audio(self, request: AudioRequest) -> TranscriptionResult:
        ...
        try:
            ...
            if len(audio_chunks) == 1:
                ...
                transcription = await self._generate_single_transcription(
                    messages, audio_chunks[0], request_id
                )
            else:
                logger.info(f"[REQ {request_id}] {len(audio_chunks)} chunks; starting sequential generation")
                transcription = await self._generate_chunked_transcription(
                    audio_chunks, messages, request_id
                )
    ...
    async def _generate_single_transcription(
        self,
        messages: list,
        audio_array,
        request_id: str
