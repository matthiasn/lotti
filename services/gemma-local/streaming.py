...
class StreamGenerator:
    ...
    async def generate_stream(
        self,
        prompt: str,
        audio_array: Optional[Union[NDArray[np.float32], List[NDArray[np.float32]]]] = None,
    ...
    async def generate_chat_stream(
        self, prompt: str, temperature: float = 0.7, max_tokens: int = 1000, top_p: float = 0.95
    ) -> AsyncGenerator[str, None]:
        """
        ...
        try:
            ...
            # Tokenize input
            inputs = self.model_manager.tokenizer(prompt, return_tensors="pt", truncation=True, max_length=2048).to(
            ...
            try:
                ...
                # Ensure thread completes and clean up resources
                try:
                    thread.join(timeout=5.0)  # Add timeout to prevent infinite wait
                    if thread.is_alive():
                        logger.warning("Generation thread still alive after timeout")
                except Exception as e:
                    logger.warning(f"Error joining generation thread: {e}")

                # Explicitly clean up streamer resources
                try:
                    if hasattr(streamer, "end"):
                        streamer.end()
                    del streamer  # Help GC
                except Exception as e:
                    logger.debug(f"Error cleaning up streamer: {e}")
