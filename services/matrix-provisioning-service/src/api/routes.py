...
async def verify_subscription_purchase(
    ...
    try:
        ...
        try:
            delivery = await paid_bundle_service.provision_or_deliver(
                verified,
                submission,
                now=enforcement_now,
            )
        ...
        current = await paid_bundle_service.require_returnable_delivery(
            delivery,
...
async def deliver_paid_bundle(
    ...
    try:
        ...
        subscription = await paid_bundle_service.require_returnable_delivery(
            delivery,
            entitlement_id=request.entitlement_id,
            now=datetime.now(timezone.utc),
        )
