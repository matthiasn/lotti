...
def test_claim_reaper_schedule_and_entitlement_quota_are_configurable(env):
    ...
    env.setenv("BUNDLE_CLAIM_ATTEMPT_LIMIT", "3")
    env.setenv("BUNDLE_CLAIM_ATTEMPT_WINDOW_SECONDS", "240")
    container = Container()
    ...
    assert identity._bundle_claim_attempt_limit == 3
    assert identity._bundle_claim_attempt_window == timedelta(minutes=4)
