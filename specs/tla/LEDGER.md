...
| [#4510](https://github.com/matthiasn/lotti/pull/4510) | Open | An unusable uploaded descriptor must not be referenced or acknowledged | Exact server read-back validates descriptor metadata before publishing. Regression controls cover malformed descriptors, MIME-derived media types and incomplete encryption metadata. Revision `2e605fab04` passed all seven scenarios in both normal and degraded-network integration jobs; the encryption-metadata follow-up still awaits its own complete CI result. The underlying empty-payload cause and attachment-byte verification remain outside this guard |
