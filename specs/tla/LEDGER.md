...
| [#4510](https://github.com/matthiasn/lotti/pull/4510) | Open | An unusable uploaded descriptor must not be referenced or acknowledged | Exact server read-back validates descriptor metadata before publishing. Regression controls cover malformed descriptors and MIME-derived media types. A completed initial-sync scenario recovered via retry in a superseded run; full corrected-build integration remains pending. SDK corruption causes and attachment-byte verification remain outside this guard |
