...
properties and 28,420 distinct states to the configurations present at
`8428b0a479`. These are incremental figures; the historical snapshot above
...
| [#4490](https://github.com/matthiasn/lotti/pull/4490) | pending | sync | `OutboxCausality` | 1 | 1 (0) | P1 | — | A concurrent inline version replaced another pending payload and advertised its clock as covered, preventing repair. The code audit found it; TLC reproduces it in four transitions. Also restores TLC triggers for startup and profile teardown |
