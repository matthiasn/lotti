...
(* agent link), so the code must match both — collapseKeyOf qualifies the *)
(* id with the family and _collapseEntity keeps only same-key rows. With   *)
(* the bare id as its key, the collapse would fold one object into another *)
(* object's send; that is Collapsible here matching on `key` alone.        *)
(*                                                                         *)
