...
|---------------|--------|---------------|-----------------|-----------------|
| `InboundQueue` | 3, one on the server at the first start | 1 | failed enqueue, incomplete walk, worker throw; one retry, two resurrection passes, a gap-recovery walk | 15,055,522 |
| `InboundQueueCipher` | 3, event 2 encrypted until its key arrives | 1 | failed resume-floor write, failed enqueue | 1,607,563 |
| `InboundQueueCrash` | 4 | 2 | incomplete walk | 2,952,243 |
...
`last_applied_ts` (`BridgeMarker.claimFloorTs`): the claim alone keeps the
...
limited-sync gaps, a late key, forward and backward walks that step, fail and
complete, worker commits, retries and skips, and crashes (a fresh queue over
...
walk's unresolved ciphertext in a checkpoint, or claiming two milliseconds
above the marker fails it; so does leaving out the gap claim in the driver.
