...
(***************************************************************************)
(* Local theme/name/flag commits, coalescing or immediate publication and    *)
(* remote register application. Each peer makes two edits while its clock    *)
...
CONSTANTS Peers, MaxEdits, MonotoneLocalStamps, PublishCommittedSnapshot,
          CoalesceLocalEdits, RepeatedPayloads
ASSUME Peers = 1..Cardinality(Peers) /\ MaxEdits \in Nat \ {0}
MaxStamp == Cardinality(Peers) * MaxEdits
Payloads == 1..(IF RepeatedPayloads THEN 2 ELSE MaxStamp)
...
LocalEdit(p) ==
    ...
    /\ LET stamp == IF MonotoneLocalStamps THEN stored[p][1] + 1 ELSE 1
           payload == IF RepeatedPayloads THEN 1 + ((p + edits[p]) % 2)
                      ELSE (p - 1) * MaxEdits + edits[p] + 1
           version == <<stamp, payload>>
       IN /\ stored' = [stored EXCEPT ![p] = version]
          /\ pending' = [pending EXCEPT ![p] =
              IF CoalesceLocalEdits THEN {version} ELSE @ \cup {version}]
...
Publish(p, v) ==
    /\ v \in pending[p]
    /\ LET version == IF PublishCommittedSnapshot THEN v
                      ELSE <<v[1], stored[p][2]>>
       IN room' = room \cup {version}
    /\ pending' = [pending EXCEPT ![p] = @ \ {v}]
...
Next == (\E p \in Peers : LocalEdit(p))
        \/ (\E p \in Peers, v \in Versions : Publish(p, v))
        \/ (\E p \in Peers, v \in Versions : Receive(p, v))
Spec == Init /\ [][Next]_vars
        /\ (\A p \in Peers : WF_vars(LocalEdit(p)))
        /\ (\A p \in Peers, v \in Versions : WF_vars(Publish(p, v)))
        /\ (\A p \in Peers, v \in Versions : WF_vars(Receive(p, v)))
TypeOK == /\ stored \in [Peers -> Versions \cup {Empty}]
          /\ pending \in [Peers -> SUBSET Versions]
...
Settled == \A p \in Peers :
    edits[p] = MaxEdits /\ pending[p] = {} /\ room \subseteq delivered[p]
