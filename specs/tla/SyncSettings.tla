...
(*                                                                         *)
(* Start/Write/Commit reflect guard, individual SettingsDb writes, then   *)
(* the stamp and successful return. No transaction surrounds these writes*)
(* in SettingsDb. Fail represents a swallowed save error: the caller sees*)
(* successful completion despite a partially written group.              *)
(*                                                                         *)
(* Green configurations have unique stamps and successful writes, or a  *)
(* shared order for unstamped flags. EqualStamps, Unordered flags, and    *)
(* FailureBudget > 0 are documented residual counterexamples, not fixes.*)
...
VARIABLES rows, stamp, done, active, nextField, failures
vars == <<rows, stamp, done, active, nextField, failures>>
...
Start(p, v) ==
    ...
    /\ IF Timestamped /\ Stamp(v) < stamp[p]
       ...
       ELSE /\ active' = [active EXCEPT ![p] = v]
            /\ UNCHANGED done
    /\ UNCHANGED <<rows, stamp, nextField, failures>>
Write(p) ==
    /\ active[p] # 0 /\ nextField[p] <= FieldCount
    /\ rows' = [rows EXCEPT ![p][nextField[p]] = active[p]]
