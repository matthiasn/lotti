...
- Use `fvm` for every Flutter command — `.fvmrc` pins the version CI uses (currently 3.44.8). Running with a different local SDK can make a test pass locally and fail in CI (or vice versa).
...
- **Unit & Widget Tests** — ten shards of `very_good test --no-optimization --coverage --exclude-tags glados -- --total-shards=10 --shard-index=N` (fast feedback, all non-property tests). The optimized bundle is deliberately disabled because Very Good CLI does not yet support sharding it across independent runners; each runner otherwise generates the bundle from filesystem order and can execute a different slice.
...
```bash
very_good test --no-optimization --coverage --exclude-tags glados -- --total-shards=10 --shard-index=0
```

Replace `0` with any shard index from `1` through `9` for the other shards. The `--` terminator forwards the sharding arguments to the underlying `flutter test` command. The standard CI lane disables Very Good's optimizer so Flutter shards deterministic test files rather than ten independently generated optimized bundles. The Make test-suite targets (`test`, `test_standard`, `test_glados`, and their `coverage_*` variants) continue to use optimized `very_good test` runs when they are not sharded. Run `make activate_very_good` once on a fresh checkout.
...
6. **Exception**: tests tagged `eval-live` (`test/features/ai/eval/`, `test/features/daily_os_next/eval/`) call a real inference provider and are skipped unless explicitly enabled via their `LOTTI_*_LIVE=1` env gate. They use the real clock deliberately — e.g. `day_planning_eval_live_test.dart` runs its same-day scenarios under a clock anchored to their start hour so the same-day "blocks must not start before current time" persist guard interacts with a real model's proposed times, which is the behavior under eval (see item 7 for why that clock moves rather than being frozen).
