...
void main() {
  ...
  group('SyncDatabase Migration Tests', () {
    ...
    test(
      'v28 migration adds the nullable queue marker resume floor without '
      'disturbing the applied marker',
      () async {
        final dbFile = File(
          path.join(testDirectory!.path, 'test_sync_v28_resume_floor.db'),
        );
        final sqlite = sqlite3.open(dbFile.path);
        ...
        expect(
          ...
          contains('resume_floor_ts'),
        );

        final marker = await (db.select(
          db.queueMarkers,
        )..where((t) => t.roomId.equals('!room:example.org'))).getSingle();
        expect(marker.lastAppliedEventId, r'$anchor');
        expect(marker.lastAppliedTs, 5000);
        expect(marker.lastAppliedCommitSeq, 7);
        expect(marker.resumeFloorTs, isNull);

        await (db.update(
          db.queueMarkers,
        )..where((t) => t.roomId.equals('!room:example.org'))).write(
          const QueueMarkersCompanion(resumeFloorTs: Value(3000)),
        );
        final updated = await (db.select(
          db.queueMarkers,
        )..where((t) => t.roomId.equals('!room:example.org'))).getSingle();
        expect(updated.resumeFloorTs, 3000);
  ...
  // Read-pool behaviour for the SyncDatabase constructor in
  // lib/database/sync_db.dart — kept in this file because it is that source
  // file's mirror, rather than adding a second test file for one source.
  //
  // These must be file-backed with `background: true`: the read pool only
  // takes effect for NativeDatabase.createInBackground, and
  // `inMemoryDatabase: true` bypasses it entirely (see openDbConnection). An
  // in-memory test here would pass without exercising a pool at all.
  group('SyncDatabase read pool', () {
    ...
    test(
      'sequential write/read pairs never observe a stale snapshot',
      () async {
    ...
    test(
      'a read outside a transaction completes while a writer holds the lock',
      () async {
        // THE regression test for `readPool: 2`. Everything else in this group
        // asserts committed-read semantics that hold at readPool: 0 too, so only
        // this one fails if the pool is removed or misrouted.
        //
        // A write transaction is held open on a Completer. With a read pool the
        // outside read is served by a read isolate and completes immediately
        // against the last committed snapshot. Without one it queues behind the
        // write executor and cannot finish until the transaction releases.
        await db
            .into(db.outbox)
            .insert(
              buildOutboxCompanion(
                status: OutboxStatus.pending,
                createdAt: DateTime(2026, 3, 15),
                subject: 'committed-before',
              ),
            );

        final releaseWriter = Completer<void>();
        final writerHoldsLock = Completer<void>();
        ...
        expect(
          ...
          1,
          reason:
              'the read must be served by a pool isolate against the committed '
              'snapshot while the writer still holds the write executor; with '
              'readPool: 0 it queues behind the transaction instead',
        );

        releaseWriter.complete();
        await writing;

        expect(await db.outbox.count().getSingle(), 2);
      },
    );

    test(
      'a rolled-back transaction leaves nothing visible to readers',
      () async {
        await expectLater(
...
/// Seeds the `queue_markers` table as of v27, before `resume_floor_ts`.
