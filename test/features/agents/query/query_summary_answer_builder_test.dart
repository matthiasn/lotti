...
void main() {
  ...
  group('category scope', () {
    ...
    const brief = 'Breathing drills for the penguin colony, run at dawn.';
    ...
    test(
      'both calls see the category name and brief, not a bare identifier',
      () async {
        await build(queryScope: categoryScope);
        ...
        // Both prompts are told they are in a category, not a task.
        expect(systems, everyElement(contains('homeScope is a category')));
        expect(systems.last, contains('category.knowledgeBrief'));
      },
    );

    test('a task question carries no category framing', () async {
      await build();

      expect(prompts, hasLength(2));
      expect(prompts, everyElement(isNot(contains('category'))));
      expect(systems, everyElement(isNot(contains('homeScope is a category'))));
    });

    test('an answer resting only on the brief is resolved', () async {
      response = {
        'answer': 'This category covers breathing drills, run at dawn.',
        'ownerIds': <String>[],
        'unresolved': false,
      };

      final answer = await build(queryScope: categoryScope);

      expect(answer!.text, response['answer']);
      expect(answer.summaryOwnerIds, isEmpty);
      expect(answer.coverage.incomplete, isFalse);
    });

    test(
      'without a brief an unattributed resolved answer is still rejected',
      () async {
        bench.categories[0] = categoryMindfulness.copyWith(
          knowledgeBrief: '   ',
        );
        response = {
          'answer': 'This category covers breathing drills.',
          'ownerIds': <String>[],
          'unresolved': false,
        };

        await expectLater(
          build(queryScope: categoryScope),
          throwsFormatException,
        );
        // A blank brief is absent, never an empty field in the prompt.
        expect(prompts.first['category'], {'name': 'Mindfulness'});
      },
    );
  });
}
