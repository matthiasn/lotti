...
void main() {
  ...
  group('TaskAgentWorkflow', () {
    ...
    group('tool handler dispatch with real Task', () {
      ...
      test('update_task_estimate with non-int minutes returns error', () async {
        final result = await executeWithToolCallOnRealTask(
        ...
        verify(
          () => mockConversationManager.addToolResponse(
            ...
            response: any(
              named: 'response',
              that: contains('must be an integer'),
            ),
          ),
        ).called(1);
      });
      ...
      test('update_checklist_items with non-array updates returns error',
          () async {
        final result = await executeWithToolCallOnRealTask(
