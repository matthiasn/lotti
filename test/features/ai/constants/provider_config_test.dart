...
void main() {
  group('ProviderConfig', () {
    ...
    group('isLocalProvider', () {
      test('should return true for local providers', () {
        expect(
          ProviderConfig.isLocalProvider(InferenceProviderType.genericOpenAi),
          isTrue,
        );
        expect(
          ProviderConfig.isLocalProvider(InferenceProviderType.ollama),
          isTrue,
        );
        expect(
          ProviderConfig.isLocalProvider(InferenceProviderType.gemma3n),
          isTrue,
        );
        expect(
          ProviderConfig.isLocalProvider(InferenceProviderType.whisper),
          isTrue,
        );
      });

      test('should return false for cloud providers', () {
        expect(
          ProviderConfig.isLocalProvider(InferenceProviderType.gemini),
          isFalse,
        );
        expect(
          ProviderConfig.isLocalProvider(InferenceProviderType.openAi),
          isFalse,
        );
        expect(
          ProviderConfig.isLocalProvider(InferenceProviderType.anthropic),
          isFalse,
        );
      });
    });

    group('getSupportedProviders', () {
      test('should return all provider types', () {
        final providers = ProviderConfig.getSupportedProviders();

        expect(providers, contains(InferenceProviderType.gemini));
        expect(providers, contains(InferenceProviderType.gemma3n));
        expect(providers, contains(InferenceProviderType.genericOpenAi));
        expect(providers, contains(InferenceProviderType.nebiusAiStudio));
        expect(providers, contains(InferenceProviderType.ollama));
        expect(providers, contains(InferenceProviderType.openAi));
        expect(providers, contains(InferenceProviderType.anthropic));
        expect(providers, contains(InferenceProviderType.openRouter));
        expect(providers, contains(InferenceProviderType.whisper));
      });

      test('should return a list', () {
        final providers = ProviderConfig.getSupportedProviders();
        expect(providers, isA<List<InferenceProviderType>>());
      });
    });
  });
}
