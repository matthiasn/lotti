...
export 'eval_text_matchers.dart' show containsAffirmativeReportClaim;
...
class LocalTaskAgentEvalCaseResult {
  ...
  int get passedQualityCheckCount {
    ...
    var passed = scenario.requiredReportTermGroups
        .where((group) => containsAnyEvalTerm(normalizedReport, group))
        .length;
    ...
    passed += scenario.forbiddenReportClaims
        .where(
          (claim) => !containsAffirmativeReportClaim(normalizedReport, claim),
        )
        .length;
    ...
    for (final entry in scenario.requiredToolArgumentTermGroups.entries) {
      final arguments = toolCalls
          ...
          .toLowerCase();
      passed += entry.value
          .where((group) => containsAnyEvalTerm(arguments, group))
          .length;
    }
