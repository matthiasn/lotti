...
- [x] **[HIGH]** `test/features/ai/functions/lotti_checklist_update_handler_test.dart` — **1 587 lines** for a 532-line impl. Natural split seams exist between: (1) `processFunctionCall` validation tests (lines ~100–600), (2) `executeUpdates` / sovereignty-guard tests (lines ~600–1400), (3) `createToolResponse` / `getRetryPrompt` contract tests (lines ~1400–1587). The 59 test cases and 3 lifecycle stages make this a prime candidate for an `_UpdateHandlerTestBench` class per the "complex state" guideline in `AGENTS.md`. **RESOLVED (adapted):** a file split would break the one-test-file-per-source rule, and the existing style is file-level helpers rather than a bench class, so the boilerplate was extracted into `stubItemFetch(ids, items)`, `stubTaskById()`, `stubUpdateSuccess(id)`, and a `makeUpdateResult(items)` builder; ~50 inline stub/builder blocks collapsed (file now 1 503 lines, all 65 tests pass).
...
- [x] **[HIGH]** `test/features/ai/functions/task_due_date_handler_test.dart` — the "no-op same date" branch (impl lines 222–240: `currentDue.dayAtMidnight == dueDate`) appears to have no dedicated test. The Glados scenario exercises it implicitly, but a named static test for this branch would document the no-write semantics clearly. **RESOLVED (already covered):** the group 'no-op when same due date' has a dedicated static test asserting success with `didWrite == false` for a matching current date.
...
- [x] **[LOW]** `test/features/ai/functions/function_handler_test.dart` — confirm that the base `FunctionCallResult` struct carries `success`, `functionName`, `arguments`, `data`, and `error` correctly.
  **RESOLVED (confirmed-true, no change):** the `FunctionCallResult` group (`stores all fields correctly`, `error defaults to null`, `success result has null error`, `failed result has error message`) asserts every field including the `data` map contents. The redundant handler-specific `wasSkipped` and `wasNoOp` getters have since been removed.
