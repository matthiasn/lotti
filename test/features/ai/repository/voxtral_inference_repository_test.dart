...
void main() {
  ...
  group('VoxtralInferenceRepository', () {
    group('transcribeAudio', () {
      ...
      test('forwards speech_dictionary terms when provided', () async {
        // Arrange
        final events = [
          createSseChunkEvent(content: 'Text'),
          createSseFinalEvent(),
        ...
        final stream = repository.transcribeAudio(
          ...
          audioBase64: audioBase64,
          baseUrl: baseUrl,
          speechDictionaryTerms: const ['Flutter', 'Riverpod', 'macOS'],
        );
        ...
        final requestBody = jsonDecode(request.body) as Map<String, dynamic>;
        expect(
          requestBody['speech_dictionary'],
          equals(['Flutter', 'Riverpod', 'macOS']),
        );
      });

      test(
        'omits speech_dictionary field when terms are null or empty so older '
        'server versions without the field continue to work',
        () async {
          final events = [
            createSseChunkEvent(content: 'Text'),
            createSseFinalEvent(),
          ...
          var requestBody =
              jsonDecode((captured.first as http.Request).body)
                  as Map<String, dynamic>;
          expect(requestBody.containsKey('speech_dictionary'), isFalse);
          ...
          await repository
              .transcribeAudio(
                ...
                audioBase64: audioBase64,
                baseUrl: baseUrl,
                speechDictionaryTerms: const <String>[],
              )
              .toList();
          captured = verify(
            () => mockHttpClient.send(captureAny()),
