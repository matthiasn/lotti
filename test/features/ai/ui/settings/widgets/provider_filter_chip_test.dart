...
void main() {
  group('Provider Filter Chip Color Tests', () {
    testWidgets('Anthropic provider has bronze color in light theme',
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            aiConfigByIdProvider('provider1').overrideWith((ref) async => provider),
          ],
          child: MaterialApp(
    ...
    testWidgets('OpenAI provider has green color in dark theme',
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            aiConfigByIdProvider('provider2').overrideWith((ref) async => provider),
          ],
          child: MaterialApp(
    ...
    testWidgets('Gemini provider has blue color', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            aiConfigByIdProvider('provider3').overrideWith((ref) async => provider),
          ],
          child: MaterialApp(
            home: Scaffold(
              body: ProviderFilterChip(
    ...
    testWidgets('Ollama provider has orange color', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            aiConfigByIdProvider('provider4').overrideWith((ref) async => provider),
          ],
          child: MaterialApp(
            home: Scaffold(
              body: ProviderFilterChip(
  ...
  group('Provider Filter Chip Data Tests', () {
    test('Null provider returns null from async state', () async {
      ...
      final result = await container.read(aiConfigByIdProvider('provider1').future);
    ...
    test('Valid provider returns correct data', () async {
      ...
      if (result is AiConfigInferenceProvider) {
        expect(result.inferenceProviderType, equals(InferenceProviderType.anthropic));
      }
