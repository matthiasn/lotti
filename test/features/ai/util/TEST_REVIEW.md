...
- [x] **[MED]** `test/features/ai/util/mlx_audio_channel_test.dart` (1077 lines) vs impl (552 lines). The test itself is fine in quality, but the `MlxAudioModelProgressStore` Riverpod Notifier lives in `mlx_audio_channel.dart`; the combined channel + progress store is 552 lines. Splitting `MlxAudioModelProgressStore` into its own file (e.g. `mlx_audio_model_progress_store.dart`) with a matching test file would keep both under 400 lines each. **RESOLVED:** Done — `MlxAudioModelProgressStore` + its three providers moved to `mlx_audio_model_progress_store.dart` (147 lines; channel file down to 416), consumers re-imported, and the test mirror split accordingly (`mlx_audio_model_progress_store_test.dart` carries the store group + fake channel).
...
- [x] **[MED]** `test/features/ai/util/ai_error_utils_test.dart` line 204: the test `'test helpers expose dynamic fields'` only verifies that inline test helper class properties are readable. This is a constructor smoke test with zero value for the production code — it proves nothing about `AiErrorUtils`. Remove or collapse it into an actual extraction test. **RESOLVED:** Done — the smoke test was deleted (the helper classes remain — their fields are read dynamically by AiErrorUtils in the real extraction tests).
...
- [x] **[MED]** `lib/features/ai/util/known_models.dart` — `KnownModel.toAiConfigModel` uses `DateTime.now()` at line 58. Tests via the Glados property (`known_models_test.dart` line 440) only assert `isA<DateTime>()` for `createdAt`. There is no test that verifies the other generated fields in isolation (non-Glados path); the existing static test at line 366 covers this adequately though. **RESOLVED:** Stale — self-resolved: the item concludes the static test covers it adequately.

- [x] **[MED]** `lib/features/ai/util/profile_seeding_service.dart` — `_hasSlotForSkillType` (lines 122–130) is a private static method with five `SkillType` cases. The `upgradeExisting` tests exercise it indirectly but the `SkillType.imageGeneration` and `SkillType.promptGeneration` branches are not directly exercised by the profile seeding tests. Add targeted tests for the `upgradeExisting` path where a profile has `imageGenerationModelId` populated. **RESOLVED:** Done (adapted, premise correction) — no template assigns an image-generation skill, so the branch is unreachable through upgradeExisting regardless of profile slots. Added a `debugHasSlotForSkillType` seam + exhaustive SkillType x slot matrix test (empty vs fully-slotted profiles, per-slot gating, thinking-model-backed types always true).
