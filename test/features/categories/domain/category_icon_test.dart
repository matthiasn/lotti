  ...
  group('CategoryIcon', () {
    ...
    group('suggestFromName', () {
      ...
      test('should find exact enum name matches', () {
        expect(
          CategoryIconExtension.suggestFromName('fitness'),
          equals(CategoryIcon.fitness),
        );
        expect(
          CategoryIconExtension.suggestFromName('running'),
          equals(CategoryIcon.running),
        );
        expect(
          CategoryIconExtension.suggestFromName('yoga'),
      ...
      test('should find word-boundary matches in display names', () {
        expect(
          CategoryIconExtension.suggestFromName('Heart'),
          equals(CategoryIcon.heartHealth),
        );
        expect(
          CategoryIconExtension.suggestFromName('Computer'),
          equals(CategoryIcon.computer),
        );
        expect(
          CategoryIconExtension.suggestFromName('Health'),
          equals(CategoryIcon.heartHealth),
        );
      });
      ...
      test('should not match partial words incorrectly', () {
        ...
        expect(
          CategoryIconExtension.suggestFromName('art'),
        ...
        expect(
          CategoryIconExtension.suggestFromName('men'),
          isNot(equals(CategoryIcon.mentalHealth)),
        );
        expect(
          CategoryIconExtension.suggestFromName('car'),
          equals(CategoryIcon.car),
        );
        expect(
          CategoryIconExtension.suggestFromName('car'),
          isNot(equals(CategoryIcon.medical)),
        ); // "care" partial match
      });
      ...
      test('should match complete words and reasonable prefixes', () {
        ...
        expect(
          CategoryIconExtension.suggestFromName('mental'),
        ...
        expect(
          CategoryIconExtension.suggestFromName('men'),
          isNull,
        ); // Too short, should not match "mental"
        expect(
          CategoryIconExtension.suggestFromName('ment'),
          equals(CategoryIcon.mentalHealth),
        ); // Should match "mental" (4 chars, 67% of "mental")
        expect(
      ...
      test('should find keyword mappings', () {
        ...
        expect(
          CategoryIconExtension.suggestFromName('exercise'),
          equals(CategoryIcon.fitness),
        );
        expect(
          CategoryIconExtension.suggestFromName('doctor'),
          equals(CategoryIcon.medical),
        );
        expect(
          CategoryIconExtension.suggestFromName('book'),
      ...
      test(
        ...
        () {
          ...
          expect(
            CategoryIconExtension.suggestFromName('prune'),
            isNull,
          ); // Should NOT match "run" keyword
          expect(
            CategoryIconExtension.suggestFromName('brunette'),
            isNull,
          ); // Should NOT match "run" keyword
          expect(
            CategoryIconExtension.suggestFromName('trunk'),
            isNull,
          ); // Should NOT match "run" keyword
          expect(
            CategoryIconExtension.suggestFromName('overrun'),
            isNull,
          ); // Should NOT match "run" keyword

          // Test that exact word matches still work for "run" keyword
          expect(
            CategoryIconExtension.suggestFromName('run'),
            equals(CategoryIcon.running),
          ); // Should match "run" keyword
          expect(
            CategoryIconExtension.suggestFromName('I want to run'),
            equals(CategoryIcon.running),
          ); // Should match "run" as whole word
          expect(
            CategoryIconExtension.suggestFromName('run fast'),
            equals(CategoryIcon.running),
          ); // Should match "run" as whole word

          // Test other keywords don't have false matches using "book" keyword
          expect(
            CategoryIconExtension.suggestFromName('handbook'),
            isNull,
          ); // Should NOT match "book" keyword
          expect(
            CategoryIconExtension.suggestFromName('notebook'),
            isNull,
          ); // Should NOT match "book" keyword
          expect(
            CategoryIconExtension.suggestFromName('facebook'),
            isNull,
          ); // Should NOT match "book" keyword
          expect(
            CategoryIconExtension.suggestFromName('book'),
            equals(CategoryIcon.reading),
          ); // Should match "book" keyword
          expect(
            CategoryIconExtension.suggestFromName('read a book'),
            equals(CategoryIcon.reading),
          ); // Should match "book" as whole word

          // Test other keyword false matches using "gym" keyword
          expect(
            CategoryIconExtension.suggestFromName('gymnasium'),
            isNull,
          ); // Should NOT match "gym" keyword
          expect(
            CategoryIconExtension.suggestFromName('gymkhana'),
            isNull,
          ); // Should NOT match "gym" keyword
          expect(
            CategoryIconExtension.suggestFromName('gym'),
            equals(CategoryIcon.fitness),
          ); // Should match "gym" keyword
        },
      );
      ...
      test('should return null for unknown input', () {
        expect(
          CategoryIconExtension.suggestFromName('unknown_category'),
          isNull,
        );
      ...
      test('should prioritize exact matches over partial matches', () {
        ...
        expect(
          CategoryIconExtension.suggestFromName('work'),
      ...
      test('should prioritize exact display name matches over word matches', () {
        ...
        expect(
          CategoryIconExtension.suggestFromName('Music'),
