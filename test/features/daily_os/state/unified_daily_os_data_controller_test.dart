...
void main() {
  ...
  group('UnifiedDailyOsDataController - Zero Duration Entries', () {
    test('includes zero-duration entries in actualSlots', () async {
      final plan = createTestPlan(plannedBlocks: []);
      when(() => mockDayPlanRepository.getOrCreateDayPlan(testDate))
          .thenAnswer((_) async => plan);

      // Create a zero-duration entry (dateFrom == dateTo)
    ...
    test('includes both zero-duration and normal entries in actualSlots', () async {
      ...
      ).thenAnswer((_) async => [zeroDurationEntry, normalEntry]);

      final result = await container.read(
        unifiedDailyOsDataControllerProvider(date: testDate).future,
      );

      expect(result.timelineData.actualSlots.length, equals(2));

      final zeroSlot = result.timelineData.actualSlots
          .firstWhere((s) => s.entry.meta.id == 'zero-entry-1');
      expect(zeroSlot.duration, equals(Duration.zero));

      final normalSlot = result.timelineData.actualSlots
          .firstWhere((s) => s.entry.meta.id == 'normal-entry-1');
      expect(normalSlot.duration, equals(const Duration(hours: 1)));
    });
