...
void main() {
  ...
  group('Scroll Retry Logic', () {
    test('retry counter increments correctly', () {
      ...
      expect(attempt, equals(maxAttempts));
    });
    ...
    test('disposed flag stops retry loop', () {
      var disposed = false;
      var attempt = 0;
      const maxAttempts = 5;

      while (attempt < maxAttempts && !disposed) {
        attempt++;
        if (attempt == 3) {
          disposed = true;
        }
    ...
    test('retry logic handles early success', () {
      var attempt = 0;
      const maxAttempts = 5;
      var found = false;
