...
- [ ] **[HIGH]** `test/.../list_cards/task_card_test.dart` is **1331 lines** for a 319-line source file. The file covers: (a) basic status/priority rendering, (b) `showCreationDate` group, (c) `showDueDate` group (including date-format, completion-status hiding, toggle interaction), (d) `showCoverArt` group (cover art layout, labels in cover art layout, cropX). Each group is self-contained and large enough to stand alone. **Split seams:** `task_card_creation_date_test.dart`, `task_card_due_date_test.dart`, `task_card_cover_art_test.dart`, and a primary `task_card_test.dart` for status/priority/navigation. However, the one-test-file-per-source rule forbids multiple test files per source file — the right fix is to collapse redundant tests and shrink the single file to under ~700 lines.

- [ ] **[HIGH]** `test/.../list_cards/journal_card_test.dart` is **1262 lines** — approaching critical size. The file has a large `'Comprehensive entity tests'` group (lines ~568–962) that creates test fixtures inline in `setUp` instead of using existing test data factories. The setup block alone is ~140 lines. Using existing `test_data.dart` fixtures and the `Comprehensive entity tests` group could cut ~200 lines.
