...
- [ ] **[MED]** `test/features/labels/ui/widgets/label_selection_modal_utils_test.dart` — 15 `pumpAndSettle` calls for a modal utility widget. Review whether each can be replaced with `tester.pump(const Duration(milliseconds: 300))` since modal opening animations are fixed-duration.
