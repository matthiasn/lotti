...
- [ ] **[LOW]** `lib/features/labels/utils/label_tool_parsing.dart` already has two Glados tests in `label_tool_parsing_phase2_test.dart`. The `_confidenceToRank` / `_normalizeConfidence` functions are small pure functions not directly tested by Glados (only through the higher-level parser). A Glados test that generates arbitrary confidence strings and asserts: (a) output rank is 0–3; (b) normalized confidence is always one of the four valid strings; (c) unknown inputs map to rank 1 / confidence 'medium' — would make the contract explicit. Tag with `tags: 'glados'`.
...
- [ ] **[LOW]** `label_color_presets.dart` — the `labelColorPresets` list has no structural test: no assertion that all hex strings are valid 7-character `#RRGGBB` strings or that all names are non-empty. The `color_contrast_test.dart` does test WCAG ratios, which provides partial coverage.
