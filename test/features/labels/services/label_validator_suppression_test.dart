...
void main() {
  test('validateForTask separates suppressed from invalid/valid', () async {
    ...
    when(() => db.getLabelDefinitionById('a')).thenAnswer(
    ...
    when(() => db.getLabelDefinitionById('e1')).thenAnswer(
      (_) async => LabelDefinition(
        id: 'e1',
        name: 'Engineering',
        color: '#000',
    ...
    when(() => db.getLabelDefinitionById('d1')).thenAnswer(
      (_) async => LabelDefinition(
        id: 'd1',
        name: 'Design',
        color: '#000',
    ...
    when(() => db.getLabelDefinitionById('z')).thenAnswer(
      (_) async => LabelDefinition(
        id: 'z',
        name: 'Zed',
        color: '#000',
  ...
  test(
    'deleted label in suppressed set is treated as invalid (deletion wins)',
    () async {
      final db = MockJournalDb();
      final validator = LabelValidator(db: db);
