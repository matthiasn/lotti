...
void main() {
  ...
  group('ProjectDetailPage', () {
    ...
    for (final intercepted in [false, true]) {
      testWidgets(
        'system back discards the editor draft (intercepted: $intercepted)',
        (tester) async {
          final nav = MockNavService();
          when(
            () => nav.beamToNamed(any(), data: any(named: 'data')),
          ...
          final controller = await pumpPage(
            tester,
            state: loadedState(hasChanges: true),
            returnPath: intercepted ? '/projects/$_projectId' : null,
          );
          final scope = tester.widget<PopScope<Object?>>(
            find.byWidgetPredicate((widget) => widget is PopScope),
          );
          scope.onPopInvokedWithResult!(!intercepted, null);
          await tester.pump();
          ...
          if (intercepted) {
            ...
            verifyNever(() => nav.beamToNamed(any(), data: any(named: 'data')));
          }
        },
      );
    }

    testWidgets('clearing the target date restores its localized placeholder', (
      tester,
    ) async {
      final controller = await pumpPage(tester, state: loadedState());
      controller.updateTargetDate(null);
      await tester.pump();
      final context = tester.element(find.byType(ProjectDetailPage));
      final targetRow = tester
          .widgetList<DesignSystemSelectionRow>(
            find.byType(DesignSystemSelectionRow),
          )
          .singleWhere(
            (row) => row.title == context.messages.projectTargetDateLabel,
          );
      expect(targetRow.subtitle, context.messages.projectTargetDateLabel);
      expect(controller.lastUpdatedTargetDate, isNull);
      expect(controller.saveChangesCalls, 0);
    });
