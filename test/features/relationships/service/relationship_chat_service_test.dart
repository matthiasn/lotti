...
void main() {
  ...
  test('an orchestrator that shuts down mid-turn fails the turn instead of '
      'hanging — the caller holds the composer disabled for this future, '
      'and no completion for this run can ever arrive now', () async {
    when(
      () => orchestrator.enqueueManualWake(
        agentId: agentId,
        reason: WakeReason.userMessage.name,
      ...
      // The completion stream closes without ever emitting this run's event.
      scheduleMicrotask(completions.close);
      return 'chat-run';
    });
    ...
    await expectLater(
      service.sendMessage(agentId: agentId, text: 'How is Anna?'),
      throwsA(
        isA<RelationshipChatTurnException>()
            .having(
              (error) => error.detail,
              'detail',
              'the agent runtime stopped before the turn completed',
            )
            .having((error) => error.messageId, 'messageId', isNotNull),
      ),
    );
