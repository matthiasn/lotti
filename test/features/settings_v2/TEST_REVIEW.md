...
- [ ] **[LOW]** `settings_tree_node_widget_test.dart` — 2 `pumpAndSettle` calls; both follow `pump()` sequences that already drain async work. Replacing with `pump()` is safe.
...
- [ ] **[LOW]** `panel_registry_test.dart:533–557` — the `setUp` for the list/detail/create group pumps a `MaterialApp` twice (once inside `dispatchFor`, once in the builder test above). The `dispatchFor` helper already encapsulates the pump; a shared `setUpAll` that calls `setUpTestGetIt()` once would remove per-test warm-up. **Estimated impact: ~10–15 % speed-up for that group.**
