...
- [ ] **[HIGH]** `test/…/services/audio_waveform_service_test.dart:1321–1374` — `prunes oldest files when exceeding 1000 entries`: calls `populateCacheEntries(1009)`, which creates 1009 files in the temp directory. On constrained CI runners this is the single most expensive test in the services suite. Consider: (a) using 110 entries instead (10 % of the limit still exercises pruning) or (b) parameterizing `_maxCacheEntries` via the constructor so tests can use a small limit (e.g. 10) and `populateCacheEntries(11)`.
