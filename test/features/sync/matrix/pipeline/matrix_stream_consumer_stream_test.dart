...
void main() {
  ...
  group('stream event processing modes', () {
    ...
    test('live scan deferred during catch-up ensures in-order ingest', () async {
  ...
  test('increments droppedByType when retry cap reached', () async {
    fakeAsync((async) async {
      ...
      when(
        () => processor.process(event: failing, journalDb: journalDb),
  ...
  test(
    'filters events from other rooms and processes only current room',
    () async {
      fakeAsync((async) async {
        final session = MockMatrixSessionManager();
        final roomManager = MockSyncRoomManager();
  ...
  group('MatrixStreamConsumer startup catch-up ordering', () {
    ...
    test(
      'schedules initial catch-up retries until room becomes ready',
      () async {
        fakeAsync((async) async {
          final session = MockMatrixSessionManager();
          final roomManager = MockSyncRoomManager();
  ...
  test(
    'start handles live timeline getTimeline exception gracefully',
    () async {
      final session = MockMatrixSessionManager();
      final roomManager = MockSyncRoomManager();
      final logger = MockLoggingService();
  ...
  group('MatrixStreamConsumer catch-up', () {
    test(
      'processes strictly after lastProcessed and advances marker',
      () async {
        fakeAsync((async) async {
          final session = MockMatrixSessionManager();
          final roomManager = MockSyncRoomManager();
          ...
          verify(
            () => processor.process(event: newA, journalDb: journalDb),
          ).called(1);
          verify(
            () => processor.process(event: newB, journalDb: journalDb),
          ).called(1);
          verify(
            () => readMarker.updateReadMarker(
              client: client,
    ...
    test(
      'prefetches attachments for remote file events during catch-up',
      () async {
        fakeAsync((async) async {
          final session = MockMatrixSessionManager();
          final roomManager = MockSyncRoomManager();
    ...
    test(
      'expands catch-up window to include backlog and does not skip events',
      () async {
        fakeAsync((async) async {
          final session = MockMatrixSessionManager();
          final roomManager = MockSyncRoomManager();
          ...
          Future<Timeline> timelineForLimit(int limit) async {
            final tl = MockTimeline();
            final start = allEvents.length > limit
  ...
  group('MatrixStreamConsumer live timeline + hydration', () {
    test('start hydrates room when missing and attaches live timeline', () async {
      final session = MockMatrixSessionManager();
      final roomManager = MockSyncRoomManager();
      final logger = MockLoggingService();
      final journalDb = MockJournalDb();
      ...
      // on subsequent scans, so the event is only processed once.
      verify(
        () => processor.process(event: newEvent, journalDb: journalDb),
      ).called(1);
    ...
    test('does not advance marker past first failed event in batch', () async {
      fakeAsync((async) async {
        ...
        when(
          () => processor.process(event: a, journalDb: journalDb),
        ).thenAnswer((_) async {});
        when(
          () => processor.process(event: b, journalDb: journalDb),
        ).thenThrow(Exception('fail B'));
        when(
          () => processor.process(event: c, journalDb: journalDb),
        ).thenAnswer((_) async {});
    ...
    test('counts skipped for non-sync msg types', () async {
      ...
      when(
        () => processor.process(event: syncE, journalDb: journalDb),
      ).thenAnswer((_) async {});
      when(
        () => readMarker.updateReadMarker(
          client: any<Client>(named: 'client'),
  ...
  test(
    'circuit breaker opens after many failures and blocks processing',
    () async {
      final session = MockMatrixSessionManager();
      final roomManager = MockSyncRoomManager();
      final logger = MockLoggingService();
      ...
      fakeAsync((async) {
        ...
        expect(
          consumer.metricsSnapshot()['circuitOpens'],
  ...
  group('Metrics exception resilience -', () {
    test('continues when metrics throw during reportDbApplyDiagnostics', () {
      final consumer = buildConsumer(
        metrics: ThrowingMetricsCounters(),
      ).consumer;
  ...
  group('Label generation -', () {
    ...
    test(
      'suppressed self events increment metrics and skip processing',
      () async {
        final session = MockMatrixSessionManager();
        final roomManager = MockSyncRoomManager();
        final logger = MockLoggingService();
  ...
  group('Wake from standby detection', () {
    test(
      'detects wake when gap exceeds threshold and triggers catch-up',
      () async {
        fakeAsync((async) async {
          final session = MockMatrixSessionManager();
          final roomManager = MockSyncRoomManager();
  ...
  group('processing serialization', () {
    ...
    test(
      'waits without timing out when previous batch takes too long',
      () async {
        fakeAsync((async) {
          final session = MockMatrixSessionManager();
          final roomManager = MockSyncRoomManager();
          ...
          expect(
            capturedMessages.any(
              (m) => m.contains('waiting for previous batch to complete'),
            ),
            isTrue,
            reason:
          ...
          expect(
            capturedMessages.any(
              (m) => m.contains('timeout waiting for previous batch, throwing'),
            ),
            isFalse,
  ...
  group('SDK sync wait before catch-up', () {
    test('skips sync wait when no room is configured', () {
      fakeAsync((async) {
        ...
        expect(
          capturedLogs.any(
            (l) => l.contains('No configured room for catch-up'),
          ),
          isTrue,
        );
    ...
    test('waits for sync completion before catch-up and logs synced=true', () {
      fakeAsync((async) {
        ...
        expect(
          capturedLogs.any(
            (l) => l.contains('catchup.waitForSync synced=true'),
          ),
          isTrue,
    ...
    test('times out and sets up pending listener on slow sync', () {
      fakeAsync((async) {
        ...
        expect(
          capturedLogs.any(
            (l) => l.contains('catchup.waitForSync synced=false'),
          ),
          isTrue,
