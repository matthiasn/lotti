...
- [x] **[MED]** **`outbox_state_controller.dart:outboxDailyVolume` provider** (`@riverpod Future<List<Observation>>`) is not tested. It maps `OutboxDailyVolume` entries to `Observation(date, totalBytes / 1024)` — a simple transform that is completely absent from `outbox_state_controller_test.dart`.
