...
- [x] **[MED]** **`BackfillStatsState.copyWith` invariants.** The state now carries only aggregate data, operation flags, and the visible error; operation result counts and their dedicated clear flags were removed because no production surface consumed them.
  **RESOLVED:** the no-arg idempotence property covers every remaining operation flag, while the error-clear property verifies `clearError` without coupling tests to removed diagnostic-only state.
...
- **2 HIGH-priority Glados candidates**: `ProvisioningController.decodeBundle` validation rules and `BackfillStatsState.copyWith` invariants — both now have tagged Glados coverage.
