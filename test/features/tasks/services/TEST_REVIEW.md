...
- [ ] **[HIGH]** `checklistItemsToEmojiList` (`checklist_markdown_exporter.dart:25–30`) — a pure function with the same filter/sanitize/format structure as `checklistItemsToMarkdown`. The existing `_AnyChecklistRows` generator and `_GeneratedChecklistRow` model in `checklist_markdown_exporter_test.dart` can be reused directly. A property confirming that (a) null items and deleted items are excluded, (b) unchecked items use `⬜` and checked items use `✅`, and (c) title sanitization matches the Markdown path's `sanitizedTitle` invariant would provide equivalent Glados coverage. Target `numRuns: 160` to match the Markdown property.
