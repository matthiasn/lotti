...
void main() {
  ...
  group('setSidebarPinned', () {
    test('persists explicit pin membership', () async {
      stubPersisted(const [
        SavedTaskFilter(id: 'sv-1', name: 'A', filter: _filterA),
      ]);
      final container = makeContainer();
      ...
      final result = await container
          .read(savedTaskFiltersControllerProvider.notifier)
          .setSidebarPinned('sv-1', pinned: true);

      final saved = container
          .read(savedTaskFiltersControllerProvider)
          .value!
          .single;
      expect(result, SavedTaskFilterPinResult.updated);
      expect(saved.pinnedToSidebar, isTrue);
      expect(saved.updatedAt, isNotNull);
    });

    test(
      'enforces the five-pin cap and allows replacement after unpin',
      () async {
        final filters = [
          for (var i = 0; i < 6; i++)
            SavedTaskFilter(
        ...
        for (var i = 0; i < maxSidebarPinnedTaskFilters; i++) {
          expect(
            await notifier.setSidebarPinned('sv-$i', pinned: true),
            SavedTaskFilterPinResult.updated,
          );
        }
        expect(
          await notifier.setSidebarPinned('sv-5', pinned: true),
          SavedTaskFilterPinResult.limitReached,
        );
        expect(
          container
              .read(savedTaskFiltersControllerProvider)
              .value!
              .where((filter) => filter.pinnedToSidebar),
          hasLength(maxSidebarPinnedTaskFilters),
        );

        expect(
          await notifier.setSidebarPinned('sv-0', pinned: false),
          SavedTaskFilterPinResult.updated,
        );
        expect(
          await notifier.setSidebarPinned('sv-5', pinned: true),
    ...
    test('does not persist missing or unchanged pin state', () async {
      stubPersisted(const [
        SavedTaskFilter(id: 'sv-1', name: 'A', filter: _filterA),
      ]);
      final container = makeContainer();
      ...
      expect(
        await notifier.setSidebarPinned('missing', pinned: true),
        SavedTaskFilterPinResult.unchanged,
      );
      expect(
        await notifier.setSidebarPinned('sv-1', pinned: false),
        SavedTaskFilterPinResult.unchanged,
      );
