...
- [x] **[HIGH]** `test/features/tasks/ui/checklists/checklists_widget_test.dart` — bypasses `setUpTestGetIt()` entirely. `setUpAll` at line 65 manually calls `await getIt.reset()` and registers 14 singletons inline (lines 67–115). This violates the "Use `setUpTestGetIt()` / `tearDownTestGetIt()`" rule and risks cross-suite contamination in `very_good` batched runs. **RESOLVED:** the setUpAll now goes through `setUpTestGetIt(additionalSetup:)` (swapping in the file's stubbed JournalDb/SettingsDb/UpdateNotifications/LoggingService and adding the extra services) + `tearDownTestGetIt`.
