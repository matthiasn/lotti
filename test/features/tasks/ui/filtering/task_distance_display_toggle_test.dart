// ignore_for_file: avoid_redundant_argument_values

import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:infinite_scroll_pagination/infinite_scroll_pagination.dart';
import 'package:lotti/classes/journal_entities.dart';
import 'package:lotti/features/journal/state/journal_page_controller.dart';
import 'package:lotti/features/journal/state/journal_page_scope.dart';
import 'package:lotti/features/journal/state/journal_page_state.dart';
import 'package:lotti/features/tasks/ui/filtering/task_distance_display_toggle.dart';
import 'package:mocktail/mocktail.dart';

import '../../../../test_helper.dart';

class FakeJournalPageController extends JournalPageController {
  FakeJournalPageController(this._testState);

  final JournalPageState _testState;
  final List<bool> showDistancesCalls = [];

  @override
  JournalPageState build(bool showTasks) => _testState;

  @override
  JournalPageState get state => _testState;

  @override
  Future<void> setShowDistances({required bool show}) async {
    showDistancesCalls.add(show);
  }
}

class MockPagingController extends Mock
    implements PagingController<int, JournalEntity> {}

void main() {
  late MockPagingController mockPagingController;
  late FakeJournalPageController fakeController;

  setUp(() {
    TestWidgetsFlutterBinding.ensureInitialized();

    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(SystemChannels.platform, (
          MethodCall methodCall,
        ) async {
          return null;
        });

    mockPagingController = MockPagingController();
  });

  JournalPageState createState({
    bool showDistances = false,
    bool enableVectorSearch = true,
  }) {
    return JournalPageState(
      match: '',
      tagIds: <String>{},
      filters: {},
      showPrivateEntries: false,
      selectedEntryTypes: const [],
      fullTextMatches: {},
      showTasks: true,
      pagingController: mockPagingController,
      taskStatuses: const ['OPEN', 'GROOMED', 'IN PROGRESS'],
      selectedTaskStatuses: {'OPEN'},
      selectedCategoryIds: {},
      selectedLabelIds: const {},
      showDistances: showDistances,
      enableVectorSearch: enableVectorSearch,
    );
  }

  Widget buildSubject(JournalPageState state) {
    fakeController = FakeJournalPageController(state);

    return WidgetTestBench(
      child: ProviderScope(
        overrides: [
          journalPageScopeProvider.overrideWithValue(true),
          journalPageControllerProvider(
            true,
          ).overrideWith(() => fakeController),
        ],
        child: const TaskDistanceDisplayToggle(),
      ),
    );
  }

  group('TaskDistanceDisplayToggle', () {
    testWidgets('renders SwitchListTile when vector search is enabled', (
      tester,
    ) async {
      await tester.pumpWidget(buildSubject(createState()));
      await tester.pumpAndSettle();

      expect(find.byType(TaskDistanceDisplayToggle), findsOneWidget);
      expect(find.byType(SwitchListTile), findsOneWidget);
      expect(find.text('Show distances on cards'), findsOneWidget);
    });

    testWidgets('renders nothing when vector search is disabled', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildSubject(createState(enableVectorSearch: false)),
      );
      await tester.pumpAndSettle();

      expect(find.byType(SwitchListTile), findsNothing);
      expect(find.byType(SizedBox), findsWidgets);
    });

    testWidgets('Switch is off when showDistances is false (default)', (
      tester,
    ) async {
      await tester.pumpWidget(buildSubject(createState()));
      await tester.pumpAndSettle();

      final switchWidget = tester.widget<Switch>(find.byType(Switch));
      expect(switchWidget.value, isFalse);
    });

    testWidgets('Switch is on when showDistances is true', (tester) async {
      await tester.pumpWidget(
        buildSubject(createState(showDistances: true)),
      );
      await tester.pumpAndSettle();

      final switchWidget = tester.widget<Switch>(find.byType(Switch));
      expect(switchWidget.value, isTrue);
    });

    testWidgets(
      'calls setShowDistances(show: true) when Switch is turned on',
      (tester) async {
        await tester.pumpWidget(buildSubject(createState()));
        await tester.pumpAndSettle();

        await tester.tap(find.byType(Switch));
        await tester.pump();

        expect(fakeController.showDistancesCalls, contains(true));
      },
    );

    testWidgets(
      'calls setShowDistances(show: false) when Switch is turned off',
      (tester) async {
        await tester.pumpWidget(
          buildSubject(createState(showDistances: true)),
        );
        await tester.pumpAndSettle();

        await tester.tap(find.byType(Switch));
        await tester.pump();

        expect(fakeController.showDistancesCalls, contains(false));
      },
    );
  });
}
