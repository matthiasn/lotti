...
void main() {
  ...
  group('TaskDueDateDisplayToggle', () {
    testWidgets('renders correctly with SwitchListTile and label',
      ...
      expect(find.byType(TaskDueDateDisplayToggle), findsOneWidget);

      // Verify SwitchListTile is present (which contains Switch internally)
      expect(find.byType(SwitchListTile), findsOneWidget);

      // Verify label text is present
      expect(find.text('Show due date on cards'), findsOneWidget);
    });

    testWidgets('Switch is on when showDueDate is true', (tester) async {
      await tester.pumpWidget(buildSubject(createState()));
      await tester.pumpAndSettle();
    ...
    testWidgets('Switch is off when showDueDate is false', (tester) async {
    ...
    testWidgets('calls setShowDueDate(show: true) when Switch is turned on',
      ...
      expect(fakeController.showDueDateCalls, contains(true));
    });
    ...
    testWidgets('SwitchListTile contains label and switch', (tester) async {
      ...
      expect(
        find.descendant(
          of: listTileFinder,
          matching: find.text('Show due date on cards'),
        ),
        findsOneWidget,
      );
    });
