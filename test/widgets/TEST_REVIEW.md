...
| `lib/widgets/app_bar/journal_sliver_appbar.dart` | — | — | **Removed** — deleted in the logbook restyle; its filter/search-mode widgets moved to `lib/features/journal/ui/widgets/logbook_filter_modal.dart` and `logbook_search_mode_row.dart`, with mirror tests under `test/features/journal/ui/widgets/` |
...
- [x] **[MED]** `test/widgets/app_bar/journal_filter_icon_test.dart` (line 117) — "is wrapped in Padding with correct spacing" test asserts `findsWidgets` (plural, uncounted) on any `Padding` ancestor of `IconButton`. This is too loose to catch regressions; assert on the specific `EdgeInsets` value. **RESOLVED:** done — asserted `EdgeInsets.only(right: AppTheme.spacingSmall)` on the nearest Padding ancestor. (Superseded: `journal_sliver_appbar.dart` and `JournalFilterIcon` were deleted in the logbook restyle; the padding no longer exists.)
