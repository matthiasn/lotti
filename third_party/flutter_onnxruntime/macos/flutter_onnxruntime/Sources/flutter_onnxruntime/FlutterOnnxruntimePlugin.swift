...
public class FlutterOnnxruntimePlugin: NSObject, FlutterPlugin {
  ...
  // LOTTI FORK PATCH (see third_party/flutter_onnxruntime/LOTTI_PATCHES.md):
  // macOS Flutter does not implement platform-channel background task queues
  // (flutter/flutter#162613, still open), so `safeMakeBackgroundTaskQueue`
  // returns nil and method calls — including multi-hundred-millisecond ONNX
  // inference — would otherwise run synchronously on the main thread, freezing
  // the UI during synthesis. Run all handler work on a dedicated serial
  // background queue instead: it is serial (so the shared session/value maps
  // are never touched concurrently) and the existing `lock` still guards
  // against `cleanupResources` on app termination.
  private let workQueue = DispatchQueue(label: "com.lotti.flutter_onnxruntime.work")
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    // LOTTI FORK PATCH: hop off the main thread (see workQueue above). The
    // reply via `result(...)` is delivered on this background thread, which
    // FlutterMethodChannel marshals back to the engine safely.
    workQueue.async { [weak self] in
      self?.handleLocked(call, result: result)
    }
  }

  // swiftlint:disable:next cyclomatic_complexity
  private func handleLocked(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    lock.lock()
    defer { lock.unlock() }
